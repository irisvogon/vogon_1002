...
* void    ArrayOfPoints(INTERFACE *intfc, float *coords);
* void    ArrayOfTri(SURFACE *surface, TRI **tris);
* void    ArrayOfTri(SURFACE *surface, float *coords, int *vertices_index);
* void    ArrayOfTri(INTERFACE *intfc, TRI **tris);
* void    ArrayOfTri(INTERFACE *intfc, float *coords, int *vertex_indices);
*
...
int main(int argc, char **argv)
{
	...
	mc_params.num_cir = 1;
        ...
        matrix(&mc_params.cen,mc_params.num_cir,3,FLOAT);
...
static float multi_circle_func(
        POINTER func_params,
        float *coords)
{
        ...
        dist = sqrt(sqr(coords[0] - cen[imin][0]) +
                    sqr(coords[1] - cen[imin][1]) +
		    sqr(coords[2] - cen[imin][2])) - rad[imin];
        return dist;
}       /* end ellipse_func */
	...
	Front *front)
{
	...
	int dim = 3;
	float *coords = (float*)malloc(num_points*dim*sizeof(float));
