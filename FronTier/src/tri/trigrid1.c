	...
	TRI_GRID	*grid)
{
	...
	NODE		**n;
	RECT_GRID	*expanded_dual_grid = &grid->rect_grid;
	float		s, *posn, *h = expanded_dual_grid->h;
	float		ntol[MAXD];
	int		**ic_of_node = NULL;
	int		is, i, n_index;
	int		dim = intfc->dim;
	...
	size_t		sizest)
{
	...
	case 1:
	{
	    register float	*xx_grid = expanded_dual_grid->edges[0];
	    int			xmax = gmax[0];
	    int			ix;
	...
	case 2:
	{
	    register float y;
	    register float *xx_grid = expanded_dual_grid->edges[0];
	    register float *yy_grid = expanded_dual_grid->edges[1];
	...
	case 3:
	{
	    ...
	    register float *xx_grid = expanded_dual_grid->edges[0];
	    register float *yy_grid = expanded_dual_grid->edges[1];
	    register float *zz_grid = expanded_dual_grid->edges[2];
...
EXPORT	float area_of_domain_with_comp(
	Front *front,
	COMPONENT comp)
{
	TRI_GRID *grid;
