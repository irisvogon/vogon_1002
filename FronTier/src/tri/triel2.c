...
LOCAL	bool	crxl_in_blk_crx_list(BLK_CRX_LIST*,BLK_CRX_LIST*);
LOCAL	int	is_proper_elN1(INTERFACE*,BOND***);
LOCAL	int	point_in_crx_list(POINT*,BOND*,CRXING**,CRXING**);
LOCAL	bool	triangulate_elN1(TRI_GRID*,int*,int,int*,int,int,int*,int,int,
...
/*ARGSUSED*/
LIB_LOCAL	bool triangulate_blk_with_nodes(
	TRI_GRID	*ntg,
	int		*icoords,
	LINEAR_ELEMENT	**tri,
	BILINEAR_ELEMENT **qd)
...
EXPORT	bool exact_blk_triangulate(
	TRI_GRID	*ntg,
	int		*icoords,
	LINEAR_ELEMENT	**lin,
	...
	int		*t_num)
{
	...
	TG_PT		 *p0, *p1, *p2;
	float		 dx1, dy1, dx2, dy2, dx11, dy11, dx22, dy22, ang1, ang2;
...
LOCAL bool crxl_in_blk_crx_list(
	BLK_CRX_LIST	*crxl,
	BLK_CRX_LIST	*head)
{
	BLK_CRX_LIST	*cxl;
	...
	INTERFACE	*blk_intfc)
{
	...
	float		tol = 0.5*IG_NTOL*(gr->h[0] + gr->h[1]);/*TOLERANCE*/
