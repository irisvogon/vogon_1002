...
struct _RING_POINT
{
	...
	union
	{
	    float *coords;	/* coordinates of position */
...
LOCAL const float MIN_RING_POINT_SEPARATION        = 1.0e-4; /*TOLERANCE*/
LOCAL const float MIN_RING_POINT_SCALED_SEPARATION = 1.0e-8; /*TOLERANCE*/
...
LOCAL	Locstate	find_state_at_corner(TRI_GRID*,NODE*,float,float,
...
LOCAL	bool	check_and_break_holes(INTERFACE*,ORIENTATION,CURVE*,TRI_CURVE**,
				      TRI_GRID*);
LOCAL	bool	cut_a_piece(CURVE***,NODE***,ORIENTATION,
			    CURVE*,CUT_A_PIECE*,ORIENTATION*);
LOCAL	bool	delete_tri_curve(TRI_CURVE*,TRI_CURVE**);
LOCAL	bool	find_boundary_curve(TRI_CURVE*,TRI_CURVE**);
LOCAL	bool	find_largest_angle_and_split(RING_POINT**,RING_POINT**,
					     int*,int,float*,LIN_EL_FACE_TYPE,
					     float*,float);
LOCAL	bool	find_tri_curve_from_curve(TRI_CURVE*,CURVE*,TRI_CURVE**);
LOCAL	bool	insert_curve_in_xylist(CURVE*,ORIENTATION,CUT_A_PIECE*);
LOCAL	bool	inside_polygon(CURVE*,XY*,XY*);
LOCAL	bool	is_a_hole(CURVE**,TRI_CURVE*,XY*,XY*);
LOCAL	bool	is_cross(POINT*,POINT*,BOND*,int);
LOCAL	bool	is_cross_in_inserted_curve(POINT*, POINT*, INTERFACE*);
LOCAL	bool	is_intersect_with_other_edge(RING_POINT*,RING_POINT*,int,float);
LOCAL	bool	make_hole_cut(NODE*,NODE*,COMPONENT,COMPONENT,
			      TRI_CURVE**,TRI_GRID*);
LOCAL	bool	recursive_decomposition_elN(TRI_GRID*,LINEAR_ELEMENT**,int*,
					    TRI_CURVE*,ORIENTATION,INTERFACE*,
					    TRI_CURVE**);
LOCAL	bool	recursive_triangulate(RECUR_TRI*,RING_POINT**,int,float);
LOCAL	bool	same_comp_on_ring(RING_POINT*);
LOCAL	bool	set_bdry_states_and_tri_curves(TRI_GRID*,INTERFACE*,
					       TRI_CURVE**,float,float,float,
					       float,int,int,int,int);
LOCAL	bool	set_nodes_for_make_hole_cut(NODE**,NODE**,NODE**,NODE**,
...
LOCAL	void	delete_small_separation_ring_points(RING_POINT**,int*,
						    float*,int);
LOCAL	void	evaluate_angles(RING_POINT*,float*);
...
LOCAL	bool	generate_tri(TRI_GRID*,int**,RING_POINT**,int,int,int,
			     LINEAR_ELEMENT**,int*);
LOCAL	bool	optimal_triangulation(TRI_GRID*,LINEAR_ELEMENT**,int*,
				      RING_POINT**,int);
LOCAL	bool	set_distance(float**,int,RING_POINT*,float,int);
LOCAL	float	area_or_ratio(int,int,int,RING_POINT**,float**,float);
LOCAL	void	optimization(float**,int**,float**,RING_POINT**,int,float);
LOCAL	void	print_cost(float**,int);
...
LIB_LOCAL	bool exact_triangulate_el3(
	TRI_GRID	*ntg,
	NODE		**node,
	int		num_nodes,
	int		*icoords,
...
LOCAL	bool recursive_decomposition_elN(
	TRI_GRID	*ntg,
	LINEAR_ELEMENT	**tri,
	int		*t_num,
	TRI_CURVE	*tri_bc,
...
LOCAL	bool set_bdry_states_and_tri_curves(
	TRI_GRID	*ntg,
	INTERFACE	*intfc,
	TRI_CURVE	**tri_curves,
	float		XL,
...
LOCAL	bool find_boundary_curve(
	TRI_CURVE	*tri_curves,
	TRI_CURVE	**tri_bc)
{
	TRI_CURVE *tri_c;
...
LOCAL bool check_and_break_holes(
	INTERFACE	*blk_intfc,
...
LOCAL 	bool insert_curve_in_xylist(
	CURVE		*c,
	ORIENTATION	orient,
	CUT_A_PIECE	*cpiece)
{
...
LOCAL bool optimal_triangulation(
	TRI_GRID	*ntg,
	LINEAR_ELEMENT	**tri,
	int		*t_num,
	RING_POINT	**p_ring,
	int		num_pts_on_ring)
{
	...
	float              *h = ntg->comp_grid.h;
	float              ls;
	int                dim = ntg->comp_grid.dim;
	int 		   i;
	static	RING_POINT **pt_list = NULL;
	static	float 	   **distance = NULL, **cost = NULL;
...
LOCAL bool find_tri_curve_from_curve(
	TRI_CURVE	*tri_curves,
	CURVE		*curve,
	TRI_CURVE	**tri_c)
{
	...
	TRI_GRID	*ntg)
{
	...
	Ring_point_coords(&Tmp_ring) = (float *)((ntg->front_points)+i);
#if defined(DEBUG_TRI_GRID)
	if (debugging("ring"))
	{
...
LOCAL  bool set_nodes_for_make_hole_cut(
	NODE		**n1,
	NODE		**n2,
	NODE		**piece_n_list,
	NODE		**hole_n_list,
...
LOCAL	bool is_cross_in_inserted_curve(
	POINT		*p1,
	POINT		*p2,
	INTERFACE	*blk_intfc)
{
...
LOCAL bool recursive_triangulate(
	RECUR_TRI	*recur_tri,
	RING_POINT	**p_ring,
	int		num_pts_on_ring,
	float		ls)
...
LOCAL bool find_largest_angle_and_split(
	RING_POINT	 **p_ring,
...
LOCAL bool is_intersect_with_other_edge(
	RING_POINT	*p_ring,
	RING_POINT	*split_ring,
	int		dim,
	float		ls)
