...
LOCAL	bool build_linear_element(POINT_COMP_ST*,int,COMPONENT,float*,
				     LINEAR_ELEMENT**,TRI_GRID*);
LOCAL	bool collect_least_sqr_cluster(POINT_COMP_ST*,int,COMPONENT,float*,
...
EXPORT	bool nearest_crossing_state_with_comp(
	int *icoords,
	float *coords,
	COMPONENT comp,
	TRI_GRID *grid,
	...
	TRI_GRID	*ntg)
{
	...
	tri_grid_index_from_comp_grid_index(ic,icoords,ntg);
	if(debugging("parab_crx"))
	{
	    print_rectangular_grid(&ntg->tg_grid);
	    print_rectangular_grid(&ntg->rect_grid);
	    print_int_vector("icrds ", icoords, 3, "\n");
	    print_int_vector("ic ", ic, 3, "\n");
	}
	gmax = ntg->rect_grid.gmax;

	//ic is on the edge of the top grid.
	for (i = 0; i < dim; ++i)
	{
	    if (ic[i] < 0 || ic[i] > gmax[i])
	    	return NULL;
	    if(ic[i] == 0 && dir == dir_l[i])
		return NULL;
	    if(ic[i] == gmax[i] && dir == dir_u[i])
	    	return NULL;
	}
...
LOCAL	bool	point_is_inside_hull(
		POINT_COMP_ST	**pcs_list,
		int		npcs,
		float		*pt,
		RECT_GRID	*gr)
{
	int	i, j, k, dir, dir1, dir2;
	float	*x1, *x2, d, d1, d2, z, v0, v1, hull_tol;
	float	vpx, vpy, vnx, vny;
	float	vpxp, vpyp, vnxn, vnyn;
	float	cang, pmin, mmin, pmax, mmax;
	bool	pf, nf;

	hull_tol = 1.0e-6*min3(gr->h[0], gr->h[1], gr->h[2]);
	...
	for(i=0; i<npcs-1; i++)
	    for(j=i+1; j<npcs; j++)
	    {
		x1 = Coords(pcs_list[i]->p);
		x2 = Coords(pcs_list[j]->p);
		d1 = x1[dir] - z;
		d2 = x2[dir] - z;
		...
		{
		    v0 = (x1[dir1]*d2 + x2[dir1]*d1)/(d1 + d2) - pt[dir1];
		    v1 = (x1[dir2]*d2 + x2[dir2]*d1)/(d1 + d2) - pt[dir2];
		}
	...
	//all points are on upper or lower half of the plane.
	if(pf == NO || nf == NO)
	    return NO;
	
	//vpy > 0  and  vny < 0, vp \times vn > 0
	if(vpx*vny-vpy*vnx < 0.0 || vpxp*vnyn-vpyp*vnxn > 0.0)
	    return NO;
	...
	TRI_GRID	*ntg)
{
	...
	if(dim == 3)
	{
	    hull_flag = point_is_inside_hull(pcs_list, npcs, 
			    coords, &(ntg->rect_grid));
	    if(hull_flag == NO)
		return FUNCTION_FAILED;
	}
...
LOCAL	bool collect_least_sqr_cluster(
	POINT_COMP_ST	*pcs,
	int		num_els,
	COMPONENT	comp,
	float		*coords,
