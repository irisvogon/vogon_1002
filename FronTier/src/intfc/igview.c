...
LOCAL   void    gview_plot_color_surfaces(INTERFACE*,RECT_GRID*,const float*,const float*, bool,const char*,const char*,bool);

LOCAL   char    *get_vtk_file_name(char*,const char*,const char*,size_t*);
LOCAL   void    vtk_print_box(const char*,const float*,const float*,bool);
LOCAL   void    vtk_plot_surfaces(INTERFACE*,const float*,
                                  const float*,bool,
                                  const char*,const char*,
                                  bool,SURFACE_COLOR,SURFACE_COLOR,bool);
LOCAL   void    vtk_plot_curves( INTERFACE*, const float*, 
                                 const float*, const char*, 
                                 const char*,SURFACE_COLOR,int,bool);
LOCAL	void    tecplot_surface_special(const char*,FILE*,SURFACE*);
LOCAL	void	tecplot_plot_surfaces(INTERFACE*,RECT_GRID*,const float*,
	                            const float*,bool,
				    const char*,const char*,
				    bool,SURFACE_COLOR,SURFACE_COLOR);
...
LOCAL   void    initialize_image(gdImagePtr, char*, int, int);
LOCAL   void    freecolors();
LOCAL   void    setcolors(gdImagePtr, int);
LOCAL   float   xtransform(float);
LOCAL   float   ytransform(float);
...
EXPORT  void tecplot_show_box_tris(
	...
	int		*icrds)
{
	float		crds_st[MAXD], crds_ed[MAXD];
	float           *L, *h;
	int		i;
	FILE		*file=NULL;
...
LOCAL float	shx=0.0, shy=0.0, shz=0.0;
	...
	FILE		*file)
{
	...
	{
	    if(NO && the_tri(tris[i]))
	    {
		//printf("the tri test\n");
		print_tri(tris[i], tris[i]->surf->interface);
	    }
	...
	BLK_TRI *blk_tri)
{
	...
	for (i = 0; i < blk_tri->num_surfaces; i++)
        {
            for(j = 0, tri = blk_tri->first[i]; j < blk_tri->num_tris[i];
             ++j, tri = tri->next)
            {
		if (the_tri(tri))
		    goto PRINT;
	...
	int        num_tris)
{
        ...
        for(k = 0; k<num_tris;  k++)
	{
	    for (j = 0; j < 3; j++)
	    {
	        p = Point_of_tri(tris[k])[j];
		//if(pp_mynode() == 9)
		//(void) fprintf(file,"%24.16e %24.16e %24.16e\n",
		//	  	   Coords(p)[0],Coords(p)[1]+1.0,Coords(p)[2]);
		//else
		(void) fprintf(file,"%24.16e %24.16e %24.16e\n",
			  	   Coords(p)[0],Coords(p)[1],Coords(p)[2]);

	    }
	    //print_tri(tris[k], tris[k]->surf->interface);
	}
...
EXPORT  void    tecplot_surface_curve(
		FILE		*file,
		SURFACE		*s)
{
	CURVE	**cc;
	...
	//print curves
	for (cc = s->pos_curves; cc && *cc; ++cc)
	{
	    //printf("Curve %d: cc %p *cc %p\n",i,cc,*cc);
	    tecplot_curve(NULL,file,*cc);
	}
	
	for (cc = s->neg_curves; cc && *cc; ++cc)
	{
	    //printf("Curve %d: cc %p *cc %p\n",i,cc,*cc);
	    tecplot_curve(NULL,file,*cc);
	}
...
LOCAL  float  tst_pt[3] = { 1.01255, 0.1418, 1.435 };
LOCAL  float  tst_pt1[3] = { 1.01255, 0.1418, 1.435 };

EXPORT	void	set_tst_posn(float    *pos)
{
		...
		SURFACE		*s)
{
	...
	float	tst_pt[3] = {  0.5227539526042586,     0.6525225774422274,       2.1854392147488 };
	float	tst_pt1[3] = {0.000132,  0.010040,  -0.002132 };
	float	tol = 1.5e-3;
...
void	tri_bound_block(float **, TRI *);
bool	blocks_sect(float**, float**);
	...
	FILE 		*file)
{
	TRI		*tri, *tris[TEC_MAX_TRIS];
	int		i,num_tris;
	static float	**fbox = NULL, **bbox = NULL;
		...
		SURFACE		*s)
{
	...
	for (tri=first_tri(s),npts=0; !at_end_of_tri_list(tri,s); tri=tri->next)
	{
		...
		if (Index_of_point(p) == -1)
		{
		    ...
		    //if(the_point(p))
			//print_general_vector("pt found", Coords(p), 3, "\n");
		}
	    }
	}
	for (tri=first_tri(s); !at_end_of_tri_list(tri,s); tri=tri->next)
	...
	POINT *p)
{
	...
	INTERFACE *intfc = p->tri->surf->interface;
	TRI **ptris;
	POINT *pp,*pts[20];
	int i,j,k,np1,nt,nt1,nt2,num_bonds;
	TRI *tris1[20],*tris2[40],*t;
	bool pp_in_list,tri_in_list;
	static const char *indent = "    ";

	nt1 = set_tri_list_around_point(p,p->tri,&ptris,intfc);
	np1 = 0;
	for (i = 0; i < nt1; ++i)
	{
	    t = tris1[i] = ptris[i];
	...
	for (i = 0; i < np1; ++i)
	{
	    nt = set_tri_list_around_point(pts[i],pts[i]->tri,&ptris,intfc);
	    for (j = 0; j < nt; ++j)
	    {
	    	t = ptris[j];
        ...
        bool          bdry)
{
        ...
        int               npt[3], ntri[3];
        static const char *indent = "    ";
        static float      *pos_pts = NULL;
        static float      *neg_pts = NULL;
        static int        *verts = NULL;
        static int        alloc_len_verts = 0, alloc_len_pts = 0;
        static char       *fname = NULL;
        static size_t     fname_len = 0;
	...
	bool print_in_binary)
{
	...
	if (intfc->dim == 2)
	vtk_plot_curves(intfc,BBL, BBU,dname,"2d-intfc", pRED, 10, print_in_binary);

	if (intfc->dim == 3)
	{
	vtk_plot_surfaces(intfc,BBL,BBU,NO,dname,"3d-intfc",
			    NO,pRED,pRED,print_in_binary);
...
LOCAL float xtransform(float x)
{
	register float xmax = PLOT_DATA.xmax - 0.1*PLOT_DATA.xheight;
	register float xmin = PLOT_DATA.xmin + 0.1*PLOT_DATA.xheight;

        return (int)( ((float)(xmax-xmin)/
	  (PLOT_DATA.maxx-PLOT_DATA.minx))*(x-PLOT_DATA.minx)
	  + xmin);
}	//END xtransform()

// Transforms given coordinates into pixel coordinates on the graph
LOCAL float ytransform(float y)
{
	register float ymax = PLOT_DATA.ymax - 0.1*PLOT_DATA.yheight;
	register float ymin = PLOT_DATA.ymin + 0.1*PLOT_DATA.yheight;

        return PLOT_DATA.ymax-(int)(((float)(ymax-ymin)/
	...
	int bg)
{
	...
	//draw box axes
	int bot =   PLOT_DATA.ymax - (int) (0.1*(float)PLOT_DATA.yheight);
	int top =   PLOT_DATA.ymin + (int) (0.1*(float)PLOT_DATA.yheight);
	int left =  PLOT_DATA.xmin + (int) (0.1*(float)PLOT_DATA.xheight);
	int right = PLOT_DATA.xmax - (int) (0.1*(float)PLOT_DATA.xheight);
	...
	for(i=0; i<N; i++)
	{
	    ...
	    gdImageLine(im,x,bot,x,bot+(int)(0.01*(float)PLOT_DATA.yheight),fg);
   	    sprintf(s,"%.2f", 
	...
	for(i=0; i<N; i++)
	{
	    ...
	    gdImageLine(im,left,x,
	    	left-(int)(0.01*(float)PLOT_DATA.xheight),x,fg);
   	    sprintf(s,"%.2f", 
...
EXPORT void gd_initplot(char *file, char *title, 
	      float minx, float maxx, float miny, float maxy, int Numi)
{
	//Window range
	...
	int bg = gdImageColorAllocate(PLOT_DATA.im, 
		PLOT_DATA.bg[0], PLOT_DATA.bg[1], PLOT_DATA.bg[2]);
	int fg = gdImageColorAllocate(PLOT_DATA.im, 
		PLOT_DATA.fg[0], PLOT_DATA.fg[1], PLOT_DATA.fg[2]);
	PLOT_DATA.fg_col = fg;
	PLOT_DATA.bg_col = bg;
	...
	INTERFACE  *intfc)
{
	...
	for (s = intfc->surfaces; s && *s; ++s)
	{
	    for (tri = first_tri(*s); !at_end_of_tri_list(tri,*s);
	    		tri = tri->next)
	    {
		const float *norm = Tri_normal(tri);
		float denom = Mag3d(norm);
