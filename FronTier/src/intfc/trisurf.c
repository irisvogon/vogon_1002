...
LOCAL   float   value_x(float*,POINT*,float*);
LOCAL   float   value_y(float*,POINT*,float*);
LOCAL   float   value_z(float*,POINT*,float*);
LOCAL   void    set_curve_bdry_flag(CURVE*,RECT_GRID*);
LOCAL   void    normal_of_plane_surface(SURFACE*,float*);
...
EXPORT  void  planar_surface_triangulation(
	SURFACE      *surf,
	RECT_GRID    *grid,
	const bool expand_buffer_zones)
{
	float        coords[MAXD];
	float        x1,x2,x3,x4,y1,y2,y3,y4,z1,z2,z3,z4,x,y,z;
	CURVE        **c1;
	POINT        *p1,*p2;
	...
	float   *nor)
{
	...
	BOND  *b;
	POINT *p;
	float area, p_cnt[MAXD], p1[MAXD], p2[MAXD];
	float pnor[3], nnor[3];
	int   i, num, dim;
