...
LOCAL	C_BOND	*test_coplanar_cross(TRI*,TRI*,int,bool*,INTERFACE*);
LOCAL	C_BOND	*test_cross(TRI*,TRI*,bool*,INTERFACE*);
...
LOCAL	bool	next_adjacent(C_BOND*,C_BOND*);
LOCAL	bool	plane_segments_cross(float*,float*,float*,float*);
LOCAL	bool	prev_adjacent(C_BOND*,C_BOND*);
LOCAL	bool    same_cb_pt(POINT*,POINT*);
	...
	INTERFACE *intfc)
{
	C_BOND      *new_cb,**cbs;
	POINT       *q[3]; 
	float  	    line[MAXD];     /* intersection line of t0, t1 planes */
	const float *tnor;

	struct {
	    float   coords[MAXD];    /* point of boundary cross */
	    double  order;           /* projection of coords on line */
	    bool edge_vertex;     /* is it a edge (1) or vertex (0) */
	...
	INTERFACE *intfc)
{
	...
	float       *p[6], pbar[3], lambda[3], *r[3], r0[3], r1[3], r2[3];
	float       p0[3], p1[3], v0[6], v1[6], ds[3], dn[3];
	float       nor[3], nor0[3], nor1[3];
	float       mag, mag0, mag1;
	const float *tnor;
	...
	case 0:
	    ...
	    p[4] = Coords(pt1[1]);
	    p[5] = Coords(pt1[2]);
	    affine_fit((const float* const*)p,3,6,nor,pbar,r,lambda);
	    for (i = 0; i < 3; ++i)
	    {
	...
	case 1:
	    ...
	    affine_fit((const float* const*)p,3,5,nor,pbar,r,lambda);
	    for (i = 0; i < 3; ++i)
	    {
	        v0[2*i]   = (Coords(pt0[i])[0] - pbar[0])*r0[0] +
	                    (Coords(pt0[i])[1] - pbar[1])*r0[1] +
...
LOCAL	bool plane_segments_cross(
	float *s0,
	float *e0,
	float *s1,
	float *e1)
	...
	INTERFACE  *intfc)
{
	...
	color_tri = (float*)store(4*n*FLOAT);
	color_tri[0] = 1.0;
	color_tri[1] = 0.0;
	color_tri[2] = 0.0;
	color_tri[3] = 1.0;
