...
struct _INTERFACE
{
	...
	//#bjet2	//interface in previous time step, used for fill comps
...
struct _EDGE {
        ...
        float           length;
        float           cen[2];    //center point of the edge
};
...
struct _BLK_CRX {
        ...
        BLK_INFO        *blk_info;
        BLK_TYPE        blk_type;
	float		****corner_coords;	/* for volume fraction */
	float		h[MAXD];		/* length in each dimension */
	float		cell_volume;		/* volume of regular cell */
	COMPONENT       comp_vfrac;		/* comp of volume fraction */
	bool		debug_flag;	/* for debugging the block */
};
...
typedef struct { float _coords[MAXD];} TG_PT;
...
enum {
        MAX_NUM_CRX  = 10, /* Maximum number of crossing on a block side */
...
typedef struct {
        float   cone_ratio, max_cos;
        float   alpha;          //alpha must be irrational.
}       SMOOTH_TOL;
