...
LOCAL   void  set_nel_max_normal(POINT*,TRI**,int,float*);
...
EXPORT	void scaled_tri_params(
	...
	float *h,
	float *scaled_area,
	float *scaled_aspect_ratio)
{
	float *p0 = Coords(Point_of_tri(t)[0]);
	float *p1 = Coords(Point_of_tri(t)[1]);
	float *p2 = Coords(Point_of_tri(t)[2]);
	...
	*scaled_aspect_ratio = (*scaled_area)/sqr(len[0]+len[1]+len[2]);
	...
	float		   *nor)
{
	...
	if ((Tri_list_at_vertex.num_tris+2) > num_pts)
	{
	    ...
	    uni_array(&pts,num_pts,sizeof(float*));
	}
	n = 0;
...

{
	POINT        *pp;
	int          n,i,j, v;
	float        pbar[3], lambda[3], dv[3], lenv;
	float        *pts[MAX_PTS]; 
	static float **r = NULL;
	...
	affine_fit((const float* const*)pts,3,n,Tri_normal(tris1[0]),pbar,r,lambda);
	...
	float		   *nor)
{
	...
	if(debugging("db_nor"))
	{
	    ...
	    printf("calc af\n");
	    sprintf(fname, "db_nor%d", cnt);
	    if(cnt == 0)
		set_shift_for_tecplot(0.0, 0.25, 0.0);
	    else
		set_shift_for_tecplot(0.0, 0.0, 0.0);
	    tecplot_tris(fname, Tri_list_at_vertex.tris, Tri_list_at_vertex.num_tris);
	    cnt++;
...

{
        int     i,j,v;
	float   dtheta,length;
	float   wm0[3],wm1[3],tmp_wm0,tmp_wm1;
	const float   *tnor;
	TRI     *tri;
	POINT   *pn0,*pn1;
...
struct _TRI_FullGeometry {
	...
	float *_side_vector[3];	  /* side uni_arrays v[i] = p[i+1]-p[i] */
};
...
LOCAL void polyfit2d_lhf(float*,float*,float*,int,int,int);
LOCAL void polyfit3d_lhf(float*,int*,float*,float*,float*,int,float*,
                                        int,int,int);
LOCAL void obtain_orthbases(float*,float*);
LOCAL void eval_curvature_lhf_surf(float*,float*,float*,float H[2][2]);
LOCAL int safeqr(float*,int*,int*,float*,int*,float);
LOCAL int eval_vander_bivar(float*,int,float*,int*,int,float*,int,int);
...
EXPORT bool WLSP_compute_normal2d(
        POINT           *p,
        HYPER_SURF_ELEMENT      *hse,
        HYPER_SURF              *hs)
{
        ...
        int deg)
{
        ...
        float b_1[13];
        float ws[7];
        static float dtemp_0[26];
        static float dtemp_1[91];
        int npnts, ncols, rnk, n, i;
        float tng[2], nrm[2];
        float Q_rotation[2][2];
        float u_max, w, f_t, f_tt, ell;
        float dtemp_2, dtemp_3, dtemp_4, dtemp_5, dtemp_6;
        int j;
        float dtemp_7;
...
LOCAL void  polyfit3d_lhf(
	...
	int strip)
{
   	static float us[3075];
   	int us_dim1;
   	static float bs[1025];
   	int bs_dim1;
   	static float nrms_v[3075];
   	int nrms_v_dim1;
   	static float ws_row[1025];
   	int ws_row_dim1;
   	float cs[6];
   	int cs_dim1;
   	static float dtemp_0[3075];
   	static float dtemp_1[3075];
   	int dtemp_1_dim1;
   	static float dtemp_2[3075];
   	int strip_1, nverts, n, itemp_0, itemp_1;
   	float absnrm[3], t1_1[3], t2[3], nrm_l[3], maxprdir_l[3];
   	int t1[3];
   	float grad[2];
   	float P[3][3];
   	float H[2][2];
   	...
   	float dtemp_3[3];
   	float dtemp_4, dtemp_5, dtemp_6, dtemp_7, dtemp_8;
   	float dtemp_9, dtemp_10, dtemp_11, dtemp_12, dtemp_13;
   	float dtemp_14, dtemp_15, dtemp_16, dtemp_17, dtemp_18;
   	int itemp_2, i, j;
   	float dtemp_19;
   	...
   	for (i=0; i<=2; i+=1) 
	{
   	...
   	dtemp_4 = 0.0;
   	for (i=0; i<=2; i+=1) 
      	    dtemp_4 = dtemp_4 + (((float) t1[i])) *  nrm_out[i];
   	dtemp_5 = 0.0;
   	for (i=0; i<=2; i+=1) 
	{
      	    dtemp_18 = (((float) t1[i])) - dtemp_4 *  nrm_out[i];
   	...
   	else 
	{
      	    ...
      	    prcurvs_out[1] = 0.0;
      	    for (i=0; i<=2; i+=1) 
         	maxprdir_out[i] = ((float) maxprdir_1[i]);
   	}
   	return;
}	/* end polyfit3d_lhf */
...
LOCAL void  eval_curvature_lhf_surf(
   	...
   	float H[2][2])
{
   	float grad_sqnorm, grad_norm, ell, ell2, ell3;
   	float c, s, kH2, tmp, dtemp_0;
   	float v[2], W1[2];
   	float W[2][2];
   	float U[3][2];
   	float d1[2];
...
LOCAL int eval_vander_bivar(
   	...
   	int strip)
{
   	static float V[28672];
   	int V_dim1, V_dim2;
   	static float ws1[1024];
   	static float ts[1024];
   	int ncols;
   	static float v[1024];
   	int degree_1_out, npnts, jj, rnk, ncols_sub;
   	float t2, t, vnrm2, s, w;
   	int ncoeff, itemp_0, itemp_1, i, j;
   	static int ncoeffs[6] = {3,6,10,15,21,28};
   	static float weights[10] = {1.,1.0,1.0,2.0,1.0,2.0,6.0,2.0,2.0,6.0};
   	float dtemp_0, dtemp_1, dtemp_2, dtemp_3, dtemp_4;
   	int i3;
      	...
      	for (i=0; i<=us_dim1 - 1; i+=1) 
	{
      	...
      	for (i=0; i<=us_dim1 - 1; i+=1) 
	{
             ws1[i] =  ws1[i] + 0.01 * (dtemp_1 / (((float)npnts)));
        }
        if (degree_1_out < 4) 
	{
            for (i=0; i<=npnts - 1; i+=1) 
   ...
   /*%%%%%%%%%%%%%%%%%%%%% */
   /* Force weights to be float precision. */
...
EXPORT  bool  WLSP_compute_normal3d0(
        POINT              *p,
        HYPER_SURF_ELEMENT *hse,
        HYPER_SURF         *hs)
{
        ...
        float nor_f[3],norm=0; ///normal of tri
        const float  *fnor;
        float t1[3],t2[3];
        INTERFACE *intfc = hs->interface;
...
EXPORT bool WLSP_compute_normal3d(
        POINT              *p,
        HYPER_SURF_ELEMENT *hse,
        HYPER_SURF         *hs)
{
        ...
        float tang[6],nrm1st[3],nrm[3],nrm_t[3],nrm_v[50][3];
        float xs_v[50][3],xss_v[50][2],nrm_vs[3];
        float PT[3][3],xss_r[50][2];
        float ws_row[50],w;

        int degree = 2;
        float temp,ncoeffs[]={3,6,10,15,21,28};
        float bs[50][2];
        float x_t[3],coeffs[20];
        float cs[10][2]={0.0},cond;
        float curvature;
        int xs_dim, nrm_coor_dim;
        float xs[60][3],prcurvs[2],maxprdir[3];
        int deg;
        float nrm_coor[60][3];       

	printf("#wlsp is called\n");
	...
	if(NO)
	{
	    ...
	    for(i=0; i<xs_dim; i++)
	    {
		print_general_vector("xs", xs[i], 3, "\n");
	    }
	    printf("nrm_coor %d\n", nrm_coor_dim);
	    for(i=0; i<nrm_coor_dim; i++)
	    {
		print_general_vector("nrm_coor", nrm_coor[i], 3, "\n");
	    }
	    print_general_vector("nrm= ", nrm, 3, "\n");
	    print_general_vector("prcurvs= ", prcurvs, 2, "\n");
	}
	...
	pc->curvature = -(prcurvs[0]+prcurvs[1])/2;
        //TMP curvature
	//pc->curvature = 20.0/PI*atan(PI/20.0*pc->curvature);

	//print_general_vector("_nor", nrm, 3, "\n");
	//printf("curva %15.8e\n", pc->curvature);
        ...
        float tol)
{
        float v[13];
        int v_dim1;
        static float dtemp_0[91];
        int dtemp_0_dim1;
        float dtemp_1[13];
        int dtemp_1_dim1;
        int rnk_out, m, n, itemp_0, itemp_1;
        float vnrm2, dtemp_2, dtemp_3, dtemp_4, dtemp_5;
