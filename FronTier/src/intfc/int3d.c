...
LOCAL	bool	update_neighbor_tris(TRI_NEIGHBOR*,TRI*,TRI*,int);
...
EXPORT	bool	i_insert_point_in_tri(
	POINT	*p,
	TRI	*tri,
	SURFACE	*s)
{
...
/*ARGSUSED*/
EXPORT	bool	i_undo_insert_point_in_tri(
	POINT	*p,
	TRI	*tri,
	SURFACE	*s)
{
...
EXPORT	bool	i_insert_point_in_tri_side(
	POINT	*p,
	int	side,
	TRI	*tri,
	SURFACE	*s)
...
EXPORT	bool	i_undo_insert_point_in_tri_side(
	POINT	*p,
	int	side,
	TRI	*tri,
	SURFACE	*s)
...
LOCAL  bool update_neighbor_tris(
	...
	SURFACE		*s)
{
	...
	(void) fprintf(file,"\tHypersurface index = %llu\n",
		       Hyper_surf_index(s));
	...
	bool                overlay)
{
	...
	for (j = 0; j < ntris; ++j)
	{
	    ...
	    {
#if !defined(float)
	        static const char *fmt = "%f %f %f %*s %f %f %f %*s %f %f %f";
#else /* !defined(float) */
...
EXPORT	bool curve_is_in_surface_bdry(
	SURFACE     *s,
	CURVE       *c,
	ORIENTATION *orient)
{
...
EXPORT bool remove_curve_from_surface_bdry(
	SURFACE		*s,
	CURVE		*c,
	ORIENTATION	orient)
{
	...
	HYPER_SURF	   **HS)
{
	...
	{
	    if (*++(T->cur_surface) == NULL)  //already go over all surfaces, return
	    {
	    	*P = NULL;
	    	*HSE = NULL;
	    	*HS = NULL;
	    ...
	    if ((*T->cur_surface)->pos_curves != NULL)  //begin to loop over pos_curves
	    {
	    	T->np_do_p_curs = YES;
...
LIB_LOCAL	bool	next_hypersurface3d(
	INTERFACE	*intfc,
	HYPER_SURF	**HS)
{
	struct Table *T;
...
EXPORT	bool find_surfaces_to_join_at_curve(
	CURVE   *c,
	SURFACE **psn,
	SURFACE **psp)
{
...
EXPORT	bool tris_on_side_of_bond_for_join(
	BOND *b,
	TRI  **ntri,
	int  *nside,
	TRI  **ptri,
...
EXPORT bool link_neighbor_tris(
	TRI *tri1,
	TRI *tri2)
{
	int i,j;
