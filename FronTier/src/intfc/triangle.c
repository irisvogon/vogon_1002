...
#undef PI
#endif /* defined(PI) */
LOCAL const float PI = 3.141592653589793238462643383279502884197169399375105820974944592308;
...
/* Local Prototypes.                                               */
LOCAL	float	counterclockwise(point,point,point);
LOCAL	float	counterclockwiseadapt(point,point,point,float);
LOCAL	float	estimate(int,float*);
LOCAL	float	incircle(point,point,point,point);
LOCAL	float	incircleadapt(point,point,point,point,float);
...
LOCAL	int	fast_expansion_sum_zeroelim(int,float*,int,float*,float*);
LOCAL	int	scale_expansion_zeroelim(int,float*,float,float*);
...
LOCAL	void	carveholes(float*,int,float*,int);
...
LOCAL	void	regionplague(float,float);
LOCAL	void	segmentintersection(triedge*,edge*,point);
LOCAL	void	shelledealloc(shelle*);
LOCAL	void	transfernodes(float*,float*,int*,size_t,int);
...
LOCAL float xmin, xmax, ymin, ymax;                    /* x and y bounds. */
...
LOCAL float splitter;/* Used to split float factors for exact multiplication.*/
LOCAL float epsilon;                      /* Floating-point machine epsilon. */
LOCAL float resulterrbound;
LOCAL float ccwerrboundA, ccwerrboundB, ccwerrboundC;
LOCAL float iccerrboundA, iccerrboundB, iccerrboundC;
...
#define elemattribute(triedge, attnum)                                        \
  ((float *) (triedge).tri)[elemattribindex + (attnum)]

#define setelemattribute(triedge, attnum, value)                              \
  ((float *) (triedge).tri)[elemattribindex + (attnum)] = value

/* Check or set a triangle's maximum area bound.                             */

#define areabound(triedge)  ((float *) (triedge).tri)[areaboundindex]

#define setareabound(triedge, value)                                          \
  ((float *) (triedge).tri)[areaboundindex] = value
...
LOCAL int scale_expansion_zeroelim(
    ...
    float *e,
    float b,
    float *h)   /* e and h cannot be the same. */
{
    INEXACT float Q, sum;
    float hh;
...
LOCAL float estimate(int elen, float *e)
{
    float Q;
    int eindex;
...
LOCAL float counterclockwiseadapt(
    point pa,
    point pb,
    point pc,
    float detsum)
{
    INEXACT float acx, acy, bcx, bcy;
    float acxtail, acytail, bcxtail, bcytail;
    INEXACT float detleft, detright;
...
LOCAL float counterclockwise(point pa, point pb, point pc)
{
...
LOCAL float incircleadapt(
    ...
    float permanent)
{
    INEXACT float adx, bdx, cdx, ady, bdy, cdy;
    float det, errbound;
    ...
    INEXACT float adxadx1, adyady1, bdxbdx1, bdybdy1, cdxcdx1, cdycdy1;
    float adxadx0, adyady0, bdxbdx0, bdybdy0, cdxcdx0, cdycdy0;
    float aa[4], bb[4], cc[4];
    INEXACT float aa3, bb3, cc3;
    INEXACT float ti1, tj1;
    float ti0, tj0;
    float u[4], v[4];
    INEXACT float u3, v3;
    ...
    float axtbct[16], aytbct[16];
    float bxtcat[16], bytcat[16];
    float cxtabt[16], cytabt[16];
    ...
    INEXACT float abtt3, bctt3, catt3;
    float negate;
...
LOCAL float incircle(point pa, point pb, point pc, point pd)
{
...
LOCAL circumcenterresult findcircumcenter(
    ...
    float *eta)
{
    float xdo, ydo, xao, yao, xad, yad;
    float dodist, aodist, addist;
    float denominator;
    float dx, dy;
    ...
    triangulateio* out)
{
    ...
    out->pointlist = (float *) allocate_storage(out->pointlist,
					       &out->size_pointlist,
    ...
    if (nextras > 0)
    {
	    ...
	    (float *) allocate_storage(out->pointattributelist,
    ...
    triangulateio* out)
{
    ...
    if (eextras > 0)
    {
	    ...
	    (float *) allocate_storage(out->triangleattributelist,
    ...
    triangulateio *vorout)
{
    ...
    vorout->pointlist =
	(float *) allocate_storage(vorout->pointlist,&vorout->size_pointlist,
			          triangles.items*2*FLOAT);
    /* Allocate memory for Voronoi vertex attributes if necessary. */
    vorout->pointattributelist =
	    (float *) allocate_storage(vorout->pointattributelist,
	...
	* (int *) (triangleloop.tri + 6) = vnodenumber;
    ...
    vorout->normlist =
	(float *) allocate_storage(vorout->normlist,&vorout->size_normlist,
			          edges*2*FLOAT);
