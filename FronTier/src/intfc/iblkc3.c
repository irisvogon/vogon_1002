...
LOCAL	int check_consistence_of_crx(BLK_CRX*,bool);
	...
	const BLK_CRX *blk_crx)
{
	if (ip[0] != ipn[0])
	{
		...
		printf("blk_crx->crx[0][ip[1]][ip[2]] = %d\n",
        ...
        BLK_TRI *blk_mem)
{
	...
	c = curve_crx_in_jdir(blk_crx,1)->c;
        ...
        {
            int i;
            printf("blk_case24_comp3, found curve %d\n", c);
            for (i = 0; i < blk_crx->blk_info->num_curves; ++i)
                printf("blk_crx curve[%d] = %d\n", i, blk_crx->blk_info->curves[i]);
        }
