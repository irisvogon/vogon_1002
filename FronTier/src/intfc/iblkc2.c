...
LOCAL	float area_plane_case01(BLK_CRX*);
LOCAL	float area_edge_case02(BLK_CRX*);
LOCAL	float area_corner_case03(BLK_CRX*);
LOCAL	float area_glider_case04(BLK_CRX*);
LOCAL	float area_hexagon_case05(BLK_CRX*);
LOCAL	float area_corner1_case06(BLK_CRX*);
LOCAL	float area_corner2_case06(BLK_CRX*);
LOCAL	float area_twister_case07(BLK_CRX*);
LOCAL	float area_corner_case08(BLK_CRX*);
LOCAL	float area_edge_case08(BLK_CRX*); 
LOCAL	float area_corner1_case09(BLK_CRX*);
LOCAL	float area_corner2_case09(BLK_CRX*);
LOCAL   float area_twister_case10(BLK_CRX*);
LOCAL   float area_edge1_case11(BLK_CRX*);	
LOCAL   float area_edge2_case11(BLK_CRX*);
LOCAL	float area_corner1_case12(BLK_CRX*);
LOCAL	float area_corner2_case12(BLK_CRX*);
LOCAL	float area_corner3_case12(BLK_CRX*);
LOCAL	float area_corner_case13(BLK_CRX*);
LOCAL   float area_glider_case13(BLK_CRX*);
LOCAL	float area_corner1_case14(BLK_CRX*);
LOCAL	float area_corner2_case14(BLK_CRX*);
LOCAL	float area_corner3_case14(BLK_CRX*);
LOCAL	float area_corner4_case14(BLK_CRX*);

/* Volume Calculation functions */
LOCAL	float volume_plane_case01(BLK_CRX*,float*,float*,float*,float*);
LOCAL	float volume_edge_case02(BLK_CRX*,float*,float*);
LOCAL	float volume_corner_case03(BLK_CRX*,float*);
LOCAL	float volume_glider_case04(BLK_CRX*,float*,float*,float*);
LOCAL	float volume_hexagon_case05(BLK_CRX*,float*,float*,float*,float*,
					float*); 
LOCAL	float volume_corner1_case06(BLK_CRX*,float*);
LOCAL	float volume_corner2_case06(BLK_CRX*,float*);
LOCAL	float volume_twister_case07(BLK_CRX*,float*,float*,float*,float*);
LOCAL	float volume_corner_case08(BLK_CRX*,float*);
LOCAL	float volume_edge_case08(BLK_CRX*,float*,float*);
//	      volume_corner1_case09() = volume_corner_case08()
LOCAL	float volume_corner2_case09(BLK_CRX*,float*);
LOCAL   float volume_twister_case10(BLK_CRX*,float*,float*,float*,float*);
LOCAL	float volume_edge1_case11(BLK_CRX*,float*,float*);
LOCAL	float volume_edge2_case11(BLK_CRX*,float*,float*);
...
LOCAL   float volume_glider_case13(BLK_CRX*,float*,float*,float*);
LOCAL	float volume_corner1_case14(BLK_CRX*,float*);
//            volume_corner2_case14() = volume_corner_case03()
//            volume_corner3_case14() = volume_corner_case08()
LOCAL	float volume_corner4_case14(BLK_CRX*,float*);

// Volume calculation functions for other interface perimeters p_i 
LOCAL   float volume_edge1_p2_case11(BLK_CRX*,float*,float*);
...
LOCAL   float volume_glider_p2_case13(BLK_CRX*,float*,float*,float*);
LOCAL	float volume_corner1_p2_case14(BLK_CRX*,float*);
...
LOCAL	float volume_hexagon_p4_case14(BLK_CRX*,float*,float*,float*,float*,
					float*); 
//	volume_corner1_p5_case14() = volume_corner4_case14()
//	volume_corner2_p5_case14() = volume_corner1_case06() 
LOCAL	float volume_hexagon_p5_case14(BLK_CRX*,float*,float*,float*,float*,
					float*); 
//	volume_corner1_p6_case14() = volume_corner2_case09()
//	volume_corner2_p6_case14() = volume_corner_case08() 
LOCAL	float volume_hexagon_p6_case14(BLK_CRX*,float*,float*,float*,float*,
					float*); 
	...
	BLK_TRI *blk_mem)
{
	...
	for (i = 1; i < num_crx; ++i)
	{
	    ...
	    {  
		printf("surface_numbe of crxs[0]->s = %d\n",
				surface_number(crxs[0]->s));    
		printf("surface_numbe of crxs[i]->s = %d\n",
	...
	BLK_TRI *bm)
{
	...
	for (is = 0; is < bm->num_surfaces; ++is)
	{
	    		...
	    		++i, tri1 = tri1->next)
	    {
	        if(the_tri(tri1))
		{
		    printf("The tri found\n");
		    add_to_debug("blk_tri_find");
		}
	...
	if(debugging("blk_tri_find"))
	{
	    printf("#prt bm \n");
	    tecplot_blk_intfc_plot("blk_fd", bm);
	    remove_from_debug("blk_tri_find");
	}
...
//point on a face of a block tol, it should be less then 0.004*hmin
//see adjust_crossings
//    adjust_for_min_spacing  var mgs.

LOCAL	float	face_tol = 10.0*MACH_EPS;
void	set_face_tol(float);
void	set_face_tol(float  tol)
{
	face_tol = tol;
}
	...
	int     is2)
{
			...
			tri1=tri1->next, ++it)
	{
	    			...
	    			tri2=tri2->next, ++jt)
	    {
	        ...
	        for (i = 0; i < 3; ++i)
	        {
		    //null side i of tri1 is on plane dir.
		    if (Tri_on_side(tri1,i) != NULL || 
		        fabs(Coords(p1[i])[dir] - Coords(p1[(i+1)%3])[dir]) > face_tol)
			continue;
	            for (j = 0; j < 3; ++j)
	            {
			//null side j of tri2 is on plane dir.
		    	if (Tri_on_side(tri2,j) != NULL || 
		            fabs(Coords(p2[j])[dir] - Coords(p2[(j+1)%3])[dir]) > face_tol)
			    continue;

			//side i of tri1 and side j of tri2 are on the same plane dir
	                if (fabs(Coords(p1[i])[dir] - Coords(p2[j])[dir]) < face_tol)
	                {
	                    if (search_step == 0)
	                    {
	                        if (p1[i] == p2[(j+1)%3])
        ...
        BLK_TRI *blk_mem)
{
	...
	{
	    float ****crn = blk_crx->corner_coords;
	    float cell_volume = blk_crx->cell_volume;
	    float *crx1,*crx2,*crx3,*crx4;
	    float c1[3], c5[3],c6[3],c7[3],c8[3];
	    int i;
        ...
        BLK_TRI *blk_mem)
{
	...
	else
	{
	    ...
	    int i;
	    float *crx1,*crx2,*crx3,*crx4;
	    float c1[3],c7[3],c8[3];
	    float norm = blk_crx->cell_volume;
        ...
        BLK_TRI *blk_mem)
{
	...
	else
	{
	    float ****crn = blk_crx->corner_coords;
	    float cell_volume = blk_crx->cell_volume;
	    float *crx1,*crx2,*crx3,*crx4,*crx5;
	    float c1[3],c6[3],c7[3],c8[3];
	    int i;
        ...
        BLK_TRI *blk_mem)
{
	...
	else
	{
	    float ****crn = blk_crx->corner_coords;
	    int i;
	    float *crx1,*crx2,*crx3,*crx4,*crx5,*crx6;
	    float c1[3],c4[3],c6[3],c7[3],c8[3];
	    float norm = blk_crx->cell_volume;
	    float *h = blk_crx->h;
        ...
        BLK_TRI *blk_mem)
{
	...
	else
	{
	    float ****crn = blk_crx->corner_coords;
	    int i;
	    float c4[3],c5[3],c8[3];
	    float norm = blk_crx->cell_volume;
	    float vol_corner,vol_edge;
        ...
        BLK_TRI *blk_mem)
{
	...
	else
	{
	    ...
	    float c4[3],c5[3];
	    float norm = blk_crx->cell_volume;
	    float area_1,area_2,vol_1,vol_2;
        ...
        BLK_TRI *blk_mem)
{
	...
	else
	{
	    float ****crn = blk_crx->corner_coords;
	    int i;
	    float c1[3],c2[3],c3[3],c4[3],c5[3],c6[3],c7[3],c8[3];
	    float norm = blk_crx->cell_volume;
	    float vol_11, vol_12, vol_21, vol_22, area_1, area_2;
        ...
        BLK_TRI *blk_mem) 
{
	...
	else
	{
	    float ****crn = blk_crx->corner_coords;
	    int i;
	    float c4[3],c6[3],c7[3],c8[3];
	    float norm = blk_crx->cell_volume;
	    float vol_1,vol_2,vol_3,vol_hex,vol_cor,area_1,area_2,area_3;
	    float *h = blk_crx->h;
        ...
        BLK_TRI *blk_mem)
{
	...
	else
	{
	    ...
	    float c1[3],c2[3],c3[3],c4[3],c5[3],c6[3],c7[3],c8[3];
	    float norm = blk_crx->cell_volume;
	    float vol_glider1, vol_corner1, vol_glider2, vol_corner2; 
	    float ar_glider, ar_corner;
...
LOCAL float area_corner_case03(
	BLK_CRX *blk_crx)
{
	int i;
	POINT *p1, *p2, *p3;
	...
	BLK_CRX *blk_crx)
{
	...
	float D1[3], D2[3], D3[3], D4[3], D5[3], D6[3];
	float area;
	...
	BLK_CRX *blk_crx)
{
	...
	float *crx1, *crx2, *crx3, *crx4;
	float A1[3], A2[3], A3[3], A4[3];
	float D1[3], D2[3], D3[3], D4[3], D5[3];
	float area;
...
LOCAL float area_glider_case04(
	BLK_CRX *blk_crx)
{
	int i;
	POINT *p1, *p2, *p3, *p4, *p5;
	float *crx1, *crx2, *crx3, *crx4, *crx5;
	float E1[3], E2[3], E3[3], E4[3], E5[3], E6[3], E7[3], E8[3], 
	      E9[3], E10[3], E11[3], E12[3], E13[3], E14[3], E15[3], 
	      E16[3], E17[3], E18[3], E19[3], E20[3];
	float A1[3], A2[3], A3[3], A4[3], A5[3], A6[3], A7[3], A8[3], 
...
LOCAL float area_glider_case13(
	BLK_CRX *blk_crx)
{
	int i;
	POINT *p1, *p2, *p3, *p4, *p5;
...
LOCAL float area_hexagon_case05(
	BLK_CRX *blk_crx)
{
	int i;
	POINT *p1, *p2, *p3, *p4, *p5, *p6;
...
LOCAL float area_corner1_case06(
	BLK_CRX *blk_crx)
{
	int i;
	POINT *p1, *p2, *p3;
...
LOCAL float area_corner2_case06(
	BLK_CRX *blk_crx)
{
	int i;
	POINT *p1, *p2, *p3;
...
LOCAL float area_twister_case07(
	BLK_CRX *blk_crx)
{
	int i;
	POINT *p1, *p2, *p3, *p4, *p5, *p6;
...
LOCAL float area_corner_case08(
	BLK_CRX *blk_crx)
{
	int i;
	POINT *p1, *p2, *p3;
...
LOCAL float area_corner1_case09(
	BLK_CRX *blk_crx)
{
	int i;
	POINT *p1, *p2, *p3;
...
LOCAL float area_corner2_case09(
	BLK_CRX *blk_crx)
{
	int i;
	POINT *p1, *p2, *p3;
...
LOCAL float area_twister_case10(
	BLK_CRX *blk_crx)
{
	int i;
	POINT *p1, *p2, *p3, *p4, *p5, *p6;
...
LOCAL float area_corner1_case12(
	BLK_CRX *blk_crx)
{
	int i;
	POINT *p1, *p2, *p3;
...
LOCAL float area_corner2_case12(
	BLK_CRX *blk_crx)
{
	int i;
	POINT *p1, *p2, *p3;
...
LOCAL float area_corner3_case12(
	BLK_CRX *blk_crx)
{
	int i;
	POINT *p1, *p2, *p3;
...
LOCAL float area_corner_case13(
	BLK_CRX *blk_crx)
{
	int i;
	POINT *p1, *p2, *p3;
...
LOCAL float area_corner1_case14(
	BLK_CRX *blk_crx)
{
	int i;
	POINT *p1, *p2, *p3;
...
LOCAL float area_corner2_case14(
	BLK_CRX *blk_crx)
{
	int i;
	POINT *p1, *p2, *p3;
...
LOCAL float area_corner3_case14(
	BLK_CRX *blk_crx)
{
	int i;
	POINT *p1, *p2, *p3;
...
LOCAL float area_corner4_case14(
	BLK_CRX *blk_crx)
{
	int i;
	POINT *p1, *p2, *p3;
...
LOCAL float volume_corner_case03(
	BLK_CRX *blk_crx,
	float crn[])
{
	int i;
...
LOCAL float volume_edge_case02(
	BLK_CRX *blk_crx,
	float crn1[],
	float crn2[])
{
	...
	float *crx1, *crx2, *crx3, *crx4;
	float A1[3], A2[3], A3[3], A4[3];
	float D1[3], D2[3], D3[3], D4[3], D5[3], D6[3], D7[3], D8[3],
	      D9[3], D10[3];
...
LOCAL float volume_edge1_case11(
	BLK_CRX *blk_crx,
	float crn1[],
	float crn2[])
{
...
LOCAL float volume_edge2_case11(
	BLK_CRX *blk_crx,
	float crn1[],
	float crn2[])
{
...
LOCAL float volume_edge1_p2_case11(
	BLK_CRX *blk_crx,
	float crn1[],
	float crn2[])
{
...
LOCAL float volume_plane_case01(
	BLK_CRX *blk_crx,
	float crn1[],
	float crn2[],
	float crn3[],
...
LOCAL float volume_glider_case04(
	BLK_CRX *blk_crx,
	float crn1[],
	float crn2[],
	float crn3[])
{
	...
	float E1[3], E2[3], E3[3], E4[3], E5[3], E6[3], E7[3], E8[3], 
	      E9[3], E10[3]; 
	float D1[3], D2[3], D3[3], D4[3], D5[3], D6[3];  
	float V1, V2, V3, V4, V5, V6, V7, V8, V9, V10, V11, V12, V13, V14, 
...
LOCAL float volume_glider_case13(
	BLK_CRX *blk_crx,
	float crn1[],
	float crn2[],
	float crn3[])
...
LOCAL float volume_glider_p2_case13(
	BLK_CRX *blk_crx,
	float crn1[],
	float crn2[],
	float crn3[])
...
LOCAL float volume_hexagon_case05(
	BLK_CRX *blk_crx,
	float crn1[],
	float crn2[],
	float crn3[],
	...
	float *h)
{
	...
	float V1,V2,V3,V4,V5,V6,V7,V8,V9,V10,V11,V12,V13,V14,V15,
	      ...
	      V42,V43,V44;
	float proj1[3], proj2[3];
	float vol_A, vol_B, vol_C, volume;
...
LOCAL float volume_hexagon_p4_case14(
	BLK_CRX *blk_crx,
	float crn1[],
	float crn2[],
	float crn3[],
...
LOCAL float volume_hexagon_p5_case14(
	BLK_CRX *blk_crx,
	float crn1[],
	float crn2[],
	float crn3[],
...
LOCAL float volume_hexagon_p6_case14(
	BLK_CRX *blk_crx,
	float crn1[],
	float crn2[],
	float crn3[],
...
LOCAL float volume_corner1_case06(
	BLK_CRX *blk_crx,
	float crn[])
{
	int i;
...
LOCAL float volume_corner2_case06(
	BLK_CRX *blk_crx,
	float crn[])
{
	int i;
...
LOCAL float volume_twister_case07(
	BLK_CRX *blk_crx,
	float crn1[],
	float crn2[],
	float crn3[],
...
LOCAL float volume_corner_case08(
	BLK_CRX *blk_crx,
	float crn[])
{
	int i;
...
LOCAL float volume_edge_case08(
	BLK_CRX *blk_crx,
	float crn1[],
	float crn2[])
{
...
LOCAL float volume_corner2_case09(
	BLK_CRX *blk_crx,
	float crn[])
{
	int i;
...
LOCAL float volume_twister_case10(
	BLK_CRX *blk_crx,
	float crn1[],
	float crn2[],
	float crn3[],
...
LOCAL float volume_corner1_case14(
	BLK_CRX *blk_crx,
	float crn[])
{
	int i;
...
LOCAL float volume_corner4_case14(
	BLK_CRX *blk_crx,
	float crn[])
{
	int i;
...
LOCAL float volume_corner1_p2_case14(
	BLK_CRX *blk_crx,
	float crn[])
{
	int i;
