...
LOCAL 	void move_bdry_node(NODE*,CURVE*,INTERFACE*,
		int (*vfunc)(POINTER,float*,float*),POINTER,float);
...
LOCAL 	void move_interface(INTERFACE*,int (*vfunc)(POINTER,float*,float*),
			POINTER,float);
LOCAL 	INTERFACE *make_line_interface(RECT_GRID*);
LOCAL 	INTERFACE *make_elliptic_interface(RECT_GRID*);
LOCAL 	bool bond_cross_bdry_curve_at_node(BOND*,NODE*,POINT*,
		CURVE**,BOND**);
LOCAL 	int translation_vel(POINTER,float*,float*);
LOCAL 	int radial_motion_vel(POINTER,float*,float*);
LOCAL 	int shear_motion_vel(POINTER,float*,float*);
LOCAL 	int sine_motion_vel(POINTER,float*,float*);
LOCAL 	float max_front_speed(RECT_GRID,int (*vfunc)(POINTER,float*,float*),
			POINTER);
#endif /* defined(TWOD) */
#if defined(THREED)
...
LOCAL	void test2d(void)
{
	...
	float L[MAXD],U[MAXD];
	float vel[MAXD];
	int (*vfunc)(POINTER,float*,float*);
	const float eps = 10.0*MACH_EPS;
	static int gmax[2] = {1, 1};
	char s[10];
	POINTER params;
	float print_interval,max_speed,dt,time,max_time;
	int num_print;
...
LOCAL bool bond_cross_bdry_curve_at_node(
	BOND *b,
	NODE *n,
	POINT *p,
	CURVE **c_crx,
