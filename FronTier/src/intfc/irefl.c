...
LOCAL	void	reflect_coords(float*,float*,float*,int);
	...
	float*		nor)	/* plane normal */
{
	INTERFACE *intfc = s->interface;
        ...
        for (tri = first_tri(s); !at_end_of_tri_list(tri,s); tri = tri->next)
        {
            ...
            bdry12 = (Tri_order(tri) & BIN_SIDE12) ? YES : NO;
            bdry20 = (Tri_order(tri) & BIN_SIDE20) ? YES : NO;
            set_12_bdry(Tri_order(tri),bdry20);
            set_20_bdry(Tri_order(tri),bdry12);
        }
