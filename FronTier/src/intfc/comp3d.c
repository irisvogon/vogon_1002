...
typedef struct {
	float   pt[3];		/* Location being projected on tri */
	float   dpt[3];		/* vector pt -> closest point; */
	float   a[3];		/* Parametric coordinates of projection point */
	float   d, d2;		/* distance and square distance pt to tri */
	float   nor_d2;		/* Square of distance pt to plane of tri */
...
LOCAL	COMPONENT	component_wrt_icoords3d(float*,int*,INTERFACE*);
LOCAL	bool	block_recorded(int**,int*,int*);
LOCAL	bool	comp_is_on_surface(SURFACE*,const COMPONENT*,int);
LOCAL	bool	new_tri_is_closer(TRI_PROJECTION*,TRI_PROJECTION*);
LOCAL	bool	old_tri_on_vertex(TRI_PROJECTION*,TRI_PROJECTION*);
LOCAL	bool	old_tri_on_edge(TRI_PROJECTION*,TRI_PROJECTION*);
LOCAL	bool	set_tri_and_surface_list_pointers(struct Table*,int,int,int);
LOCAL	int	**add_to_bls_list(int**,int*,int*,int*);
LOCAL	int	block_dimension(int*);
LOCAL	void	blocks_on_grid_based_tri(TRI*,int***,COMPONENT***,RECT_GRID*, INTERFACE *);
...
LOCAL	bool	nearest_local_on_front_grid_block(int*,int*,int, INTERFACE*);
...
LOCAL	float	crx_toll;
LOCAL	float	ctol;
LOCAL	const float	blk_tol = 1.0e-6;

LOCAL	bool	comp_is_on_surface(
	SURFACE         *surf,
	...
	INTERFACE	*intfc)
{
	...
	//T->rect_grid = topological_grid(intfc)
	...
	{
	    //if(pp_mynode() == 6)
	    //set_shift_for_tecplot(1.0, 0.0, 0.0);
	...
	INTERFACE	*intfc)
{
	...
	if(debugging("tst_comp3d"))
	{
	    printf("#proj_tri=%d  %d  d2=%24.16e comp=%d  comp0=%d\n", 
...
LOCAL	bool	nearest_local_on_front_grid_block(
	int		*icoords,
	int		*icrds,		//output
	int		width,
	INTERFACE	*intfc)
	...
	INTERFACE	*intfc)
{
	...
	{
	    float d1 = coords[idir] - Closest.pt[idir];
	    float d2 = Tri_normal(Closest.tri)[idir];
	    if (same_sign(d1,d2))
...
LIB_LOCAL bool nearest_interface_point3d(
	float		   *coords,
	COMPONENT	   comp,
	INTERFACE	   *intfc,
	USE_BOUNDARIES	   bdry,
	...
	HYPER_SURF	   **phs)
{
	...
	{
		if(!nearest_local_on_front_grid_block(icoords, icoords, 2, intfc))
		{
...
LIB_LOCAL bool nearest_similar_interface_point3d(
	float		   *coords,
	COMPONENT	   compp,
	COMPONENT	   compn,
	INTERFACE	   *intfc,
...
LIB_LOCAL bool long_nearest_interface_point3d(
	float		*coords,
	COMPONENT	comp,
	INTERFACE	*intfc,
	USE_BOUNDARIES	bdry,
...
LIB_LOCAL bool long_nearest_similar_interface_point3d(
	float		*coords,
	COMPONENT	compp,
	COMPONENT	compn,
	INTERFACE	*intfc,
	...
	TRI_PROJECTION	*tri_proj)
{
	...
	float *p0, *p1, *p2;	/* pi = Coords(Pi)  */
	float dp00, dp01, dp02;	/* dpij = ptj - pi[j] */
	float dp10, dp11, dp12;
	float dp20, dp21, dp22;
	const float *nor;       /* normal to tri       */
	float n0, n1, n2;	/* components of nor   */
	float np, x, x0, x1, x2;/* temporary variables */
	float L;		/* L = 2*area(tri)^2   */
	float ps;
	float x01,  x12,  x20;
	float v100, v101, v102, lv10s;
	float v210, v211, v212, lv21s;
	float v020, v021, v022, lv02s;
	float q00, q01, q02,	/* Displacement uni_arrays from p to     */
	      q10, q11, q12,    /* the closest point on each tri side */
	      q20, q21, q22;
	float d0_2, d1_2, d2_2; /* Square distance from pt to tri sides */
	float cos0, cos1, cos2;
	const float* const *s;
	const float *l;
	...
	TRI_PROJECTION *tp)
{
	...
	case ONVERTEX:
	    ...
	    {
		float dot,ndot,dots,dotns,ndots,ndotns;
		POINT *po, *npo;
		int i;
		float sv[3], nsv[3];
...
EXPORT bool make_tri_comp_lists(
	INTERFACE	*intfc)
{
...
LOCAL  bool is_point_inside_box(
	float          *coords,
	const float    *VL,
	const float    *VU)
{
...
EXPORT bool is_tri_outside(INTERFACE *intfc, TRI *t, RECT_GRID *grid)
{
const float *h = grid->h;
const float *VL = grid->VL, *VU = grid->VU;
int	    i, j;
float	   *p1 = Coords(Point_of_tri(t)[0]);
float	   *p2 = Coords(Point_of_tri(t)[1]);
float	   *p3 = Coords(Point_of_tri(t)[2]);
float	   p[3];
int	   ib[4];
static const float SHIFT = 1.0e-6; // TOLERANCE see rect_in_which
...
EXPORT  bool is_tri_outside_box(TRI *t, float **fbox)
{
	int    i, j;
...
EXPORT bool    is_outside_surface(
		INTERFACE  *intfc, 
		SURFACE    *s, 
		RECT_GRID  *gr)
{
...
EXPORT bool    is_outside_surfaces(INTERFACE *intfc, RECT_GRID *gr)
{
	SURFACE    **s;
...
EXPORT  void    delete_outside_surface(INTERFACE *intfc)
{
	...
	for(c=intfc->curves; c && *c; c++)
	{
		...
		//is an inside surface or the surface can not be deleted
	        if(!is_outside_surface(intfc, surf, top_grid) || surf->number == INSIDE_DOMAIN)
		    found = YES;
	    }
	    ...
	    for(btris=Btris((*c)->first); btris && *btris; btris++)
	    {
		(*btris)->surface->number = found==YES ? INSIDE_DOMAIN : OUTSIDE_DOMAIN;
	    }
	}
	...
	INTERFACE	*intfc)
{
	...
	for (s = intfc->surfaces; s && *s; ++s)
	{
	    for (t = first_tri(*s); !at_end_of_tri_list(t,*s); t = t->next)
	    {
	        float *crds0 = Coords(Point_of_tri(t)[0]);
	        float *crds1 = Coords(Point_of_tri(t)[1]);
	        float *crds2 = Coords(Point_of_tri(t)[2]);
	        ...
	        if(is_tri_outside(intfc, t, top_grid))
		{
	        ...
	        for (i = 0; i < dim; ++i)
	        {
	    	    float a = crds0[i], b = crds1[i], c = crds2[i];
	...
	if (max_size != out_cnt)
	{
	    ...
	    if (interface_reconstructed(intfc) == YES)
            {
                for (s = intfc->surfaces; s && *s; ++s)
                    for (t = first_tri(*s); !at_end_of_tri_list(t,*s); t = t->next)
                        blocks_on_grid_based_tri(t,T->num_of_tris,T->compon3d,top_grid,intfc);
            }
            else
	    {
	        for (s = intfc->surfaces; s && *s; ++s)
	    ...
	    //DEBUG_TMP printf("#number of %d  %d  %d\n", interface_reconstructed(intfc), 
	    		//DEBUG_TMP max_size,total_num_of_tri_blocks);
	...
	{
	    //DEBUG_TMP printf("#out_cnt=max_size= %d  default_comp = %d\n", max_size, intfc->default_comp);
...
LOCAL bool set_tri_and_surface_list_pointers(
	struct Table	*T,
	int		xmax,
	int		ymax,
	int		zmax)
...
void	sort_tris_on_blocks(INTERFACE	*intfc)
{
	...
	TRI		**tris;
	SURFACE		**surfs;
	int		ix, iy, iz, i, nt, xmax, ymax, zmax, comp;
	...
	for(iz = 0; iz < zmax; ++iz)
	{
		...
		for(ix = 0; ix < xmax; ++ix)
		{
		    ...
		    comp = T->compon3d[iz][iy][ix];
		    if(nt == 0 && comp == ONFRONT)
		    {
			printf("ixy on front %d %d %d\n", ix, iy, iz);
		    }
		    if(nt != 0 && comp != ONFRONT)
		    {
			printf("on front no tri %d %d  %d %d %d\n", nt, comp, ix, iy, iz);
		    }
	...
	float	    tol)
{
	...
	float	v21[3], v13[3], v32[3];
	float	l1,l2,l3,l21,l13,l32;
	float	D1, D2, D3;
	int 	i;
	...
	float *h)
{
	...
	float	    crds_end[MAXD];
	int	    i,j;
	float	    *p1,*p2,d,pe_crx[MAXD],*pt;
	float	    crds_max[MAXD],crds_min[MAXD];
	...
	*ie = ERROR;
	...
	for (i = 0; i < 3; ++i)
	{
	    ...
	    if (d < crx_tolv && crds_start[ic] <= pt[ic] && crds_end[ic] > pt[ic])
	    {
	        crds_crx[ic] = Coords(p[i])[ic];
		*iv = i;

		if(debugging("tri_edge_crossing"))
		{
		    float   tol = 1.0e-5;
	            if(fabs(crds_crx[0] - 0.07167) < tol && 
	               fabs(crds_crx[1] + 0.00482) < tol &&
	               fabs(crds_crx[2] + 0.03825) < tol)
	               {
			   printf("#dis  %d  %12.8e %12.8e   %d\n", 
			       Point_of_tri(tri)[i],
			       d, crx_tolv, 
			       within_interval(crds_start[ic],crds_end[ic],
			       Coords(Point_of_tri(tri)[i])[ic]));

			   print_general_vector("crx_start =", crds_start, 3, "\n");
			   print_general_vector("crx_end =", crds_end, 3, "\n");
	...
	if ( crds_start[ic] <= crds_crx[ic] && crds_end[ic] > crds_crx[ic]  &&
	     within_tri(crds_crx,Coords(p[0]),Coords(p[1]),Coords(p[2]),norm,0.0) )
	{
	    return YES;
	}
...
float	*p0 = Coords(pt[0]), *p1 = Coords(pt[1]), *p2=Coords(pt[2]);
int	i, iv0;
...
float	*p0 = Coords(pt[0]), *p1 = Coords(pt[1]), *p2=Coords(pt[2]);
const float *n;
float    norm[MAXD], v[MAXD], D, t1, t2;
int	i,iv,ie;
...
EXPORT bool line_point_projection(
	float *crx,
	int   *iv,
	float *pt,
	float *crds1,
...
int	 i;
	...
	*iv = ERROR;
	...
	if(t1*t2 > 0.0)
	    if(fabs(t1) < fabs(t2))
	    {
	        ft_assign(crx, crds1, 3*sizeof(float));
		*iv = 0;
		return YES;
	    }
	    else
	    {
	        ft_assign(crx, crds2, 3*sizeof(float));
		*iv = 1;
		return YES;
	    }
	...
	float		tol)
{
	...
	int		*gmax = (&topological_grid(intfc))->gmax;
	float		dist, dist0, crds2[3], crx[3], crx0[3];
	const float     *tnor;
	struct Table	*T = intfc->table;
	...
	INTERFACE	*intfc)
{
	...
	int		*gmax = gr->gmax;
	struct Table	*T = intfc->table;
	float		*h = gr->h, crds[3], v[3], tol, hmin, dist;
...
LOCAL float shortest_2line_dist_dir(
        int   dir,
        float *crds_start,
        float *p1,
        float *p2,
