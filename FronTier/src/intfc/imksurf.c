...
LOCAL	bool 	install_bdry_objects(INTERFACE*,NODE****,CURVE****,
...
LOCAL   bool    face_crx_in_dir(float (*func1)(POINTER,float*),POINTER,
                        float (*func2)(POINTER,float*),POINTER,
			float*,float*,float*,int);
LOCAL   int     count_bdry_coner_crx(int*,COMPONENT***);
LOCAL   int     install_bdry_corner_crx(INTERFACE*,EG_CRX*,RECT_GRID,float*,float*,
			NODE****,CURVE****,SURFACE***);
...
LOCAL   float hyperboloid_func(POINTER,float*);
LOCAL   float paraboloid_func(POINTER,float*);
LOCAL   float i_random_pert_func(POINTER,float*);
LOCAL   float multi_ellipsoid_func(POINTER,float*);
...
LOCAL	bool install_bdry_objects(
	INTERFACE *intfc,
	NODE ****corners,
	CURVE ****edges,
	SURFACE ***faces,
...
EXPORT 	float dumbbell_func(
        POINTER func_params,         
	float *coords)
{         
	DUMBBELL_PARAMS *d_params = (DUMBBELL_PARAMS*)func_params;
        float x0,x1,y,z,f0,f1,R,rr;
        float arg;
	...
	bool         include_curve_crx)
{
	...
	tnc = make_curves_from_blk(new_c, n_c, gmax,blk_mem,eg_crx->curves,eg_crx->num_curves);
	
	//tnc >= eg_crx->num_curves, if they are not equal, at least one curve is cut into two pieces.
...
LOCAL	float i_random_pert_func(
	POINTER func_params,
	float *coords)
{
	FOURIER_POLY *fpoly = (FOURIER_POLY*)func_params;
	float z = fourier_poly(coords,fpoly);

	return coords[2] - z;
}	/* end random_pert_func */

LOCAL	float multi_ellipsoid_func(
	POINTER func_params,
	float *coords)
{
	M_ELLIP_PARAMS *params;
	int num_ellip,i,imin;
	static float **cen,**rad;
	static float *d;
	float arg,min_d;
	...
	RECT_GRID *gr)
{
	...
	SURFACE **surf;
	CURVE   *curv;
	float cen1[MAXD], rad1[MAXD];
	float cen2[MAXD], rad2[MAXD];
...
EXPORT	bool grid_line_crx_in_dir(
	float (*func)(POINTER,float*),
	POINTER func_params,
	int dim,
	float *crds1,
	...
	int dir)
{
	float f1,f2,fm,a1,a2, gtol = 10*MACH_EPS;
	int i,num_iter = 50;
...
LOCAL   bool    grid_crx_from_comp(int (*func)(POINTER,float*),
                        POINTER,int,float*,float*,float*,int);
LOCAL	int     install_grid_crx_from_comp(
	int       (*func)(POINTER,float*), POINTER, EG_CRX*, RECT_GRID,int);
LOCAL   int     install_grid_curve_crx_from_comp(
	int       (*func)(POINTER,float*), POINTER, EG_CRX*, RECT_GRID,int*);
LOCAL 	void    assign_comp(int (*func)(POINTER,float*),POINTER,COMPONENT***,RECT_GRID);
...
EXPORT bool make_surfaces_from_comp(
	RECT_GRID   *rgr,
	int         (*func)(POINTER, float*),
	POINTER     func_params,
	SURFACE     **s,
	...
	SURFACE	    **ps)
{
	...
	float *vertex;
	int *index;
	int i,j,k,l,i1,i2,i3,num_vtx,num_pts;
	...
	INTERFACE *sav_intfc;
	float L[MAXD],U[MAXD];
	float dist,max_side,min_side,ave_side;
	...
	for (i = 0; i < num_pts; ++i)
	{
	    ...
	    for (j = 0; j < 3; ++j)
	    {
	    	if (vertex[3*i+j] < L[j]) L[j] = vertex[3*i+j];
	    	if (vertex[3*i+j] > U[j]) U[j] = vertex[3*i+j];
	    }
	}
	num_tris = num_vtx/3;
	...
	for (i = 0; i < num_tris; ++i)
	{
		...
		for (l = 0; l < 3; ++l)
		{
		    if ((Point_of_tri(tri1)[k] == Point_of_tri(tri2)[(l+1)%3] &&
		         Point_of_tri(tri1)[(k+1)%3] == Point_of_tri(tri2)[l]))
		    {
		    	Tri_on_side(tri1,k) = tri2;
	...
	if (debugging("sdl_input"))
	{
	    ...
	    (void) printf("num_pts = %d  num_tris = %d\n",num_pts,num_tris);
	...
	SURFACE	    **ps)
{
	...
	{
	    int ns;
	    fscanf(vtk_file,"%d %d %d %d",&ns,&i1,&i2,&i3);
	    tris[i] = make_tri(points[i1],points[i2],points[i3],
	    			NULL,NULL,NULL,NO);
	    tris[i]->surf = surf;
	    for (j = 0; j < 3; ++j)
