...
#if defined(TWOD)
LOCAL	bool	check_curve2d(CURVE*,INTERFACE*);
LOCAL	bool 	i_consistent_interface2d(INTERFACE*);
#endif /* defined(TWOD) */
#if defined(THREED)
LOCAL	bool 	i_consistent_interface3d(INTERFACE*);
LOCAL	bool	check_consistency_of_tris_on_surface(SURFACE*);
LOCAL	bool	check_tri(TRI*,INTERFACE*);
LOCAL	bool	tri_side_consistent(TRI*,TRI_NEIGHBOR*,int,INTERFACE*);
LOCAL	bool	allow_null_sides = NO;
LOCAL	bool	check_curve3d(CURVE*,INTERFACE*);
...
EXPORT	bool i_consistent_interface(
	INTERFACE	*intfc)
{
	switch (intfc->dim)
	{
