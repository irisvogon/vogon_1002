...
LOCAL	float	distance_between_points_on_curve(POINT*,BOND*,POINT*,BOND*,
						 CURVE*,ORIENTATION);
	...
	ORIENTATION	*c12_orient)
{
	...
	BOND		*bb;
	float		cp, cp1;
	float		l1f;	/* distance on cross->c1 to following cross */
	float		l1b;	/* distance on cross->c1 to preceeding cross */
	float		l2f;	/* forward distance on cross->c2 to *cross1 */
	float		l2b;	/* backward distance on cross->c2 to *cross1 */
...
LOCAL	float distance_between_points_on_curve(
	POINT		*ps,
	BOND		*bs,
	POINT		*pe,
	BOND		*be,
