...
*	NumOfCurvePoints(curve);		
*	NumOfSurfPoints(surface);
*	NumOfIntfcPoints(intfc);		
*
*	NumOfNodes(intfc);		
*
*	NumOfCurveBonds(curve);		
*	NumOfIntfcBonds(intfc);
*
*	NumOfCurves(intfc);
*	NumOfSurfaces(intfc);
*
*	NumOfSurfTris(surface);		
*	NumOfIntfcTris(intfc);		
*
 ...
 ***********************************************************************/ 
EXPORT 	int NumOfCurvePoints(CURVE *c)
{
	return c->num_points;
}

/***********************************************************************
 *	NumOfSurfPoints(SURFACE *s)
 ***********************************************************************/
EXPORT	int NumOfSurfPoints(SURFACE *surf)
{
	POINT *p;
	TRI *tri;
	int i,n;
 ...
 ***********************************************************************/
EXPORT  int NumOfIntfcPoints(INTERFACE *intfc)
{
 ...
 ***********************************************************************/
EXPORT int NumOfCurveBonds(CURVE *c)
{
	BOND *b;
	int num_bonds = 0;
 ...
 ***********************************************************************/
EXPORT int NumOfIntfcBonds(INTERFACE *intfc)
{
	CURVE **c;
	int num_bonds = 0;
	for (c = intfc->curves; c && *c; ++c)
            num_bonds += NumOfCurveBonds(*c);
        return num_bonds;
}
 ...
 ***********************************************************************/
EXPORT  int NumOfCurves(INTERFACE *intfc)
{
        CURVE **c;
        int num_curves;
 ...
 ***********************************************************************/
EXPORT  int NumOfSurfaces(INTERFACE *intfc)
{
        SURFACE **s;
        int num_surfs = 0;
 ...
 **********************************************************************/
EXPORT	int NumOfSurfTris(SURFACE *s)
{
	return s->num_tri;
}
 ...
 **********************************************************************/
EXPORT	int NumOfIntfcTris(INTERFACE *intfc)
{
	SURFACE **s;
        int num_tris = 0;
        for (s = intfc->surfaces; s && *s; ++s)
            num_tris += NumOfSurfTris(*s);
        return num_tris;
}
 ...
 **********************************************************************/
EXPORT	void ArrayOfIntfcPoints(INTERFACE *intfc, float *coords)
{
	int i, n, dim = intfc->dim;
	POINT	*p;
	HYPER_SURF_ELEMENT *phse;
 ...
 * 	ArrayOfCurves()
 * purpose:
 * 	return the curves from the intfc; curves are stored as an 1D array.
 * input: intfc, curves;
 * output: curves;
 *	The size of curves is NumOfCurves(intfc);
 **************************************************************************/
EXPORT	void ArrayOfCurves(
	INTERFACE *intfc,
	CURVE **curves)
{
	CURVE **c;
...
EXPORT	bool IntfcGetPointChain(
