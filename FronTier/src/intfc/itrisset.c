...
LOCAL	bool	apply_pt_constraint(POINT*,POINT*,POINT*,TRI*);
LOCAL  	bool    null_side_tris_loop(NULL_LOOP*,TRI*,int,ORIENTATION);
LOCAL	int 	compare_tri_sort(const void*,const void*);

//  tolerance for plane_side_intersection
LOCAL  float  ps_tol = 1.0e-12;
EXPORT	void set_tol_for_tri_sect(float    tol)
{
	ps_tol = tol;
}
...
EXPORT bool link_neighbor_null_side_tris(
	TRI *tri1,
	TRI *tri2)
{
	int i,j;
...
EXPORT  bool two_tris_share_side(
        TRI *tri1,
	TRI *tri2,
	int side)
{
...
EXPORT	bool two_tris_share_pts(
        TRI *tri1,
	TRI *tri2,
	int side)
{
...
EXPORT	bool tris_with_pt_inside(
        TRI *tri1,
	TRI *tri2,
	int side)
{
...
EXPORT	bool plane_side_intersection(
	const float *plane,
	TRI         *tri,
	int         side,
	float       *pi,
...
EXPORT	bool	tris_sect(
	TRI	*t1,
	TRI	*t2)
{
	static float	**fbox1 = NULL, **fbox2 = NULL;
...
EXPORT	bool	test_tris_intersection(
	TRI	*t1,
	TRI	*t2)
{
	int		i, j, side1, side2, iv;
	static float	ang_tol = radians(0.1);
	const  float	*n1, *n2;
	float		n[3], plane[4];
	float		ps1[3], pe1[3], ps2[3], pe2[3], pi[3];
	float		de1, ds2, de2;
	POINT		*p;
	...
	const void *b)
{
	...
	float	  *cen1 = c1->cen, *cen2 = c2->cen;
	float	  min_len, tol = 1.0e-8;
	int	  i;
...
EXPORT  bool tri_in_grid_block(
	TRI		*tri,
	int		*box_min,
	int		*box_max,
	RECT_GRID	*gr)
{
	float		*L = gr->L, *h = gr->h, *p;
	float		cmin[3], cmax[3], bmin[3], bmax[3], pmin[3], pmax[3];
	float		tol;
	int		i, j;
	bool		bflag;
...
EXPORT	bool	tangled_tris_bound_box(
	int		*bmin,
	int		*bmax,
	int		*kmin,
	int		*kmax,
...
EXPORT	bool sep_common_point_from_loop(
	TRI		**tris,
	int		num_tris,
	TRI		**nbtris,
	int		*num_nbtris,
...
bool	sep_common_edge_from_loop(TRI**,TRI**,int*,POINT**,int,INTERFACE*);

//assume the point on the ending of null sides are not boundary points
//tris form a loop and it is called after sep_common_point_from_loop.
EXPORT	bool sep_common_edge_from_loop(
	TRI		**new_tris,
	TRI		**tris,
	int		*sides,
	POINT		**pts,
...
{
	float	*p0, *p1, *p2, *p3;
	float	n1, n2, dist, ndist, v1[3], v2[3];
	float	ang_tol	= -0.2;
	float	tol = 1.0e-4;
...
LOCAL  bool  check_adjecant_constrain(
	POINT	*p0,
	POINT	*p1,
	POINT	*p2,
	POINT	*p3, 
...
LOCAL	bool	make_tri_pairs_with_constraint(
	TRI_PAIR	*tri_pair,
	TRI		**new_tris,
	int		*n_new)
{
	...
	INTERFACE	*intfc)
{
	...
	float		tol;
	float		pt[][3]={{3.90084e-5, 0.0180396, -0.00628228},
...
LOCAL	bool null_side_tris_loop(
	NULL_LOOP	*null_loop,
	TRI		*start,
	int		start_side,
	ORIENTATION	orient)
...
EXPORT	bool	seal_degenerated_null_loop(
	TRI		**seal_tris,
	int		*nstris,
	NULL_LOOP	*null_loop)
{
...
LOCAL	float	tri_area_tol;

	bool	set_tri_area_tol(float);
	bool	set_tri_area_tol(float	tol)
{
	tri_area_tol = tol;
}

LOCAL	bool	apply_pt_constraint(
	POINT	*p0,
	POINT	*p1,
	POINT	*p2,
	TRI	*tri)
...
bool	compute_smooth_para(SMOOTH_PARA*,POINT*,TRI*,SURFACE*,SMOOTH_TOL*);
	...
	char		*fname)
{
	...
	float			s, smin, cosang, s1[3][3], s2[3][3];
