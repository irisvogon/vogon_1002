...
//  #bjet2
#define    ADD_MAX_LEN   200
LOCAL int  add_top = 0;
LOCAL float add_time[ADD_MAX_LEN], add_time_st[ADD_MAX_LEN];


EXPORT void add_time_start(int ind)
{
	add_time_st[ind] = MPI_Wtime();
}

EXPORT void add_time_end(int ind)
{
	add_time[ind] += MPI_Wtime() - add_time_st[ind];
}
...
EXPORT float add_time_get(int ind)
{
	return  add_time[ind];
}
