...
//        printf_rect_comp(ip->root);
	...
	if(YES)
	{
	    ...
	    null_sides_are_consistent();
	    check_print_intfc("After init state", "init_state", 's', 
	        front->interf, 0, 0, NO);
            
            sprintf(s, "depo_%s", right_flush(0, TSTEP_FIELD_WIDTH));
	...
	for (i = 0; i < 3; ++i)
	{
	    for (j = 0; j < 2; ++j)
	    {
		if(rect_boundary_type(front->interf,i,j) == OPEN_BOUNDARY && i == 0 && j == 1)
		    rect_boundary_type(front->interf,i,j) = REFLECTION_BOUNDARY;
	    }
	}
        }
	...
	if (ip->cauchy_deposition)
	    (*ip->cauchy_deposition)(init,ip,wave);
	...
	RESTART_DATA	*restart)
{
	...
	bool		got_intfc_from_file = got_intfc_from_file(init);
	int		n_restart_vars = ip->prt->n_restart_vars;
	int		var;
	int             *iperm,idir,dim;
...
EXPORT	bool	position_file_at_read_time(
	FILE	*file,
	int	step,
	float	*time,
	float	*dt_last)
...
#if !defined(float)
	    if (line[0] == '#')
	    	(void) sscanf(line,"%*s%*s%*s%*s%*s%f%*s%*s%d%*s%*s%f",
			      time,&current_step,dt_last);
	    else
	    	(void) sscanf(line,"%*s%*s%*s%*s%f%*s%*s%d%*s%*s%f",
			      time,&current_step,dt_last);
#else /* !defined(float) */
 	    if (line[0] == '#')
...
*			         in the file have a real (ie float) size
...
LOCAL int to_parallel_restart(
	RECT_GRID	*rst_grid)
{
	float		*L = rst_grid->L, *U = rst_grid->U, *h = rst_grid->h;
	float		*VL = rst_grid->VL, *VU = rst_grid->VU;
	float		tol;
	int		i;
...
*/
	...
	//    	         printf("coords = [%f %f %f] comp = %d\n", coords[0], coords[1], coords[2], comp);
	    ...
	    if(debugging("initpert"))
	    {
		init_period_pertb(front, wave);
		break;
	    }
	...
	INIT_PHYSICS	*ip)
{
	...
	{
	    float *VU = comp_glbgr->VU, *VL = comp_glbgr->VL;
	    float *GU = comp_glbgr->GU, *GL = comp_glbgr->GL;
	    int tgmax[MAXD];

	    copy_rect_grid(comp_glbgr,&pp_grid->Zoom_grid);
	    for (i = 0; i < dim; ++i)
	    {
	    	float h = top_glbgr->h[i];
