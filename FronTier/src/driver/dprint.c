...
LOCAL	float	comm_time_step(float,int*,float*,int,TIME_STEP_SET_BY*);
...
LOCAL   bool    file_exists(const char * filename); /* Determine if a file exists */
...
// needed for VTK
LOCAL   bool    subsampling_is_valid(Wave*,Front*,int);
...
/*ARGSUSED*/
void increment_next_print_time(OUTPUT_DATA *mod)
{
        int sten_step = Output_time_stencil_step(mod)=
                                        (Output_time_stencil_step(mod)+1)%
                                        Output_time_stencil_size(mod);
        if(sten_step>0)
            Output_next_print_time(mod) +=
               Output_time_freq_small(mod);
        else if(sten_step==0)
	{
            Output_next_print_time(mod) +=
               Output_time_freq(mod);
	    //clean_up(0);
	}
        else
            clean_up(-1);
	...
	int		error)
{
	...
	{
            // TODO: RK This is where output time is detected
	   ...
	   if (print_time == YES)
	   {
	           ...
	           if (is_ts_for_output_format(i,grid,prt))
		   {
		       if (real_time_output(Output_mode(mod[i])))
                       {
                           increment_next_print_time(mod[i]);
	...
	char *var_name)
{
	...
	GD_POINT	 *points;
	float		 *x,*y;
	float		 xmin,xmax,ymin,ymax,height;
	char		 title[100];
	...
	bool		print_set_by)
{
	...
	*/
	...
	newdt = comm_time_step(newdt,&n_proc,coords,dim,&TimeStepSetBy);
        ...
        bool         print_set_by)
{
        ...
        for(i = 0; i < num_patches; i++)
        {
                 ...
                 rts[i]->wave->patch_level == 0)
            {
                ...
                {
                    dt = tmpdt;
                    ftft_assign(coords,crds,sizeof(float)*MAXD);
	...
	VTK_plot_data   *vtk_data)
{
       	char	dirname[1024];
	int 	step = front->step;
	int	node_id = pp_mynode();
        VTK_PRINT_OPTIONS *opts = &VTK_print_opts(vtk_data);
	int     sub_factor = opts->subsample_factor;
	bool    print_in_binary = opts->_print_in_binary;
        static  bool verified = false;

	sprintf(dirname,"%s",vtk_base(opts));
	sprintf(dirname,"%s-vtk.ts%s",dirname,right_flush(step,5));
	if (pp_numnodes() > 1)
	{
	    sprintf(dirname,"%s-nd%s",dirname,right_flush(node_id,4));
	}
...

{
	...
	float           sum[2][nvars];
	Locstate	state;
	FILE		*file, *bank, *intfc_bank;
	char		buff[10000];
	float		l[2],u[2];
	float		*coordss;
	int		icoords[3],a,b;
	char		name[100000],nametwo[10000],basename[10000];
	char            param_name[100], intfc_name[100];
	...
	sprintf(buff,"%s.%s.scalar.ts%s",vtk_base(opts),name,
			right_flush(front->step,5));
	if (pp_numnodes() > 1)
	{
	    sprintf(buff,"%s-nd%s",buff,right_flush(pp_mynode(),4));
	}

	sprintf(param_name, "%s.%s.visit", vtk_base(opts), name);
	sprintf(intfc_name, "%s.intfc.visit", vtk_base(opts));

        if(file_exists(param_name))
        {
             bank = fopen(param_name, "a");
        ...
        else
        {
	     ...
	     {
                  bank = fopen(param_name, "w");
                  intfc_bank = fopen(intfc_name, "w");
	          fprintf(bank, "!NBLOCKS %d\n", pp_numnodes());
		  fprintf(intfc_bank, "!NBLOCKS %d\n", pp_numnodes());
	     }
             else if(pp_numnodes() == 1)
             {
                  bank = fopen(param_name, "w");
                  intfc_bank = fopen(intfc_name, "w");
             }
        ...
        if(print_in_binary)
	{
	    ...
	    file = fopen(buff, "wb");
            ...
            sprintf(str, "DIMENSIONS %d %d 1\n", width + 1, height + 1);
            length = count_digits(width + 1);
            length2 = count_digits(height + 1);
            fwrite(str, sizeof(char), 15 + length + length2, file);
            
            sprintf(str, "%g",gr->h[0]);
            length = count_digits_in_string(str);
            sprintf(str, "%g", gr->h[1]);
            length2 = count_digits_in_string(str);
            sprintf(str, "SPACING %g %g 0\n", gr->h[0], gr->h[1]);
            ...
            length2 = count_digits_in_string(str);
            sprintf(str, "ORIGIN %g %g 0\n",coords[0],coords[1]);
            fwrite(str, sizeof(char), 11 + length + length2, file);

            length = count_digits(width*height);
            sprintf(str, "CELL_DATA %d\n", width*height);
            fwrite(str, sizeof(char), 11  + length, file);

	    length = count_digits_in_string(name);
            sprintf(str, "SCALARS %s float 1\n", name);
            fwrite(str, sizeof(char), 17 + length, file);
	    ...
	    (void) fprintf(file, "DIMENSIONS %d %d 1\n", width + 1, height + 1);
	    (void) fprintf(file, "SPACING %g %g 0\n", gr->h[0], gr->h[1]);
	    (void) fprintf(file, "ORIGIN %g %g 0\n", coords[0], coords[1]);
   	...
   	for (j = 0; j < height; ++j)
	{
		...
		#define not_float
		#endif
		float tmp= (float)(*VTK_frame_plot_function(vtk_data->frame_data[var]))(
						         coords,front,wave,
	      		                                 comp,state);
                #if defined(not_float)
		#define float double
		#undef not_float
		#endif
        ...
        if(pp_numnodes() > 1 && pp_mynode() == 1)
        {
            ...
            {
                sprintf(buff, "%s.%s.scalar.ts%s", opts->base_name, name,
                                          right_flush(front->step,5));
                sprintf(buff, "%s-nd%s.vtk\n", buff,right_flush(i,4));
                fprintf(bank, "%s", buff);

                sprintf(buff, "%s-vtk.ts%s", opts->base_name, right_flush(front->step, 5));
                sprintf(buff, "%s-nd%s/3d-intfc.vtk\n", buff, right_flush(i, 4));
                fprintf(intfc_bank, "%s", buff);
            }
        }
        else if(pp_numnodes() == 1)
        {
            fprintf(bank, "%s.%s.scalar.ts%s.vtk\n", opts->base_name, name, right_flush(front->step,5));
            fprintf(intfc_bank, "%s-vtk.ts%s/2d-intfc.vtk\n", opts->base_name, right_flush(front->step,5));
        }
...

{
        ...
        if(print_in_binary)
        {
            ...
            length = count_digits(width/factor + 1);
            length2 = count_digits(height/factor + 1);
            fwrite(str, sizeof(char), 15 + length + length2, file);

            sprintf(str, "%g", factor*gr->h[0]);
	    length = count_digits_in_string(str);
	    sprintf(str, "%g", factor*gr->h[1]);
	    length2 = count_digits_in_string(str);
	    sprintf(str, "SPACING %g %g 0\n", factor*gr->h[0], factor*gr->h[1]);
            fwrite(str, sizeof(char), 12 + length + length2, file);
            ...
            length = count_digits(width/factor*height/factor);
        ...
        if(pp_numnodes() > 1 && pp_mynode() == 1)
        {
            for(i = 0; i < pp_numnodes(); i++)
            {
                ...
                sprintf(buff, "%s-vtk.ts%s", opts->base_name, right_flush(front->step, 5));
                sprintf(buff, "%s-nd%s/3d-intfc.vtk\n", buff, right_flush(i, 4));                                                                                          fprintf(intfc_bank, "%s", buff);                                          
            } 
        }
        else if(pp_numnodes() == 1)
        {
...

{
	...
	float           sum[3][nvars];
	Locstate	state;
	FILE		*file, *bank, *intfc_bank, *test;
	char		buff[10000];
	float		l[3],u[3];
	float		*coordss;
	int		icoords[3],a,b;
	static const char         *separators = " \t";
	char		*c,name[100000],tempname[10000], param_name[100], intfc_name[100];
	char		selected_plots[10000];

	width = gr->gmax[0];
	length =  gr->gmax[1];
	height =  gr->gmax[2];
	fopts = VTK_frame_options(opts);
	for (var = 0; var < nvars; ++var)
	{
	icoords[0]=0;
	icoords[1]=0;
        ...
        if(print_in_binary)
	{
	    /* Print the vtk header for a binary file */
            ...
            length1 = count_digits(width + 1);
            length2 = count_digits(length + 1);
	    length3 = count_digits(height + 1);
	    ...
	    fwrite(str, sizeof(char),10 + length1 + length2 + length3 , file);
            ...
            length1 = count_digits(width*length*height);
            sprintf(str, "CELL_DATA %d\n", width*length*height);
...

{
	...
	int             factor = opts->subsample_factor;
	    ...
	    sprintf(str, "%g", factor*gr->h[1]);
            length2 = count_digits_in_string(str);
            sprintf(str, "%g", factor*gr->h[2]);
            length3 = count_digits_in_string(str);
            sprintf(str,"SPACING %g %g %g\n", factor*gr->h[0], factor*gr->h[1], factor*gr->h[2]);
            fwrite(str, sizeof(char), 11 + length1 + length2 + length3, file);
...
bool subsampling_is_valid(Wave *wave, Front *front, int factor)
{
        RECT_GRID *gr = wave->rect_grid;
