...
LOCAL	bool	d_last_time_step_modification(void);
...
LOCAL	void	set_modified_time_step(Grid*,Front*,Wave*,Printplot*,float);
	...
	Printplot	*prt)
{
	...
	int		cnt = 0;
#if defined(TIMING)
	float		start_cpu_time, end_cpu_time;
	float		start_wall_time, end_wall_time;
...
/*
	...
	{
	    init_pert_from_file();
	    read_pert_from_file(initial_step, 0);
	    //read_pert_from_file(initial_step, 1);
	}
*/
	 if(fr->turb_boundary_layer == YES)(*root->set_up_merge_cell)(wave);
	...
	for ( ; ; )
	{
	    ...
	    if (fr->step == 1)
		add_to_debug("my_refelect_state");
	    ...
	    int *iperm,i,dim=fr->rect_grid->dim;
...
/*
	    //if(debugging("const_bdry"))
	    {
		if(read_pert_cond(grid->step))
		{
		    read_pert_from_file(grid->step, 0);
		    //read_pert_from_file(grid->step, 1);
		}
		set_pertn(grid->step);
	    }
*/
...
//	     (void) printf("#TMP_TK Step %d uses %d bytes memory\n\n",
//	    	grid->step,get_vmalloc_storage_use());
...
void    turbulent_inflow(Wave *wave, Front *front);
	...
	float		time)
{
	...
	Grid		*grid;
	Wave		*wave;
	int		(*hyp)(float,float*,Wave*,Front*);
	...
	// Pressure averaging at wall 3/20/12 RK
	{
	...
	*/
	...
	while(n_bub > 0)
	{
	    ...
	    if(make_bubbles)
	    {
	        n_bub = (*make_bubbles)(wave, front);
		//make large bubble once.
	...
	float		dt_frac)
{
	...
	{
	    float dt_max = dt_frac;
	    float dt_min = dt_frac;
	...
	if (!debugging("no_cfl"))
	{
	    ...
	    float coords[MAXD];
	    float max_dt, min_dt;
	    static const float MIN_DT_FACTOR = 1.0e-5;	/*TOLERANCE*/
	    static const float MAX_DT_FACTOR = 10.0;	/*TOLERANCE*/
