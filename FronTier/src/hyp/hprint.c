...
#if defined(TWOD)
LOCAL	void	nearest_interior_state(float*,COMPONENT,Front*,Wave*,
				       INTERFACE*,float*,Locstate);
#endif /* defined(TWOD) */
...
EXPORT	void	print_tri_soln(
	...
	TRI_SOLN	*soln,
	size_t		n_vars,
	float		(**fcn)(float*,Front*,POINTER,COMPONENT,Locstate))
{
	...
	bool		binary_output = is_binary_output();
	char		field[15];
	...
	if (num_alloc < 2 + n_vars)
	{
		...
		num_alloc = 2+n_vars;
		uni_array(&node_binary,2+n_vars,              sizeof(float));
	...
	if (binary_output == YES)
	{
		...
		(void) fwrite((const void *) node_binary,sizeof(float),
			      2+n_vars,file);
	    }
	}
	else
	...
	Wave	*wave)
{
	...
	if (is_binary_output())
	{
	    ...
	    (void) fwrite((const void *) Maxsp(wave),sizeof(float),dim,file);
	}
	else
	{
	...
	Stencil		*sten)
{
	...
	for (i = -sten->npts/2; i < imax; ++i)
	    printf("  %d", sten->comp[i]);
	
	printf("\n");
	printf("newcomp = \n");
	for (i = -sten->npts/2; i < imax; ++i)
	    printf("  %d", Rect_comp(sten->icoords[i], sten->newwave));
