...
LOCAL INTERFACE *init_interface1d(RECT_GRID*,INTERFACE*,LEVEL_FUNC_PACK*);
LOCAL INTERFACE *init_interface2d(RECT_GRID*,INTERFACE*,LEVEL_FUNC_PACK*);
...
LOCAL INTERFACE *init_interface3d(RECT_GRID*,INTERFACE*,LEVEL_FUNC_PACK*);
	...
	Front     *front)
{
	...
	if (StateSize(init) != 0)
	{
	    screen("\n\t\tflow specified state enforcement at fronts\n\n");
            ...
            front->turb_boundary_layer = NO;
            screen("Type 'y' to have the turbulent boundary layer (dflt = %s): ",
                        (front->turb_boundary_layer==YES)?"yes":"no");
            (void) Gets(s);
            if (s[0] == 'y' || s[0] == 'Y')
                front->turb_boundary_layer = YES;
            else if (s[0] == 'n' || s[0] == 'N')
                front->turb_boundary_layer = NO;
	...
	LEVEL_FUNC_PACK *level_func_pack)
{
	...
	{
	case 1:
	    front->interf = init_interface1d(gr,front->interf,level_func_pack);
	    break;
	case 2:
	    front->interf = init_interface2d(gr,front->interf,level_func_pack);
	    break;
	case 3:
	    front->interf = init_interface3d(gr,front->interf,level_func_pack);
	    break;
	default: 
...
/*ARGSUSED*/
LOCAL   INTERFACE *init_interface1d(
        RECT_GRID  *gr,
        INTERFACE  *intfc,
        LEVEL_FUNC_PACK *level_func_pack)
{
        float **points = level_func_pack->point_array;
        POINT *p;
	    ...
	    level_func_pack->point_array != NULL)
	{
        ...
        return intfc;
}       /* end init_interface1d */
LOCAL	INTERFACE *init_interface2d(
	RECT_GRID  *gr,
        INTERFACE  *intfc,
	LEVEL_FUNC_PACK *level_func_pack)
{
	...
	{
	    prompt_make_level_curves(intfc,gr);
	}
	...
	{
	    COMPONENT left_c,right_c;
	    float **point_array = level_func_pack->point_array;
...
LOCAL	INTERFACE *init_interface3d(
	RECT_GRID  *gr,
	INTERFACE  *intfc,
	LEVEL_FUNC_PACK *level_func_pack)
{
	...
	set_topological_grid(intfc,gr);
	return intfc;
}	/* end init_interface3d */
