...
LOCAL	bool add_to_correspond_list = NO;
...
EXPORT bool set_correspondence_between_interfaces(
	INTERFACE	*oldintfc,
	INTERFACE	*newintfc)
{
...
EXPORT	bool rst_cor_after_delete_hyper_surf(
	HYPER_SURF	*hs)
{
	CORRESPOND	*crspd;
	HYPER_SURF	**newhs, **oldhs;
...
EXPORT	bool rst_cor_after_make_hyper_surf(
	HYPER_SURF	*hs)
{
	CORRESPOND	*crspd;
	INTERFACE	*intfc = hs->interface;
	...
	INTERFACE	*intfc)
{
	...
	float		min_dist;
	float		t_on[MAXD], coords_on[MAXD], *coords;
	int		i;
...
EXPORT	bool find_correspond_of_oriented_curve(
	O_CURVE		*c,
	O_CURVE		*corr_c,
	NODE		*corr_node,
	Front		*fr,
...
EXPORT	bool rst_cor_after_split_curve(
	CURVE		*curve,
	CURVE		**curves)
{
...
EXPORT	bool rst_cor_after_invert_curve(
	CURVE		*curve)
{
...
EXPORT	bool rst_cor_after_attach_curve_to_node(
	CURVE		*c1,
	CURVE		*c2)
{
