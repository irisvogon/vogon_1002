...
LOCAL   void    pp_clip_rect_grids(Front*,int[3][2]);
LOCAL 	void  	bundle_array_buffer(int,int*,int*,int*,float*,byte*);
LOCAL 	void  	unbundle_array_buffer(int,int*,int*,int*,float*,byte*);
...
LOCAL   void    reflect_array_buffer(int,int,int,int*,int*,int*,float*);
	...
	int   rbt[3][2])
{
	...
	for (i = 0; i < dim; ++i)
	{
	    if (zoom_gr->gmax[i] <= zoom_gr->lbuf[i] + zoom_gr->ubuf[i])
	    {
	...
	int *uexpand)
{
	...
	static 		float *x_average,*y_average,*z_average;
	int         	ix, iy, iz;
	int         	xmax, ymax, zmax;
	char		debug_name[100];
	...
	case 3:
		...
		find_Cartesian_coordinates(pp_mynode(),pp_grid,sub_icoords);
                ...
                if (l > 0)
                {
                    ...
                    if (debugging("adapt_subdomain"))
                    {
                        (void) printf("Comparing x-average with lower side:\n");                        (void) printf("x_ave[%d] = %10.6f x_ave[%d] = %10.6f\n",                                k,l-1,x_average[l],x_average[l-1]);
                        (void) printf("lambda = %f\n",lambda);
                        (void) printf("lexpand[0] = %d\n",lexpand[0]);
                    }
                }
	    ...
	    for (i = 0; i < G[1]; ++i)
            ...
            if (G[1] > 1)
            {
                ...
                if (l > 0)
                {
                    ...
                    if (debugging("adapt_subdomain"))
                    {
                        (void) printf("Comparing y-average with lower side:\n");                        (void) printf("y_ave[%d] = %10.6f y_ave[%d] = %10.6f\n",                                k,l-1,y_average[l],y_average[l-1]);
                        (void) printf("lambda = %f\n",lambda);
                        (void) printf("lexpand[1] = %d\n",lexpand[1]);
                    }
                }
	...
	POINTER ijk_to_I)
{
	...
	for (i = 0; i < dim; ++i)
	{
		...
		size = (gmax[(i+1)%dim] + lbuf[(i+1)%dim] + ubuf[(i+1)%dim]);
	...
	int nb)
{
	...
	{
	case 2:
	    return pack_index_in_dir2d(fr,(int**)IJK_to_I,lbuf,ubuf,bfs,
	    				dir,nb);
	case 3:
	    return pack_index_in_dir3d(fr,(int***)IJK_to_I,lbuf,ubuf,bfs,
	    				dir,nb);
	}
	...
	int nb)
{
	...
	{
	case 2:
	    return unpack_index_in_dir2d(fr,(int**)IJK_to_I,lbuf,ubuf,bfr,
	    				dir,nb);
	case 3:
	    return unpack_index_in_dir3d(fr,(int***)IJK_to_I,lbuf,ubuf,bfr,
	    				dir,nb);
	}
	...
	int nb)
{
	...
	{
	case 2:
	    return reflect_index_in_dir2d(fr,(int**)IJK_to_I,lbuf,ubuf,
	    				dir,nb);
	case 3:
	    return reflect_index_in_dir3d(fr,(int***)IJK_to_I,lbuf,ubuf,
	    				dir,nb);
	}
...
LOCAL 	void pack_index_in_dir3d(
	...
	int nb)
{
	printf("TODO: pack_index_in_dir3d() not written\n");
	clean_up(ERROR);
...
LOCAL 	void unpack_index_in_dir3d(
	...
	int nb)
{
	printf("TODO: unpack_index_in_dir3d() not written\n");
	clean_up(ERROR);
...
LOCAL 	void reflect_index_in_dir3d(
	...
	int nb)
{
	printf("TODO: reflect_index_in_dir3d() not written\n");
	clean_up(ERROR);
