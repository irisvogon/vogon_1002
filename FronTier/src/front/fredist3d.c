...
LOCAL	bool	check_and_rm_tetrahedron(TRI*,int,POINTER_Q**,SURFACE*);
LOCAL	void	tri_queue_test(const char*, POINTER_Q*);
LOCAL	void	change_buffer_for_intfc(INTERFACE*);
LOCAL	bool	small_area_tri_on_intfc(INTERFACE*);
	float	sqr_scaled_side_length(TRI*,int,RECT_GRID*);
...
EXPORT	bool	compute_smooth_para(
	...
	SMOOTH_TOL	*stol)
{
	...
	int	n, j, k;
	POINT	*p1, *p2, *p3;
	float	hv1[3], hv2[3], ang, tri_area;
	float	dist, lenk, max_cos, avep[3];
...
EXPORT	bool	compute_average_point(
	SMOOTH_PARA	*smooth_para,
	POINT		*p,
	TRI		*tri,
	SURFACE		*s,
...
EXPORT  void	smooth_curve(
	CURVE	*c,
	float	curve_dist_tol)
{
	POINT	*p, *prevp, *nextp, *pt[MAX_CURVE_PTS];
	BOND	*b;
	int	i, k, num;
	float	nor[4], dist, smo_fac = sqrt(0.9);
	float	newpt[MAX_CURVE_PTS][3];
...
EXPORT	float	min_null_pair_angle(
	float	*p0,
	float	*p1,
	float	*p2,
	float	*p3)
{
	float	hv1[3], hv2[3], ang;
	...
	Front		*fr)
{
	...
	
	{
		...
		if(fr->step == 1645 && i == -1)
		{
		    ...
		    add_to_debug("prt_que");
	            add_to_debug("pt_surface");
		    if(pp_mynode() == 7)
...
LOCAL	bool  small_area_tri_on_intfc(
	INTERFACE	*intfc)
{
	const float	*nor;
	SURFACE		**s;
	...
	float		tol = min3(gr->h[0],gr->h[1],gr->h[2]);
	...
	bool		restart_init)
{
	...
	//if(fr->step > 0)
	{
	    ...
	    //add_to_debug("ref_scat");
	    //reflect_extend_intfc(fr);
	...
	if(fr->step == 0)
	    null_sides_are_consistent();
	if((fr->step%100 == 0 && fr->step != 0) || fr->step == 2)
	    check_print_intfc("After surface detect", "contour", 's', 
	               fr->interf, fr->step, fr->step, NO);
	
	if(NO && (fr->step >= 170 && fr->step <= 179))
	    check_print_intfc("After surface detect", "contour", 's', 
	               fr->interf, fr->step, fr->step, NO);
	...
	//if ((do_redist && fr->step > 0) || pp_max_status(fr->redis_flag))
	if ((do_redist && fr->step > 0) || fr->step == 1)
	{
	    int		num_iter=3;
	...
	//if ((do_redist && fr->step > 0) && debugging("mgtris"))
	if(recon_repeat() >= 1)
	//if(NO)
	{
	...
	bool		*force_redist)
{
	SURFACE		**s;
	CURVE		**c;
	bool		status, min_status, delete_flag;
...
LOCAL   float	min_sqr_norm;   //min sqr area of a tri

float   Min_sqr_norm(RECT_GRID *gr)
{
	float	*h = gr->h;
	float	hmin = min3(h[0], h[1], h[2]);
...
float	reset_min_sqr_norm(float  new_min)
{
	...
	RECT_GRID *gr)
{
	...
	int	    i, j;
	float	    cen[3];
	float	    coef[3]={1.414, 0.618, 1.323};
	...
	POINTER_Q 	*p_q)
{
	...
	float	pt[3] = { 0.9733961893900565,     -0.301512040795133,     -15.64756169180028  };
	float	pt1[3] = { -1.026603810609944,     -0.301512040795133,     -15.64756169180028 };
	...
	POINTER_Q 	*p_q)
{
	...
	TRI 		*tri;
	int		k, i;
	float		tst_pt[3] = {  0.0,     0.8985,                 1.47066};
	float		tst_pt1[3] = { 1.0,     0.8985,                 1.47066};
	float		tol = 1.0/40.0;
...
LOCAL	bool	use_bd_dist = NO;
void	set_use_bd_dist(bool);
void	set_use_bd_dist(bool fg)
{
	use_bd_dist = fg;
}
	...
	Front		*fr)
{
	...
	for (tri=first_tri(s); !at_end_of_tri_list(tri,s); tri=tri->next)
	{
	    ...
	    if(tri_in_nozzle(tri) && tri_inflow_side(tri))
		continue;
...
LOCAL	bool	is_critical_tri(TRI *, int);
...
LOCAL bool check_and_rm_tetrahedron(
	TRI		*tri,
	int		side,
	POINTER_Q	**pq,
	SURFACE		*s)
...
bool	compare_pointers(
	POINTER_Q	*pq1,
	POINTER_Q	*pq2)
{
	float	ave_norm, norm1, norm2;
	float	tol = 1.0e-8;
...
bool	compare_bd_dist(
	POINTER_Q	*pq1,
	POINTER_Q	*pq2)
{
	float	ave_dist, dist1, dist2;
	float	tol = 1.0e-8;
...
EXPORT	bool	point_outside_open_bdry(
	int		*k,
	float		*nor,
	POINT		*p,
	INTERFACE	*intfc)
