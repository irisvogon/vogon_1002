...
LOCAL	int	degen_cr_tr(double**,int,double*,double,float*,
			    POINT*,POINT*,POINT*);
LOCAL	int	robust_H_extend_cross_trace(RECT_GRID*,POINT*,POINT*,BOND*,
					BOND*,POINT*,float*,int,float*,POINT*);
...
LOCAL	void	find_partial_time_cross(BOND*,BOND*,BOND*,BOND*,
					ORIENTATION,ORIENTATION,
					NODE*,NODE*,POINT*,float*,POINT*,
					NODE**,Front*);
LOCAL	void	find_partial_time_cross_by_extension(O_CURVE*,O_CURVE*,BOND*,
						     NODE*,NODE*,POINT*,float*,
						     POINT*,NODE**,Front*);
LOCAL	void	leaving_robust_cross_trace(int,float,POINT*);

LOCAL const float SC_TOL = 1.5; /*TOLERANCE*/
...
LOCAL const float EXTEND_FAC = 100.0; /*TOLERANCE*/
	...
	RECT_GRID	*gr)
{
	...
	float		coords_on[MAXD], *coords, t;
	BOND		*b;
	float		max_min_sep, min_sep;
	float		*h = gr->h;
	int		dim = gr->dim;
	...
	POINT		*pans)
{
	...
	*t_frac = (tmin > RCT_MIN) ? (float) min(max(tmin,0.0),1.0) :
				     (float) min(max(tmax,0.0),1.0);
	for (i = 0; i < dim; i++)
	{
	...
	POINT		*pans)
{
	...
	if (max_cr_pr < epsilon)
	{
		...
		else if (max(fabs(a[1]),fabs(a[3])) < epsilon)
		{
			...
			{
				*t_frac = (float) min(max(t[0],0.0),1.0);
		...
		else if (fabs(a[3]) < epsilon)
		{
			...
			*t_frac = (tmin < 0.0) ? 0.0 : (float) tmin;
			Coords(pans)[0] =  Coords(op)[0] +
				(*t_frac) * (Coords(np)[0] - Coords(op)[0]);
			Coords(pans)[1] =  Coords(op)[1] +
				(*t_frac) * (Coords(np)[1] - Coords(op)[1]);
...
{
	POINT		NPC;
	float		r,xcen,ycen,numxc,numyc,den,xbar,ybar,rsqr;
...
EXPORT int robust_circle_cross_trace(
	...
	float		radius, /*Radius at end of time step*/
	float		*t,     /*Fractional time of intersection*/
	POINT		*pans)  /*Intersection point*/
{
	...
	tmin = min(t1,t2);
	*t = (float) min(max(tmin,0.),1.);
	Coords(pans)[0] = (float) x1 + (*t) * (float) (nx1 - x1);
	Coords(pans)[1] = (float) y1 + (*t) * (float) (ny1 - y1);
	debug_print("r_cir_cross_trace",
