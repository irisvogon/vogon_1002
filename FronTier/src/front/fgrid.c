...
LOCAL	void count_crossings(INTERFACE*,float,float,float,float,int,int,int,
			int,float,float,float,float,CURVE*,int*,RECT_GRID*);
	...
	INTERFACE	*intfc)
{
	...
	if(prev_interface(intfc) != NULL)
	{
		...
		show_grid_components(smin, smax, 0, prev_interface(grid_intfc));
		clean_up(ERROR);
	    }
	}

	//DEBUG_TMP printf("#grid_intfc = %d\n", grid_intfc);
	...
	INTERFACE	*intfc)
{
	...
	if (debugging("xg_inconsist"))
	{
	    ...
	    xgraph_2d_intfc_within_range(open_name,intfc,center,5*gr->h[0]);
	}
...
LOCAL	int	grid_crossings_on_segment(INTERFACE*,float,float,float,float,
					  int,int,int,int,float,float,float,
					  float,CURVE*,int*,int*,RECT_GRID*,
					  int*,POINT**);
        ...
        INTERFACE       *grid_intfc)
{
        ...
        for (n_crx = 0, p = grid_intfc->points; p && *p; ++p)
        {
            ...
            {
                float x = Coords(*p)[0];
                float L = rgr->L[0], U = rgr->U[0], h = rgr->h[0];
                if (((L-h) <= x) && (x < L))
                {
                    ic[0] = 0;
                }
	...
	INTERFACE *intfc)
{
	...
	{
	case 2:
	    return show_grid_components2d(smin,smax,idir,intfc);
	case 3:
	    return show_grid_components3d(smin,smax,idir,intfc);
	}
