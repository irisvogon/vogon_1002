...
struct _BOX_INFO
{
	...
	bool *is_entirely_inside;	/* flag for entirely inside curve */
...
LOCAL	bool 	connected_boxes(RECT_BOX*,RECT_BOX*);
LOCAL 	bool 	box_untangle(Front*,RECT_BOX*,CROSS**);
LOCAL	bool 	set_box_comp_and_crx(INTERFACE*,RECT_BOX*,BOX_INFO*);
LOCAL	bool 	reconstruct_box_bonds(INTERFACE*,RECT_BOX*,BOX_INFO*);
LOCAL	bool 	reconnect_box_bonds(INTERFACE*,RECT_BOX*,BOX_INFO*);
LOCAL	bool 	make_cell_bonds(CELL_INFO*,BOX_INFO*);
LOCAL	bool 	check_and_reorganize_curves(INTERFACE*,BOX_INFO*);
...
LOCAL	bool 	bond_in_box(BOND*,INTERFACE*,RECT_BOX*);
LOCAL	bool 	bond_enclose_point(BOND*,POINT*);
LOCAL	bool 	out_box_bond(BOND*,RECT_BOX*);
LOCAL	bool 	out_box_point(POINT*,RECT_BOX*);
LOCAL	bool 	point_on_box_bdry(POINT*,RECT_BOX*);
LOCAL	bool 	in_clockwise_order(BOND*,CURVE*,BOND*,CURVE*,RECT_BOX*);
LOCAL 	bool 	new_closed_loop(BOND*,int*,NODE*);
LOCAL   bool    seg_cross_bond(int,BOND*,float,float,float,POINT*);
	...
	((separation((cr1)->p,(cr2)->p,(dim)) <				\
	 ...
	 (((cr1)->b1 == (cr2)->b1) || ((cr1)->b1 == (cr2)->b2) ||	\
	  ((cr1)->b2 == (cr2)->b1) || ((cr1)->b2 == (cr2)->b2)))
		...
		{
	    	    char xg_name[100];
		    float *center = Coords(cr->p);
		    float radius = 3.0*h[0];
	...
	for (box = boxes; box; box = box->next)
	{
		...
		(void) printf("Untangling box:\n");
	    	...
	    	(void) printf("box->bmax = %d %d\n",box->bmax[0],
				  		box->bmax[1]);
		(void) printf("Before untangle box %d:\n",box);
		for (cr = *crx; cr; cr = cr->next)
	    	    (void) printf("cr = %d\n",cr);
	    }
		...
		if (debugging("lgb2d"))
		{
		    (void) printf("After untangle box %d:\n",box);
		    for (cr = *crx; cr; cr = cr->next)
	    	        (void) printf("cr = %d\n",cr);
		}
	    }
	...
	for (box = Box.next; box != NULL; box = box->next)
	{
		...
		if (connected_boxes(box,nbox))
		{
		    if (debugging("lgb2d"))
		    {
			    ...
			    (void) printf("box->cross[%d] = %d\n",
					  j,box->cross[j]);
		    	for (j = 0; j < nbox->num_cross; ++j)
			    (void) printf("nbox->cross[%d] = %d\n",
					  j,nbox->cross[j]);
		    }
		    for (i = 0; i < 2; ++i)
		    {
	...
	CROSS **cross)
{
	...
	if (status == FUNCTION_SUCCEEDED)
	{
	    ...
	    for (cr = *cross; cr; cr = cr->next)
	    {
	    	bool is_b1_on_c1 = is_b_on_curve(cr->c1,cr->b1);
	    	bool is_b2_on_c2 = is_b_on_curve(cr->c2,cr->b2);
		/*
	...
	BOX_INFO *box_info)
{
	...
	else if (cell->num_cell_crx == 4)
	{
		...
		crx1 = cell->crxs[0];
		crx2 = cell->crxs[1];
		float d = distance_between_positions(Coords(crx1->pt),
				Coords(crx2->pt),2);
		if (d < d_shortest)
		{
		    i_rot = i;
...
LOCAL	bool set_box_comp_and_crx(
	INTERFACE *intfc,
	RECT_BOX *box,
	BOX_INFO *box_info)
{
...
LOCAL	bool reconnect_box_bonds(
	INTERFACE *intfc,
	RECT_BOX *box,
	BOX_INFO *box_info)
{
...
LOCAL	bool reconstruct_box_bonds(
	INTERFACE *intfc,
	RECT_BOX *box,
	BOX_INFO *box_info)
{
...
LOCAL	bool check_and_reorganize_curves(
	INTERFACE *intfc,
	BOX_INFO *box_info)
{
...
LOCAL bool new_closed_loop(
	BOND *b,
	int *num_points,
	NODE *n)
{
	BOND *b1;
	bool node_in_loop = NO;
	*num_points = 1;
	...
	RECT_BOX *box)
{
	...
	XB[0] = box->grid->L[0] + (float)box->bmin[0]*box->grid->h[0];
	XB[1] = box->grid->L[0] + (float)box->bmax[0]*box->grid->h[0];
	YB[0] = box->grid->L[1] + (float)box->bmin[1]*box->grid->h[1];
	YB[1] = box->grid->L[1] + (float)box->bmax[1]*box->grid->h[1];
...
LOCAL	bool out_box_bond(
	BOND *b,
	RECT_BOX *box)
{
	int i;
	bool cross_bdry = NO;
	float t,crx_crds,XB[2],YB[2];
...
LOCAL	bool	seg_cross_bond(
	...
	POINT	*p)
{
	float	x1=Coords(b->start)[0];
	float	y1=Coords(b->start)[1];
	float	x2=Coords(b->end)[0];
	float	y2=Coords(b->end)[1];
