...
LOCAL  bool	 form_patch_subintfc_via_cut(Front*);
LOCAL  bool 	 set_amr_subdomain_boundary(Front*,COMPONENT);  
LOCAL  bool      amr_check_for_cut_nodes(const char*,INTERFACE*);
...
LOCAL  bool form_patch_subintfc_via_cut(
        Front   *front)
{
        if (front == NULL or front->interf == NULL)
...
LOCAL bool amr_check_for_cut_nodes(
        const char      *msg, 
        INTERFACE       *intfc)
{
        NODE            **n, **n2;
        ...
        int        all_level)
{
        ...
        for(i = 1; i < total_patch; i++)
        {
            ...
            set_amr_intfc_tol(tmpfrs[i]->interf,
	    		pow(2.0,(float)tmpfrs[i]->patch_level));
