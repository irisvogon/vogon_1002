...
LOCAL	bool set_wall_tangent_space(
	Front		   *fr,
	Tparams		   *tp,
	float		   *t1,
	TRI	           **tris,
...
int	i;
	...
	for(i=0; i<3; i++)
	    printf(" %d  ", Point_of_tri(tn->tri)[i]);
	printf("\n");
...
LOCAL  bool find_triad_at_point(
	float			*t1,
	float			*t2,
	float			*nor,
	POINT			*p,
...
{
BOND    	*bp;
float		p0[3],p1[3],p2[3],pc[3], d0,d1;
int		i;
float		tol = 0.01;
...
EXPORT	bool set_up_wall_tangent_params(
	Tparams			*tp,
	POINT			*p,
	BOND			*b,
	CURVE			*c,
	...
	Front			*fr)
{
	...
	BOND_TRI  **btris;
	float	  t1[3], t2[3], lent;
	static float   nor[3];    //nor will be assing to tp->nor
	bool      folded;
	int	  i, nt1, nt2, nt;
...
EXPORT	bool fill_tan_stencil_along_wall(
	Tan_stencil	*sten,
	Front           *fr,
	const Tparams   *tp,
