...
IMPORT  INTERFACE *make_emb_grid_intfc(INTERFACE*);
	...
	/* fgb3d.c*/
IMPORT	bool repair_intfc_at_crossings3d(Front*);
IMPORT	bool rebuild_intfc_at_crossings3d(Front*);
IMPORT	bool rebuild_intfc_at_crossings3d2(Front*);
IMPORT  bool check_extension_of_surface_global(SURFACE*);
IMPORT  bool check_normal_on_intfc(INTERFACE*);
IMPORT	bool check_degenerated_loop(TRI**,int*,POINT**,int);

	//fgb3dutil.c
IMPORT	void	adjust_crossings(int*,int*,INTERFACE*);
IMPORT	bool	next_ip_in_dir(const int*,int,int*,int*,int*);
IMPORT	bool	check_and_repair_crx(INTERFACE*,int*,int*);
...
IMPORT	bool	remove_unphysical_crossings3d(INTERFACE*,int*,int*);
IMPORT  SURFACE* find_surf_with_comp(INTERFACE*, int, int);
IMPORT	bool	curves_on_bdry_side(int,int,INTERFACE*);

IMPORT	bool 	adjacent_cell(int*,int*);
IMPORT	int 	record_unphysical_ips(int*,int*,INTERFACE*,int**);
IMPORT  bool	reconstruct_intfc3d_in_box_lgb(INTERFACE*,int*,int*,bool,VOLUME_FRAC*);
IMPORT  bool	reconstruct_intfc3d_in_box(INTERFACE*,int*,int*,bool,VOLUME_FRAC*);
...
IMPORT	void	linear_interp_coefs_three_pts(float*,float*,float*,float*,float*);
IMPORT	bool	track_comp_through_crxings3d(int*,int*,int*,INTERFACE*,CRX_TYPE);
...
IMPORT	bool	fill_comp_from_prev_intfc(INTERFACE *, int *, int *);

	//fgb3comp.c
IMPORT  bool  is_wall_side(TRI*,int);
IMPORT  TRI*  Tri_on_side_along_wall(int*,TRI*,int);
IMPORT  bool  is_wall_vertex(TRI*,int);
IMPORT  int   tri_list_along_wall(POINT*,TRI*,TRI***,INTERFACE*);
IMPORT	int   check_wall_crx_orient(INTERFACE *, int *, int *);
IMPORT  bool  edge_index_of_face(int *,int,int,int,GRID_DIRECTION,int*);
IMPORT  bool  vertex_index_of_face(int *,int,int,int,GRID_DIRECTION,int*);
IMPORT	bool  rebuild_intfc_at_crossings3d3(Front *);
...
IMPORT  void    FrontStartUp(Front*,F_BASIC_DATA*);
IMPORT  void    FrontInitIntfc(Front*,LEVEL_FUNC_PACK*);
...
IMPORT  void    set_use_wall_edge(bool);
IMPORT  bool    use_wall_edge(void);
...
IMPORT	void	show_front_output(Front*,char*,bool);
#if defined(USE_HDF)
...
IMPORT	void	set_weight_for_tri_interpolation(float*,TRI*,float*,float*,
	                                         INTERFACE*);
IMPORT  void    f_surface_propagate(Front*,Front*,POINTER,float,float*);
...
IMPORT  void    first_order_point_propagate(Front*,POINTER, POINT*, POINT*,
		                HYPER_SURF_ELEMENT*,HYPER_SURF*,float,float*);
//#bjet2
IMPORT	bool    set_up_wall_tangent_params(Tparams*,POINT*,BOND*,CURVE*,float*,
		SURFACE**,TRI**,INTERFACE*,Front*);
IMPORT	bool    fill_tan_stencil_along_wall(Tan_stencil*,Front*,const Tparams*,
		TRI**,int);
IMPORT	void    find_position_along_wall(float*,TN*,float,const Tparams*,Front*);
...
IMPORT  int     double_vortex_vel(POINTER,Front*,POINT*,HYPER_SURF_ELEMENT*,
		                        HYPER_SURF*,float*);
IMPORT	void	FrontInitVelo(Front*,VELO_FUNC_PACK*);
...
IMPORT	bool    compute_smooth_para(SMOOTH_PARA*,POINT*,TRI*,SURFACE*,
				SMOOTH_TOL*);
IMPORT  bool    point_outside_open_bdry(int*,float*,POINT*,INTERFACE*);
IMPORT	void	smooth_curve(CURVE*, float);
IMPORT	void 	triangle_height_vec(float*,float*,float*,float*);
IMPORT	bool    compute_average_point(SMOOTH_PARA*,POINT*,TRI*,SURFACE*,
					SMOOTH_TOL*);
IMPORT	float	min_null_pair_angle(float*,float*,float*,float*);
...
IMPORT  bool    find_curves_at_rp_with_status(O_CURVE**,O_CURVE**,
                                              int,RPROBLEM*,int);
...
IMPORT  void    scatter_top_grid_float_array(float*,Front*);
IMPORT  void    scatter_cell_index(Front*,int*,int*,POINTER);
IMPORT	bool	cpu_adapt_front(Front*,float,int*,int*);
...
IMPORT	void	set_default_comp(bool);
...
//#bjet2
IMPORT  bool    f_assign_btri_states(BOND_TRI*,BOND_TRI*);
...
IMPORT	float	length_of_scalar_wave(Front*);
...
IMPORT	float	f_mean_curvature_at_point3d(POINT*,HYPER_SURF_ELEMENT*,
					    HYPER_SURF*,Front*);
IMPORT	float	f_mean_curvature_at_point3d1(POINT*,HYPER_SURF_ELEMENT*,
					    HYPER_SURF*,Front*);
IMPORT	float	f_wlsp_curvature(POINT*,HYPER_SURF_ELEMENT*,HYPER_SURF*,Front*);

//#bjet2
IMPORT  void    set_full_average(bool);
...
IMPORT	bool	cross_rect_grid_bdry(BOND*,RECT_GRID*);
...
IMPORT	void	f_set_tangent_function(const char*,TANGENT_FUNCTION*,
                                       INTERFACE*);
IMPORT  void    linear_state_interpolator(float,float,float*,Locstate,float*,
                                       Locstate,RECT_GRID*,Locstate);
IMPORT  bool    linear_tri_state_interpolator(float,float,float,float*,
                                       Locstate,float*,Locstate,float*,
				       Locstate,RECT_GRID*,Locstate);
...
IMPORT  void    test1d(Front*);
	...
	/* fmap.c */
IMPORT  int FrontAdvance(float,float*,Front*,Front**,POINTER);
IMPORT	float FrontHypTimeStep(Front*);
IMPORT	float FrontOutputTimeControl(Front*,bool*,bool*,bool*,int*,int*);
IMPORT  void FrontSetSpacing(Front*,float);
IMPORT  void FrontRedistMesh(Front*);
IMPORT	void FrontSetTriParams(Front*,float,float,float,float);
IMPORT	void FrontResetTriParams(Front*);
IMPORT	void FrontInitStandardIO(int,char**,F_BASIC_DATA*);
IMPORT	void FrontPrintOut(Front*,char*);
IMPORT	void FrontMovieFrame(Front*,char*,bool);
IMPORT	void FrontFreeAll(Front*);
IMPORT	void FrontMakeGridIntfc(Front*);
IMPORT	void FrontFreeGridIntfc(Front*);
IMPORT	void FrontFreeGridIntfc(Front*);
IMPORT	void FrontScatIntfcBuffer(Front*);
IMPORT	void FrontScatExpDualGridArray(float*,Front*);
IMPORT	bool FrontCpuAdaptSubdomain(Front*,float,int*,int*);
IMPORT	bool FrontStateAtGridCrossing(Front*,int*,GRID_DIRECTION,COMPONENT,
			float (*func)(Locstate),float*,float*);
IMPORT	bool FrontIntrpStateAtPoint(Front*,COMPONENT,float*,float*,
			float (*func)(Locstate),float*);
IMPORT  float FrontLinIntrp(float*,INTRP_CELL*,bool);
IMPORT  float FrontBilinIntrp(float*,INTRP_CELL*,bool);
