...
LOCAL	bool  outside_reflecting_bdry(POINT*,INTERFACE*);
LOCAL 	bool  topologically_close_crosses(CROSS*,CROSS*);
	...
	bool		*force_redistribute)
{
	...
	CURVE		**c;
	bool		save_intrp = interpolate_intfc_states(interf);
	...
	bool		status)
{
	...
	float		slopex;	    /* Unit uni_array, joining pair of fr pts */
	float		slopey;
...
LOCAL	bool	outside_reflecting_bdry(
	POINT 	  *p,
	INTERFACE *intfc)
{
	RECT_GRID *gr = computational_grid(intfc);
...
LOCAL bool topologically_close_crosses(
	CROSS *cr1,
	CROSS *cr2)
{
	int num_bonds;
