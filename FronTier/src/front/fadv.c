...
LOCAL	bool	BothSidesActive(HYPER_SURF*,Front*);

LOCAL	int	advance_front1d(float,float*,Front*,Front**,POINTER);
LOCAL	void	EnforceFlowSpecifedStates1d(Front*);

LOCAL	int	advance_front2d(float,float*,Front*,Front**,POINTER);
LOCAL	int	delete_phys_curves_at_old_node(NODE*,int);
LOCAL	int	node_modify_time_step(NODE*,Front*,float*,int);
LOCAL	void	EnforceFlowSpecifedStates2d(Front*);
LOCAL	void	check_bond_lengths(INTERFACE*);

LOCAL 	int 	advance_front3d_tracking_control(float,float*,Front*,
						 Front**,POINTER);

LOCAL 	int 	hybrid_advance_front3d(float,float*,Front*,Front**,POINTER);
LOCAL 	int 	mixed_advance_front3d(float,float*,Front*,Front**,POINTER);
LOCAL	int	preserve_front_advance_front3d(float,float*,Front*,
					       Front**,POINTER);
LOCAL	int	propagate_3d_front(POINTER,Front*,Front*,float,float*,bool);
LOCAL   void    init_intfc_curvature3d1(Front*,INTERFACE*); //for serial mode
LOCAL   void    init_intfc_curvature3d2(Front*,INTERFACE*);
LOCAL   int	propagate_node_points(Front*,Front*,POINTER,float,float*);
LOCAL 	int 	propagate_points_tangentially(Front*,Front*,int,float,float*,
						  int);
LOCAL	int	reconstruct_front_advance_front3d(float,float*,Front*,
						  Front**,POINTER);
LOCAL	void	EnforceFlowSpecifedStates3d(Front*);
LOCAL	void	debug_propagate_3d_front(Front*);
LOCAL	void	propagate_curve_points(Front*,Front*,POINTER,float);
LOCAL	void	unfold_interface_section(POINT*,POINT*,TRI*,TRI*,
	                                 SURFACE*,SURFACE*);
LOCAL   void    propagate_surface_points(Front*,Front*,POINTER,float,float*);

LOCAL	void	detach_and_propagate_curves(Front*,Front*,POINTER,float);
...
/*ARGSUSED*/
EXPORT	bool consistent_propagated_loop_orientations(
	float		dt,
	float		*dt_frac,
	Front		*fr,
	POINTER		wave)
{
	static const float AREA_FAC = 0.01;	/*TOLERANCE*/
	static const int   num_attempts = 5;	/*TOLERANCE*/
	CURVE	**c,*oldc,*newc;
	NODE	*oldn, *newn;
	float	dt_f, dT;
	float	A_old, A_new;
	float	min_area = AREA_FAC*fr->rect_grid->h[0]*fr->rect_grid->h[1];
	bool	status = YES;
	int	i;
...
LOCAL	int	recon_step = 200, recon_start = 502;

int	recon_cond(int);
int	recon_cond(int step)
{
	if(!debugging("recon3d"))
	    return -1;
	if(step < recon_start)
	...
	return (step - recon_start)%recon_step;
}
	...
	POINTER		wave)
{
	...
	if(recon_cond(front->step) == 1)
	{
	    printf("#recon is called.\n");
	    status = reconstruct_front_advance_front3d(dt,dt_frac,
				    front,newfront,wave);
	}
	else
	{
	...
	Front		**newfront)
{
	...
	*newfront = copy_front(front);
	...
	interpolate_intfc_states((*newfront)->interf) = YES;
        ...
        //DEBUG_TMP check_print_intfc("Before test_lgb_interface", "lgb_test_bf", 'g',
                //DEBUG_TMP (*newfront)->interf, 1, -1, NO);
	make_interface_topology_lists((*newfront)->interf);
	//DEBUG_TMP check_valid_intfc("Bf test_lgb", (*newfront)->interf);
	...
	//DEBUG_TMP fflush(NULL);
        ...
        //DEBUG_TMP check_print_intfc("After test_lgb_interface", "lgb_test_af", 'g',
                //DEBUG_TMP (*newfront)->interf, 1, -1, NO);
	make_interface_topology_lists((*newfront)->interf);
	//DEBUG_TMP check_valid_intfc("Af test_lgb", (*newfront)->interf);
	...
	POINTER		wave)
{
	...
	//DEBUG_TMP {
	//test lgb code.
	//DEBUG_TMP for(i=0; i<500; i++)
	//DEBUG_TMP {
	    //DEBUG_TMP printf("#test_lgb_interface bf i = %d.\n", i);
	    //DEBUG_TMP fflush(NULL);
	    //DEBUG_TMP test_lgb_interface(front, newfront);
	    //DEBUG_TMP printf("#test_lgb_interface af i = %d.\n", i);
	    //DEBUG_TMP fflush(NULL);
	    
	    //DEBUG_TMP if(i%50 == 0)
		//DEBUG_TMP long_alloc_view(stdout);
	//DEBUG_TMP }
	//DEBUG_TMP long_alloc_view(stdout);
	...
	bool		reconstruct_front)
{
	...
	check_print_intfc("Before prop previous intfc", "detach_bf", 'g',
                         front->interf, front->step, 101, NO);
	...
	remove_from_debug("cut_ref");
	if(front->step%10 == 0)
	    add_to_debug("cut_ref");
	...
	//TMP sample speed
	//check_print_intfc("After surface prop", "contour", 's', 
	//               newfront->interf, newfront->step, newfront->step, YES);
...
printf("@xiaoxue\n");
	...
	if (front->_tan_point_propagate != NULL && debugging("fcrxsten") )
	{
	    start_clock("tangentiall");
        ...
        //DEBUG_TMP check_print_intfc("After scatter front tangent", "scat_tan", 'g',
                          //DEBUG_TMP newfront->interf, newfront->step, -1, NO);
       ...
       INTERFACE           *intfc)
{
	...
	DEBUG_ENTER(init_intfc_curvature3d)
	   ...
	   if(front->step == 450 && the_point(p))
	   {
		...
		tecplot_interface_in_ball("stri", intfc);
	...
	DEBUG_LEAVE(init_intfc_curvature3d)
}
...
LOCAL void   init_intfc_curvature3d1(
       Front               *front,
       INTERFACE           *intfc)
{
       HYPER_SURF              *hs;
       HYPER_SURF_ELEMENT      *hse;
       POINT                   *p;
       POINT                   **p_list,**p_list_in;
       RECT_GRID               *gr = front->rect_grid;
       float                   h[3],coords[3],T[3];
       float                   lbuf[3],ubuf[3];
       ...
       for(i = 0; i < 3; ++i)
       {
           T[i] = gr->U[i] - gr->L[i];
	   h[i] = gr->h[i];
	   lbuf[i] = h[i]*(gr->lbuf[i]+1.0);
	   ubuf[i] = h[i]*(gr->ubuf[i]+1.0);
	}
	uni_array(&p_list,intfc->num_points,sizeof(POINT*));
	uni_array(&p_list_in,intfc->num_points,sizeof(POINT*));
        count1 = count2 = 0;
        (void) next_point(intfc,NULL,NULL,NULL);
	while (next_point(intfc,&p,&hse,&hs))
	{
	    for(i = 0; i < 3; ++i)
	    {
	        if(Coords(p)[i] < gr->L[i] ||Coords(p)[i] > gr->U[i])
		{
		    p_list[count1] = p;
		    count1++;
		    break;
	    ...
	    if(i == 3)
	    {
	        ...
	        for(j = 0; j < 3; j++)
	        {
	            if(Coords(p)[j] <= gr->L[j]+lbuf[j] ||
		       Coords(p)[j] >= gr->U[j]-ubuf[j])
		    {
		        p_list_in[count2] = p;
			count2++;
			break;
	...
	for(i = 0; i < count1; i++)
	{
	    for(j = 0; j < 3; j++)
	    {
	        if(Coords(p_list[i])[j] < gr->L[j])
	            coords[j] = Coords(p_list[i])[j] + T[j];
		else if(Coords(p_list[i])[j] > gr->U[j])
		    coords[j] = Coords(p_list[i])[j] - T[j];
		else
		    coords[j] = Coords(p_list[i])[j];
            }
	    ...
	    for(j = 0; j < count2; j++)
	    {
	        for(k = 0; k < 3; k++)
	        {
	            if(fabs(Coords(p_list_in[j])[k]-coords[k]) > 0.001*h[k] )
		       break;
		}
		if(k == 3)
		{
		    p_list[i]->curvature = (p_list_in[j])->curvature;
		    break;
		}
	    }
	}
	free_these(2,p_list,p_list_in);
}
	...
	float	dt)
{
	...
	if(debugging("detach_and_propagate_curves"))
	{
	    ...
	    for(i=0, c = front->interf->curves; c && *c; c++, i++)
	    {
	        printf("#c  %d  %3d   ", *c, curve_type(*c));
	        for(cc = new_curves[i]; cc && *cc; cc++)
		    printf("|cc %d %3d  ", *cc, curve_type(*cc));
		printf("|\n");
	    }
	}
...
LOCAL void debug_propagate_3d_front(
	Front 	*newfront)
{
	//DEBUG_TMP (void) printf("step = %d \n", newfront->step);
        //DEBUG_TMP if(newfront->step%10 == 0)
        //DEBUG_TMP     check_print_intfc("After curve prop", "detach_af", 'g',
        //DEBUG_TMP            newfront->interf, newfront->step, -1, NO);
...
void    distort_point(POINT     *);

void    distort_point(POINT     *pt)
{
        float   p0[] = {-0.022, -0.022, 0.32};
        float   nor[] = {0.37756, 0.776678, 1.12557};
        float   tol = 1.0e-4, L = 0.11, hmin = 2.2e-4;
        float   len, v[3];
        int     i;

        //L is the length of v;
        difference(Coords(pt), p0, v, 3);
        len = tol*hmin*Dot3d(v, nor)/Dot3d(nor, nor)/L;

        for(i=0; i<3; i++)
            Coords(pt)[i] += len*nor[i];
}

bool	point_in_nozzle(float*);

bool	point_in_nozzle(
	float	*crds)
{
	float	jetradius=0.1+0.012/4.0;
	float	jetcenter[] = {0.,0.,4.};

	if(crds[2] > jetcenter[2]-0.012 && 
	   crds[2] < jetcenter[2]+0.012 &&
	   sqr(crds[0]-jetcenter[0])+sqr(crds[1]-jetcenter[1]) < sqr(jetradius))
	{
	    return YES;
	}
...
bool	tri_in_nozzle(TRI*);
bool	tri_inflow_side(TRI*);
        ...
        float           *V)
{
	...
	(void) next_point(intfc_old,NULL,NULL,NULL);
	    ...
	    if(debugging("jet3d_nozzle") && wave_type(oldhs) 
	    		 == FIRST_SCALAR_PHYSICS_WAVE_TYPE)
	    {
		if(point_in_nozzle(Coords(oldp)))
		{
