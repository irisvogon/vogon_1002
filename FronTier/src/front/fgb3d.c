...
LOCAL	bool    track_comp_and_repair3d(int*,int*,int*,INTERFACE*,Front*);
LOCAL	bool	repair_intfc3d_in_box(INTERFACE*,int*,int*,Front*);
LOCAL	bool 	set_reconstruction_boxes(int*,int*,int**,int,RECT_BOX**,
                                        INTERFACE*);
LOCAL	bool 	seal_strip_with_tris(RECT_BOX*,TRI**,int*,int,POINT**,
				float*,char*,bool,INTERFACE*);
LOCAL   bool    seal_strip_with_tris_once(RECT_BOX*,TRI**,int*,int,POINT**,
                           float*,char*,bool,INTERFACE*,TRI**,int,TRI**,int);

LOCAL   bool    null_sides_sharing_same_vertex(TRI*,int,TRI**,int);
	bool    null_side_loop(TRI*,int,ORIENTATION,TRI***,int**,POINT***,
                                int*,float**);
LOCAL	bool 	null_side_tri_in_list(TRI**,int,TRI**,int*);
LOCAL	bool 	grid_based_box_untangle(INTERFACE*,RECT_BOX*);
LOCAL   bool    tri_in_box(TRI*,INTERFACE*,RECT_BOX*);
...
LOCAL   bool    pts_in_cross_zone(float*,float,float,int);
LOCAL   bool    pts_cross_line(float*,float,float,int);
LOCAL   bool    tri_cross_zone(TRI*,float,float,int);
LOCAL   bool    tri_cross_line1(TRI*,float,float,int);
LOCAL   void    find_comm_box(INTERFACE*,RECT_BOX*,int*,COMM_BOX*);
LOCAL   void    initialize_comm_box(COMM_BOX*,bool);
...
LOCAL   bool    compare_comm_box(INTERFACE*,RECT_BOX*,
				COMM_BOX*,COMM_BOX*,int*,int);
LOCAL   bool    boxes_sharing_tris(RECT_BOX*,RECT_BOX*, int*, INTERFACE*);
LOCAL   bool    boxes_crxing_same_boundary(RECT_BOX*,RECT_BOX*,INTERFACE*);
LOCAL   void    extend_boundary_side(INTERFACE*,int*,int*,RECT_GRID*);
LOCAL   void    check_and_extend_point(POINT*,TRI*,SURFACE*,float*,
                                float*,float*);
LOCAL   void    check_and_extend_side(TRI*,int,SURFACE*,float*,
                                float*,float*);
LOCAL   bool    crx_bnd_out_tris(TRI**,int,TRI**,int,TRI*,INTERFACE*,RECT_BOX*);
LOCAL   bool    crx_out_tris_twice(TRI**,int,TRI**,int,TRI*,INTERFACE*,RECT_BOX*);
...
LOCAL	bool	check_adjecant_constrain0(POINT*,POINT*,POINT*,POINT*,
					TRI**,int);
LOCAL	void	seal_null_loop(TRI**,int*,POINT**,int);
LOCAL	bool 	rm_bad_crxs_in_box(int*,int*,int**,int,RECT_BOX*,INTERFACE*);
LOCAL	void  	smooth_null_loop(TRI**,int*,POINT**,int);
LOCAL	bool  	set_tst_recon_boxes(int*,int*,RECT_BOX**,INTERFACE*);
	void 	smooth_tris(TRI **,int);

bool    set_use_rect_tris(bool);

int	neighbor_tri_side(TRI*, TRI*);
void    merge_near_tri_pairs(int*,int*,INTERFACE*);
bool	connect_tris_holes(TRI*,TRI*,INTERFACE*,char*);
bool	merge_near_interface(Front*);
...
EXPORT	bool	merge_near_interface(
	Front     *front)
{
...
int compare_crx_pair(const void *a, const void *b)
{
	...
	float	 	dist1 = c1->dist, dist2 = c2->dist;
...
*/
EXPORT	bool	rebuild_intfc_at_crossings3d(
	Front     *front)
{
...
EXPORT	bool	rebuild_intfc_at_crossings3d2(
	Front     *front)
{
	INTERFACE *intfc = front->interf;
	bool   	  sav_intrp = interpolate_intfc_states(intfc);
...
if(debugging("mergecrx"))
{
	...
	if(debugging("addbox"))
	{
	    //float	kmin[3] = {1.38750000e-02, -3.75000000e-04, -9.87500000e-03};
	    //float	kmax[3] = {1.43750000e-02,  3.75000000e-04, -8.87500000e-03};
	    float	kmin[3] = {1.38750000e-02,  0.0, -9.87500000e-03};
	    float	kmax[3] = {1.43750000e-02,  3.75000000e-04, -8.87500000e-03};
	    float	x, y, z, tol;
	    int		i, j, k;
	...
	for (pb = boxes; pb != NULL; pb = pb->next)
	{
	    ...
	    if(fr->step == -1 && box_index == 0)
	    {
		add_to_debug("pairsfix");
	    }
...
EXPORT	bool	check_normal_on_intfc(
	INTERFACE	*intfc)
{
	const float	*nor;
	SURFACE		**s;
	...
	//DEBUG_TMP printf("#check_normal_on_intfc intfc normal is valid.\n");
	return YES;
}
...
delete_s:
	...
	//check_print_front("After grid_based_box_untangle", "gb_un_af", 'g', 
	//		intfc, 1, -1, NO);
...
bool	compute_average_point(SMOOTH_PARA*,POINT*,TRI*,SURFACE*,SMOOTH_TOL*);
	...
	int	num_tris)
{
	...
	//DEBUG_TMP printf("#smooth tris  %d\n", num);
...
LOCAL   bool seal_strip_with_tris_once(
        RECT_BOX *box,
	TRI **null_tris,
	int *null_sides,
	int num_null_sides,
	...
	int num_in_tris)
{
	...
	bool end_in_stitch = NO,status_in = NO,done = NO,test = NO,
	     status_out = NO, crx_buffer2 = NO;
	float dist1,dist2,min_dist = HUGE,min_dist_buffer = HUGE;
	SURFACE *surf;
	FILE *file;
	char fname[100];
	float *h,*L,crx_coord[2],min_dist_buffer2 = HUGE;
	...
	INTERFACE *intfc)
{
	...
	bool end_in_stitch = NO,status = NO;
...
{
	RECT_GRID gr = topological_grid(intfc);
	float *pi,*po, *pi1, *po1,*tp;
	int i,j,k,m,l,test1 = -1,test2,n,test;
	float dist=0.0,dist1=0.0,dist2=0.0;
	float min_dist = HUGE,min_dist_buffer = HUGE;
	float min_dist_buffer2 = HUGE;
	RECT_GRID *rgr = box->grid;
	int ipo[3],tipo[3],ipo1[3],*ipi,*tipi;
	float tol = 1.0e-5,*L,*h,crx_coord[2];
	bool status = NO,crx_buffer2 = NO;
...
LOCAL  bool  check_adjecant_constrain0(
	POINT	*p0,
	POINT	*p1,
	POINT	*p2,
	POINT	*p3, 
...
LOCAL  bool tri_in_box_pre(
       TRI *tri,
       INTERFACE *intfc,
       RECT_BOX *box)
{
       float *L,*h,min_coord,max_coord,p_coord;
       int i,j,k,m,count = 0;
       RECT_GRID gr = topological_grid(intfc);
...
LOCAL   bool null_sides_sharing_same_vertex(
        TRI *tri,
	int side,
	TRI **tri_list,
	int num_tri)
...
LOCAL	bool	merge_adjacent_boxes(
	RECT_BOX	*boxes)
{
	RECT_BOX 	*box, *nbox;
	bool		boxes_merged;
...
LOCAL   bool    use_rect_tris = YES;
bool    set_use_rect_tris(bool);
bool    set_use_rect_tris(bool fg)
{           
	use_rect_tris = fg;
}
...
LOCAL	bool  set_reconstruction_boxes(
	int *smin,
	int *smax,
	int **ips,
	int num_ip,
	...
	INTERFACE	*intfc)
{
	...
	for(box = box1; box != NULL; box = box->next)
	{
	    //DEBUG_TMP print_int_vector("bminn", box->bmin, 3, "\n");
	    //DEBUG_TMP print_int_vector("bmaxn", box->bmax, 3, "\n");
	    //DEBUG_TMP printf("#crxblen %d    %3d %3d %3d\n", i,
	    	//DEBUG_TMP box->bmax[0] - box->bmin[0],
	    	//DEBUG_TMP box->bmax[1] - box->bmin[1],
	    	//DEBUG_TMP box->bmax[2] - box->bmin[2]);
	    i++;
...
LOCAL	bool  set_tst_recon_boxes(
	int *smin,
	int *smax,
	RECT_BOX **boxes,
	INTERFACE *intfc)
{
	...
	*boxes = Box.next;
	...
	for (box = Box.next; box != NULL; box = box->next)
	{
	    //DEBUG_TMP print_int_vector("bminn", box->bmin, 3, "\n");
	    //DEBUG_TMP print_int_vector("bmaxn", box->bmax, 3, "\n");
	    //DEBUG_TMP printf("#blen %d    %3d %3d %3d\n", i,
	    	//DEBUG_TMP box->bmax[0] - box->bmin[0],
	    	//DEBUG_TMP box->bmax[1] - box->bmin[1],
	    	//DEBUG_TMP box->bmax[2] - box->bmin[2]);
	    i++;
...
LOCAL   bool boxes_crxing_same_boundary(
        RECT_BOX  *box,
	RECT_BOX  *nbox,
	INTERFACE *intfc)
{
        int i;
	float min_coord,max_coord,nmin_coord,nmax_coord;
	float *L,crx_tol,*h;
	RECT_GRID gr = topological_grid(intfc);
	RECT_GRID comp_grid = Computational_grid(intfc);
	...
	RECT_GRID comp_gr)
{
        ...
        int start_pts = 1000;
        POINT *p0,*p1,*p2,*p3;
        float dist,min_dist = HUGE;
        float *L,*h,crx_coord[2];
        bool pts_in_match = YES;
	float r0,r1,u0[3],u1[3];
	float lgbtol = 10*MACH_EPS;
...
EXPORT	bool	check_degenerated_loop(
	TRI	**tris,
	int	*sides,
	POINT	**pts,
	int	num_tris)
...
EXPORT  bool check_extension_of_surface_global(
        SURFACE *s)
{
        TRI *tri1,*tri2,*t[2];
        POINT *p1,*p2;
	...
	int side2)
{
	...
	float r0, r1, ke, dm, r2,r3;
	float r4,r5,d,d2,d3,d4,d5;
	float u2[3],u3[3],u4[3],u5[3];
	int i,pi[3],po[3];
	float lgbtol = 10*MACH_EPS,thres = -0.5;
	...
	COMM_BOX         *comm_box)
{
	RECT_GRID gr = topological_grid(intfc);
	RECT_GRID comp_grid = Computational_grid(intfc);
        float *L,*h,crx_tol,min_coord,max_coord;
	...
	INTERFACE *interfa)
{
	...
	info = (POINTER) buf;
	ft_assign(info, &(comm_box->L0),sizeof(float));
	buf += sizeof(float);

	info = (POINTER) buf;
	ft_assign(info, &(comm_box->U0),sizeof(float));
	buf += sizeof(float);

	info = (POINTER) buf;
	ft_assign(info, &(comm_box->L1),sizeof(float));
	buf += sizeof(float);

	info = (POINTER) buf;
	ft_assign(info, &(comm_box->U1),sizeof(float));
	buf += sizeof(float);
	...
	INTERFACE *interfa)
{
	...
	info = (POINTER) buf;
	ft_assign(&(comm_box->L0), info, sizeof(float));
	buf += sizeof(float);

	info = (POINTER) buf;
	ft_assign(&(comm_box->U0), info, sizeof(float));
	buf += sizeof(float);

	info = (POINTER) buf;
	ft_assign(&(comm_box->L1), info, sizeof(float));
	buf += sizeof(float);

	info = (POINTER) buf;
	ft_assign(&(comm_box->U1), info, sizeof(float));
	buf += sizeof(float);
...
LOCAL	bool  set_reconstruction_boxes_prev(
	int *smin,
	int *smax,
	int **ips,
	int num_ip,
...
//rm_bad_crxs_in_box
LOCAL	bool  rm_bad_crxs_in_box_prev(
	int *smin,
	int *smax,
	int **ips_in,
	int num_ip,
