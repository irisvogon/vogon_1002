...
LOCAL   float   compute_curvature2d(float*,float*,float,int);
...
/*----------------------End by Eric-----------------------*/
/*TMP*/
LOCAL void    no_continuation_along_curve(int,ORIENTATION,int,CURVE*,float*,
                                POINT**,BOND**,Front*);
LOCAL void    point_at_distance_along_curve(POINT*,ORIENTATION,float,int,
				POINT**,BOND**,float*,Front*);
	...
	Front		*fr)
{
	...
	case 3:
		...
		//assign normal for restart case
		if(fr->interf != NULL)
		{
		    f_user_interface(fr->interf)._interface_normal_function = 
		        fuh->_interface_normal_function;
		    f_user_interface(fr->interf)._mean_curvature_at_point = 
		        fuh->_mean_curvature_at_point;
		}
	    }
...
//#bjet2
LOCAL  bool full_average = YES;
EXPORT  void   set_full_average(bool y_or_n)
{
    full_average = y_or_n;
}
	...
	INTERFACE *intfc)
{
	...
	for (n = intfc->nodes; n && *n; ++n)
	{
		...
		if (wave_type(cin) == DIRICHLET_BOUNDARY || wave_type(cin) == FLOW_BOUNDARY )
		{
...
{ 
	...
	float	 tmp_p,curvature,tmpx,tmpy; 
	float    tmp[3],x[5],nor[3],local_x[3],local_y[3];
        float    length_x,**new_coords = NULL;
       
	static float    *unit_vec=NULL,*limit=NULL,*height = NULL;
	static float    **rotation=NULL,**tmp2 = NULL,*tmp1 = NULL;
	static float    **tmp_matrix = NULL,**least = NULL;
	...
	if(height == NULL)
	{
	    ...
	    max_curvature = 20.0/fr->rect_grid->h[0];
	}
...
EXPORT float length_of_scalar_wave(Front *front)
{
	float length;
...
LOCAL void point_at_distance_along_curve(
	...
	float		ds,	/* distance along curve for state evaluation  */
