...
LOCAL   void init_blk_edges(BLK_CRX2*,BLK_EDGE*,float*,float*);
...
LOCAL 	void adjust_for_min_spacing2d(CRXING*,float*,int*,float*,int,int,int,
        		int**,CRXING*);
LOCAL   void rm_unphy_crx_along_grid_line2d(INTERFACE*,int*,int*,int*,int*,
			GRID_DIRECTION,CRX_TYPE);
LOCAL	void count_crossings_on_grid(INTERFACE*,float,float,float,float,int,int,
			int,int,float,float,float,float,CURVE*,int*,RECT_GRID*);
LOCAL   void set_crx_storage_for_reconstruction_on_grid2d(INTERFACE*,
			RECT_GRID*);
LOCAL	int grid_crossings_on_segment_on_grid(INTERFACE*,float,float,float, 
			float,int,int,int,int,float,float,float,float,CURVE*,
			int*,int*,RECT_GRID*);
...
LOCAL   int walk_comp_along_grid_line2d(INTERFACE*,int*,int*,int*,int*,
        		GRID_DIRECTION);
LOCAL   bool reconstruct_crx_intfc2d(INTERFACE*,int,int*,int*);
LOCAL   bool remove_unphysical_crossings2d_on_grid(INTERFACE*,INTERFACE*,
        		int*,int*,int*);
LOCAL  	bool track_comp_through_crxings2d_on_grid(int*,int*,int*,INTERFACE*,
        		CRX_TYPE);

static double TOL = 1.0e-6;
static double NTOL = 1.0e-5;
//static double ONEMNTOL = (1.0 - NTOL);
//static double ONEMTOL = (1.0 - TOL);
#define ONEMNTOL (1.0 - NTOL)
#define ONEMTOL (1.0 - TOL)
	...
	RECT_GRID	*rgr)
{
	...
	float           hx = rgr->h[0];
	float           hy = rgr->h[1];
	float           max_lenx,max_leny;
	float		tolx, toly;
	float           *xx_grid,*yy_grid;
	int		n_index;
	int		n_crx = 0;
        int             ix1,ix2,iy1,iy2;
...
LOCAL   bool remove_unphysical_crossings2d_on_grid(
        INTERFACE  *oldintfc,
        INTERFACE  *grid_intfc,
        int        *n_fr_blk,
        int        *smin,
...
LOCAL  bool track_comp_through_crxings2d_on_grid(
        int             *smin,
        int             *smax,
        int             *gmax,
        INTERFACE	*grid_intfc,
        ...
        int          *smax)
{
        ...
        int             blk_ip[2];
        int             l, nc, nn, list;
        float           blk_start[2];   //start position of the blk
        CRXING          *crx;
        ...
        float           *h)
{
		...
		bi_array(&coords,2,2,sizeof(float));
