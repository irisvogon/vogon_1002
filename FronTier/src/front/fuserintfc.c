...
LOCAL	void	state_at_point(COMPONENT,float*,HYPER_SURF_ELEMENT*,
			       HYPER_SURF*,Locstate);
LOCAL 	void 	assign_boundary_node_type(NODE*,INTERFACE*,RECT_GRID*,float);
...
LOCAL	bool	f_set_boundary1d(INTERFACE*,RECT_GRID*,COMPONENT,float);
LOCAL	bool	f_set_boundary2d(INTERFACE*,RECT_GRID*,COMPONENT,float);
LOCAL	void	f_fshow_intfc_states2d(FILE*,INTERFACE*);
LOCAL	void	f_lagrangian_tangent(POINT*,BOND*,CURVE*,float*,Front*);
LOCAL	void	f_wlsp_tangent(POINT*,BOND*,CURVE*,float*,Front*);
// linpack dependent LOCAL	void	f_spline_tangent(POINT*,BOND*,CURVE*,float*,Front*);
...
/*ARGSUSED*/
EXPORT bool linear_tri_state_interpolator(
	float		alpha,
	float		beta,
	float		gamma,
	float		*crds0,
...
LOCAL bool  state_on_bdry_point(
	Locstate     *sl,
	Locstate     *sr,
	POINT	     *p, 
	TRI	     *tri,
...
void	test_normal(INTERFACE*);
void	test_normal(INTERFACE *intfc)
{
	if(f_user_interface(intfc)._mean_curvature_at_point == f_wlsp_curvature)
	    printf("#wlsp is used\n");
	if(f_user_interface(intfc)._mean_curvature_at_point == f_mean_curvature_at_point3d)
	    printf("#old c is used\n");
}
	...
	int             dim)
{
	...
	case SINE_WEIGHTED_NORMAL:
	    ...
	    fuh->_mean_curvature_at_point = f_mean_curvature_at_point3d;
	    fuh->_interface_normal_function._normal_name =
	        strdup("f_sine_weighted_normal3d");
	    break;
	case WLSP_NORMAL:
	default:
	    ...
	    fuh->_mean_curvature_at_point = f_wlsp_curvature;
	    fuh->_interface_normal_function._normal_name =
	        strdup("f_wlsp_normal");
	}
        ...
        Front           *front)
{
	...
	//    f_mean_curvature_at_point3d(p,hse,hs,front));
	...
	INTERFACE *intfc)	/* intfc may not be the same as front->interf */
{
	...
	if (debugging("wlsp_curvature"))
	    ...
	    {
		if (wave_type(hs) == SUBDOMAIN_BOUNDARY ||
		    wave_type(hs) == PASSIVE_BOUNDARY) 
		    continue;
                if (!WLSP_compute_normal2d(p,hse,hs))
		{
		    /* Use old algorithms as back-up */
