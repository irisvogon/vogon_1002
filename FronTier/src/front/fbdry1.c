...
LOCAL	bool	interior_curve_crosses_boundary(CROSS*);
...
LOCAL	void	nearest_boundary_curve(float*,INTERFACE*,float*,BOND**,
				       CURVE**,COMPONENT*,int*);
LOCAL	void	nearest_linked_bdry_like_curve(POINT*,BOND*,CURVE*,float*,
					       BOND**,CURVE**,int);
	...
	bool		normalize)
{
	...
	BOND		*bbdry;
	CURVE		*cbdry;
	float		t, tgnt[MAXD], db[MAXD], wtan;
	...
	int		*type)
{
	...
	CURVE		**c1;
	POINT		*ps, *pe;
	float		coords_c1[MAXD];
	float		l1s, l1e;
	float		residual = HUGE_VAL, new_residual;
	float		d1,d2,d3;
	float		dim = intfc->dim;
	int		side;
	int		i;
...
LOCAL	bool interior_curve_crosses_boundary(
	CROSS		*cross)
{
	BOND		*btmp;
	CURVE		*ctmp;
