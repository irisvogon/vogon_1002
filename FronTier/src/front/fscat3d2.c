...
LOCAL	bool      append_adj_intfc_to_buffer2(INTERFACE*,INTERFACE*,RECT_GRID*,
					      RECT_GRID*,int,int);
LOCAL	bool      append_adj_intfc_to_buffer2_old(INTERFACE*,INTERFACE*,RECT_GRID*,
					      RECT_GRID*,int,int);
...
LOCAL   bool  seal_closed_curve(CURVE*, int);
LOCAL   void  set_btri_on_bond(BOND*, CURVE*);
LOCAL   void  check_bond_comp(const char*, BOND*, BOND*);
LOCAL   void  merge_overlap_curves(INTERFACE *);

LOCAL	float	ltol[3];/*LINE TOLERANCE*/
//BTMPM
LOCAL   bool    comp_comm = YES;
LOCAL   void    set_comp_comm(bool  y_or_n) 
{
	comp_comm = y_or_n;
}
	...
	Front		*fr)
{
	...
	if(Tracking_algorithm(fr) == THREE_COMP_GRID_BASED_TRACKING)
	    set_comp_comm(NO);
	else    
	    set_comp_comm(YES);
	...
	{
	    //only clip_front_to_subdomain need this null...
	    //printf("#clip org intfc bf\n");
	    null_sides_are_consistent();
	    if (!consistent_interface(intfc))
	    {
		screen("ERROR in f_intfc_communication3d2(), "
		       "grid boundary interface is inconsistent\n");
		clean_up(ERROR);
	    }
	    else
	    {
	        printf("#clip_intfc_at_grid_bdry2 bf. intfc con.\n\n");
	    }
	}
	...
	if(debugging("out_surf"))
	{
	    ...
	    printf("#clip_org_intfc af\n");
	    sprintf(s, "intfc_clip%d.plt", pp_mynode());
	    ...
	    {
	        printf("#clip_intfc_at_grid_bdry2 af. intfc con.\n\n");
	    }
	}
	...
	for (i = 0; i < dim; ++i)
	{
		...
		/*Receive adjacent buffer region if necessary*/
			...
			{
			    null_sides_are_consistent();
			    if(!f_consistent_interface(adj_intfc[(j+1)%2]))
		            {
		                printf("#recv intfc incon\n");
		            }
			    else
			    {
		                printf("#recv intfc con\n");
		            }
			}
		...
		if (rect_boundary_type(intfc,i,j) == SUBDOMAIN_BOUNDARY)
			...
			//if(pp_mynode()==8 || pp_mynode() == 12)
			    ...
			    sprintf(s, "buf_intfc%d_%d_%d.plt", pp_mynode(), i, j);
			    tecplot_interface(s, intfc);
			    
			    sprintf(s, "buf_adj_intfc%d_%d_%d.plt", pp_mynode(), i, j);
			    tecplot_interface(s, adj_intfc[j]);
			    
			    printf("#check intfc consistence %d %d\n", i, j);
			    null_sides_are_consistent();
			    if(!consistent_interface(intfc))
			    {
			    ...
			    printf("#check adj_intfc consistence %d %d\n", i, j);
			    null_sides_are_consistent();
			    if(!consistent_interface(adj_intfc[j]))
			    {
	...
	bool		status)
{
	...
	if(comp_comm)
	{
	    if(!append_adj_intfc_to_buffer2_old(intfc,adj_intfc,gr,&dual_gr,dir,nb))
	    {
...
LOCAL   bool  seal_closed_curve(CURVE *c, int n)
{
	...
	if(n == 0)
	{
	    ...
	    {
	        printf("ERROR in seal_closed_curve, delete from_end node failed.\n");
		clean_up(ERROR);
	    }
	    delete_node(c->end);
	    c->end=c->start;
	    ...
	    {
	        printf("ERROR in seal_closed_curve, add_to_end node failed.\n");
		clean_up(ERROR);
	    }
	    return YES;
	}
	...
	if(n == 1)
	{
	    ...
	    {
		printf("ERROR in seal_closed_curve, delete from_start node failed.\n");
		clean_up(ERROR);
	    }
	    delete_node(c->start);
	    c->start=c->end;
	    ...
	    {
		printf("ERROR in seal_closed_curve, add_to_start node failed.\n");
		clean_up(ERROR);
	    }
	    return YES;
	}
...
BOND    *b1;
	...
	for(b1=c->first; b1; b1=b1->next )
	    ...
	    {
	        printf("ERROR: find_match_bond, try to merge reverse order curve.\n");
		clean_up(ERROR);
	    }
	return NULL;
}
...
LOCAL  void  merge_overlap_curves(INTERFACE *intfc)
{
CURVE     **c, **curve;
BOND      *b1, *b2;
BOND_TRI  **btris, **btris1, *bt;
...
EXPORT   void  merge_curves(INTERFACE *intfc)
{
CURVE     **c, **curve;
SURFACE	  *surf;
BOND      *b, *b1, *b2, *bc, *bo;
...
merge_curve:
	for (c = intfc->curves; c && *c; c++)
	{
		...
		if(bc != NULL)
		{
			...
			//printf("case 1: change bond to curve %p\n", *c);
			//print_bond(b2);
			...
			if((*c)->first->start == (*c)->last->end)
			{
			    ...
			    merge_btris(b1->prev, (*c)->last, *c,  NEGATIVE_ORIENTATION, intfc);
	...
	//     link_tri_list_to_surface   //this function relinks the tri->surf to surface
	...
	for(c = intfc->curves; c && *c; c++)
	{
		...
		for(bt = Btris(b); bt && *bt; bt++)
		{
		    //surf = which_surface_tri_on(intfc, (*bt)->tri);
		    surf = (*bt)->tri->surf;
		    if((*bt)->surface != surf)
			(*bt)->surface = surf;
		}
	...
	int		nb)
{
	...
	for (as = adj_intfc->surfaces; as && *as; ++as)
	{
		   ...
		   positive_component(*as) == positive_component(*s))
		{
		    ...
		    {
		        printf("surface %d [%d %d] of intfc (%d) join buf surf %d [%d %d] of adj_intfc (%d)\n",
                             *s, negative_component(*s), positive_component(*s), intfc,
                             *as, negative_component(*as), positive_component(*as), adj_intfc);
		    }
...
LOCAL 	bool append_adj_intfc_to_buffer2_old(
	INTERFACE	*intfc,		/* my interface 	       */
	INTERFACE	*adj_intfc,	/* received interface	       */
	RECT_GRID	*grid,		/* Rectangular grid for region */
	RECT_GRID	*dual_gr,	/* Dual grid for region        */
