...
LOCAL	bool	fill_comp_from_solid_intfc(Front *, int *, int *);
LOCAL	void    set_wall_flag_storage(INTERFACE *);
LOCAL	void    free_wall_flag_storage(INTERFACE *);
LOCAL	bool    set_wall_crossings3d(INTERFACE *, int *, int *);
LOCAL	bool    fix_crx_on_wall(CRXING*,INTERFACE*,int, int);
LOCAL	bool    make_wall_surfaces(INTERFACE *, int *, int *);
...
LOCAL   void    adjust_face_crossing(float *, INTERFACE *, int *, GRID_DIRECTION);
...
EXPORT	bool	rebuild_intfc_at_crossings3d3(
	Front     *front)
{
...
EXPORT  bool  is_wall_side(TRI *t, int side)
{
BOND_TRI    *btri;
CURVE	    *c;
...
int	    i;
	...
	printf("#across curve  %d  %d\n", n_t, *is);

	return n_t;
}

//YES mean vertex is not on NEUMANN_CURVE_P and NEUMANN_CURVE_W
EXPORT  bool  is_wall_vertex(TRI *t, int iv)
{
SURFACE      *surf;
POINT	     *p;
TRI	     **tris;
...
EXPORT bool edge_index_of_face(
	int 	*ie, 
	int 	ix,
	int	iy,
	int	iz, 
...
EXPORT bool vertex_index_of_face(
	int 	*iv, 
	int 	ix,
	int	iy,
	int	iz, 
...
LOCAL bool  tst_line_tri_crossing_on_face(
	float	   	*crx,
	TRI		**ptri,
	SURFACE		**psurf,
	float	   	*pt1,	//two points of the line, should be on face icoords.
	float	   	*pt2,
	int	   	*icoords,
	INTERFACE  	*intfc,
	float		tol)
...
LOCAL bool  line_tri_crossing_on_face(
	float	   	*crx,
	TRI		**ptri,
	SURFACE		**psurf,
	float	   	*pt1,	//two points of the line, should be on face icoords.
...
LOCAL  bool  insert_fixed_curve_crx_on_face(
	POINT	   *p, 
	BOND	   *b,
	CURVE      *curve, 
	int	   *smin, 
	...
	int	   *index)
{
	...
	BOND_TRI	**btris;
	Locstate  	sl, sr;
	float		face_crds[3], *coords, d, dmin;
	int		i, j, k;
	...
	int		*index)
{
	...
	POINT		*newp;
	Locstate  	sl, sr;
	float		crds_crx[MAXD], *pt1, *pt2, *coords;
	float           *h = rgr->h, tol;
	    ...
	    for (iy = smin[1]; iy <= smax[1]; ++iy)
	    {
			...
			for(k=0; k<4; k++)
				...
				ort = curve_surface_orientation(surf, curve);
			        printf("%2d %10d | %2d | %2d %2d | %2d %2d | %2d %2d |%10s\n",k, surf, is_wall_surface(surf),
	...
	int       *smax)
{
	...
	bool	   sav_copy_st = copy_intfc_states();
	GRID_DIRECTION   dir[3] = {EAST,NORTH,UPPER};
	RECT_GRID  *gr = &topological_grid(intfc);
	Table	   *T = table_of_interface(intfc);
	COMPONENT  *comp = T->components;
...
LOCAL	bool set_wall_crossings3d(
	INTERFACE *intfc,
	int *smin,
	int *smax)
{
...
COMPONENT      *comp_node = T->components;
int	       *gmax = gr->gmax;
int	       ix, iy, iz, i, ip1[3], ip2[3];
float	       p0[3], tol = 1.0e-5;
	...
	*c = comp_node[d_index3d(ip1[0],ip1[1],ip1[2],gmax)];

	return NO;
}

EXPORT	bool	fill_comp_from_prev_intfc(
	INTERFACE *intfc,
	int       *smin,
	int       *smax)
{
...
float	       h = min3(gr->h[0], gr->h[1], gr->h[2]), pt[3];
	...
	bool	  copy_st)
{
	...
	{
	    char  s[40];
	    sprintf(s, "fill_comp%d_%d.plt", front->step, pp_mynode());
	    tecplot_interface(s, intfc);
	}

	*cc = comp1;

	//printf("#merge_surfs 2\n");
...
LOCAL	bool	fill_comp_from_solid_intfc(
	Front     *front,
	int       *smin,
	int       *smax)
{
	...
	//show_grid_components(smin,smax,2,intfc);
	//intfc->modified = NO;
	//extend_fluid_bdry_side(front, intfc, comp1);
