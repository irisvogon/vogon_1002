...
LOCAL void oned_turbulence_boundarylayer_solver(
		...
		double *flux,
                Locstate statep)	
{     
	printf("oned_trubulence_boundary_solver!\n");
        ...
        Wave            *wave)
{
	...
	float   sin10 =  sin(3.1415926*10/180); 
	float   yd;  
	printf("imin[%d] = %d  imax[%d] = %d imin[%d] = %d  imax[%d] = %d imin[%d] = %d  imax[%d] = %d \n", 0,imin[0],0,imax[0],1,imin[1],1,imax[1],2,imin[2],2,imax[2]);
	//debug lines
	printf("z_direction!\n");
        for (i0 = imin[0]; i0 < imax[0]; i0++)
        {
			...
			icoords[2] = i2;
                        state = Rect_state(icoords,wave);
		        if(is_obstacle_state(state)) printf("Obstacle!\n");
                        coords = Rect_coords(icoords,wave);
			printf("icoords is %d  %d  %d coords is %f  %f   %f\n", icoords[0],icoords[1],icoords[2], coords[0],coords[1],coords[2]);
		}
	   }
	}
	printf("x_direction!\n");
...
/*        for (i0 = imin[0]; i0 < imax[0]; i0++)
        {
            ...
            for (i1 = imin[1]; i1 < imax[1]; i1++)
            {
                ...
                for (i2 = imin[2]; i2 < imax[2]; i2++)
                {
                    icoords[2] = i2;
                    state = Rect_state(icoords,wave);
		    if(is_obstacle_state(state)) continue;
                    coords = Rect_coords(icoords,wave);
			...
			Locstate state0;
		        for(i = 0; i < 3; i++)
				icoords2[i] = icoords[i];
			for(i = -3; i < 3; i++)
			{
				icoords2[0] = icoords[0] + i;
				coords2 = Rect_coords(icoords2,wave);
				state0 = Rect_state(icoords2,wave);
				printf(" icoords is %d  %d  %d coords is %f  %f   %f\n", icoords2[0],icoords2[1],icoords2[2], coords2[0],coords2[1],coords2[2]);
				if(is_obstacle_state(state0)) printf("obstacle!\n");
			}
			...
			double* coordstencl;
			if(is_obstacle_state(state))printf("the state we update is obstacle!\n");
			for(i = 0; i < 2; i++)
				for( j = 0; j < 2; j ++)
				{
					if(is_obstacle_state(statep[2*i+j]))
					printf("obstacle!\n");
					coordstencl = Rect_coords(icoordsp[i][j],wave);
					printf("coords is %f  %f  %f with icoordinate %d  %d  %d \n", coordstencl[0],coordstencl[1],coordstencl[2],icoordsp[i][j][0],icoordsp[i][j][1],icoordsp[i][j][2]);
					
					printf("the point we update is %f  %f  %f with icoordinate %d   %d  %d\n", coords[0],coords[1],coords[2], icoords[0],icoords[1],icoords[2]);
			...
			y0 = yd + delta;
			yw = yd;
			printf("before oned_turbulenct at the ramp.\n");
		    	if ( fabs(u[0]) > MACH_EPS || fabs(u[1]) > MACH_EPS)
                    	    oned_turbulence_boundarylayer_solver(dim,u[0],u[1],u[2],y0,yw,flux,state2);
			printf("after oned_turbulence at the ramp.\n");
			float f[3];
			for( i = 0; i < 3; i++)
			...
			Flux(state)[5][1] += flux[0];
			    ...
			    Boundary_flux(state)[2] =  flux[2];
			    Boundary_flux(state)[3] =  flux[3];
			    printf("ramp flux[0] = %f, flux[1] = %f, flux[2] = %f,flux[3] = %f\n", flux[0],flux[1],flux[2],flux[3]);
		    }
		}
	    }
	}      
