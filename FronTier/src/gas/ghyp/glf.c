	...
	Stencil		*sten)
{
	...
	{
	    printf("#neg dens \n");
	    print_general_vector("p = ", Coords(sten->p[0]), 3, "\n");
	}
	...
	if(g_composition_type() == MULTI_COMP_NON_REACTIVE)
        {
            if(Params(s2)->n_comps != 1)
            {
	            ...
	            pdens(ans)[i] = 0.5*(pdens(s1)[i]+pdens(s3)[i]) 
                                   - lm*(v3*pdens(s3)[i] - v1*pdens(s1)[i]);
...
//TMP_TK
	if (!check_ans(fname[idir],dh,dt,ans,comp,sten,NO))
	{
	    ...
	    (void) printf("idir = %d\n",idir);
	    print_general_vector("#TK p = ", Coords(sten->p[0]), 3, "\n");
	    verbose_print_state("s1",s1);
	    verbose_print_state("s2",s2);
	    verbose_print_state("s3",s3);
	    verbose_print_state("ans",ans);
	...
	Front		*fr)
{
	...
	float           pr1, pr3;
#if defined(COMBUSTION_CODE)
	float		coef, rt2, rt1, rt3;
	int		reaction;
#endif /* defined(COMBUSTION_CODE) */
	static bool	is_grav = NO;
	static float    a;
	float           rad1, rad3, rmin;
	float           k1, k3;
	static bool     first = YES;
