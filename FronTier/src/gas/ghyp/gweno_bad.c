...
/*	    float *coords;
	    ...
	    if (coords[0] >=-4.0659 && coords[0] <= -4.0658 && coords[1] >= -3.9323 && coords[1] <= -3.9322 && coords[2] >= 2.1510 && coords[2] <= 2.1511)	{
		    printf("swp_num = %d\n", swp_num);
		    Locstate tmpst = Rect_state(icoords, wave);
		    verbose_print_state("XY", tmpst);
		    printf("In vector part.\n");
...
/*	for (j = 0; j < vsize + 2; ++j) {
	...
	//	u[i][vsize+1] = uu1[i][vsize+1] = uu2[i][vsize+1] = u[i][vsize];
		u[i][vsize+1] = u[i][vsize];
		uu1[i][vsize+1] = u[i][vsize];
		uu2[i][vsize+1] = u[i][vsize];
	...
	//	uu1[i][vsize-1] = uu2[i][vsize-1] = u[i][vsize-1];
		uu1[i][vsize] = u[i][vsize];
		uu2[i][vsize] = u[i][vsize];
		uu1[i][vsize-1] = u[i][vsize-1];
		uu2[i][vsize-1] = u[i][vsize-1];
	}
...
//		F_mid[j][5+i] = F_mid[j][0]*u[5+i][j]/u[0][j];
	...
	for ( j = start; j < end; j++ )
		for ( i = 5; i < kmax; i++ )
		    if (uu1[i][j] < 0 && fabs(uu1[i][j]) > 1.e-8) {
			printf("Step 1: uu1[%d][%d] = %e\tu[%d][%d] = %e\tF_mid[%d][%d] = %e\tF_mid[%d][%d] = %e\n", i, j, uu1[i][j], i, j, u[i][j], j, i, F_mid[j][i], j+1, i, F_mid[j+1][i]);
			for (int jj = j - 3; jj < j + 4; ++jj) {
			    if (jj >=0) {
			    printf("u[0][%d] = %e   ", jj, u[0][jj]);
			    for (int ii = 5; ii < kmax; ++ii)
				if (u[ii][jj] != 0)
				    printf("u[%d][%d] = %e   ", ii, jj, u[ii][jj]);
			    }
			    printf("\n");
			}
		    }
...
//	for (j = start; j < end + 1; ++j)
//	    for (i = 0; i < num_comps; ++i)
//		F_mid[j][5+i] = F_mid[j][0]*uu1[5+i][j]/uu1[0][j];
	...
	for ( j = start; j < end; j++ )
		for ( i = 5; i < kmax; i++ )
		    if (uu2[i][j] < 0 && fabs(uu2[i][j]) > 1.e-8) {
			printf("Step 2: uu2[%d][%d] = %e\tuu1[%d][%d] = %e\tF_mid[%d][%d] = %e\tF_mid[%d][%d] = %e\n", i, j, uu2[i][j], i, j, uu1[i][j], j, i, F_mid[j][i], j+1, i, F_mid[j+1][i]);
			for (int jj = j - 3; jj < j + 4; ++jj) {
			    if (jj >= 0){
			    printf("uu1[0][%d] = %e   ", jj, uu1[0][jj]);
			    for (int ii = 5; ii < kmax; ++ii)
				if (uu1[ii][jj] != 0)
				    printf("uu1[%d][%d] = %e   ", ii, jj, uu1[ii][jj]);
			    }
			    printf("\n");
			}
		    }
...
//	for (j = start; j < end + 1; ++j)
//	    for (i = 0; i < num_comps; ++i)
//		F_mid[j][5+i] = F_mid[j][0]*uu2[5+i][j]/uu2[0][j];
	...
	for ( j = start; j < end; j++ )
		for ( i = 5; i < kmax; i++ )
		    if (uu2[i][j] < 0 && fabs(uu2[i][j]) > 1.e-8) {
			printf("Step 3: u[%d][%d] = %e\tuu2[%d][%d] = %e\tF_mid[%d][%d] = %e\tF_mid[%d][%d] = %e\n", i, j, u[i][j], i, j, uu2[i][j], j, i, F_mid[j][i], j+1, i, F_mid[j+1][i]);
			for (int jj = j - 3; jj < j + 4; ++jj) {
			    if (jj >= 0) {
			    printf("uu2[0][%d] = %e   ", jj, uu2[0][jj]);
			    for (int ii = 5; ii < kmax; ++ii)
				if (uu2[ii][jj] != 0)
				    printf("uu2[%d][%d] = %e   ", ii, jj, uu2[ii][jj]);
			    }
			    printf("\n");
			}
		    }
...
//			F_mid[j][5+i] = F_mid[j][0]*u[5+i][j]/u[0][j];
	...
	for ( j = startx[0]; j < endx[0]; j++ )
		for ( i = 5; i < kmax; i++ )
		    if (uu1[i][j] < 0 && fabs(uu1[i][j]) > 1.e-8) {
			printf("startx[0] = %d  endx[0] = %d\n", startx[0], endx[0]);
			printf("Step 1(1): uu1[%d][%d] = %e\tu[%d][%d] = %e\tF_mid[%d][%d] = %e\tF_mid[%d][%d] = %e\n", i, j, uu1[i][j], i, j, u[i][j], j, i, F_mid[j][i], j+1, i, F_mid[j+1][i]);
			for (int jj = j - 3; jj < j + 4; ++jj) {
			    if (jj >= 0) {
			    printf("u[0][%d] = %e   ", jj, u[0][jj]);
			    for (int ii = 5; ii < kmax; ++ii)
...
//			F_mid[j][5+i] = F_mid[j][0]*uu1[5+i][j]/uu1[0][j];
	...
	for ( j = startx[1]; j < endx[1]; j++ )
		for ( i = 5; i < kmax; i++ )
		    if (uu2[i][j] < 0 && fabs(uu2[i][j]) > 1.e-8) {
			printf("Step 2(1): uu2[%d][%d] = %e\tuu1[%d][%d] = %e\tF_mid[%d][%d] = %e\tF_mid[%d][%d] = %e\n", i, j, uu2[i][j], i, j, uu1[i][j], j, i, F_mid[j][i], j+1, i, F_mid[j+1][i]);
			for (int jj = j - 3; jj < j + 4; ++jj) {
			    if (jj >= 0) {
			    printf("uu1[0][%d] = %e   ", jj, uu1[0][jj]);
			    for (int ii = 5; ii < kmax; ++ii)
...
//			F_mid[j][5+i] = F_mid[j][0]*uu2[5+i][j]/uu2[0][j];
	...
	for ( j = startx[2]; j < endx[2]; j++ )
		for ( i = 5; i < kmax; i++ )
		    if (uu2[i][j] < 0 && fabs(uu2[i][j]) > 1.e-8) {
			printf("Step 3(1): u[%d][%d] = %e\tuu2[%d][%d] = %e\tF_mid[%d][%d] = %e\tF_mid[%d][%d] = %e\n", i, j, u[i][j], i, j, uu2[i][j], j, i, F_mid[j][i], j+1, i, F_mid[j+1][i]);
			for (int jj = j - 3; jj < j + 4; ++jj) {
			    if (jj > 0) {
			    printf("uu2[0][%d] = %e   ", jj, uu2[0][jj]);
			    for (int ii = 5; ii < kmax; ++ii)
...
*/	
        ...
        if( (swp_num == 2) /*&& (sten==NULL)*/)
	{
		...
		//TMP_XY_weno
/*	    	if (coords[0] >=-4.0659 && coords[0] <= -4.0658 && coords[1] >= -3.9323 && coords[1] <= -3.9322 && coords[2] >= 2.1510 && coords[2] <= 2.1511)	{
			Locstate tmpst = Rect_state(ic, wave);
			verbose_print_state("XY_before_chem", tmpst);
			printf("Coords: (%f, %f, %f)\n", coords[0], coords[1], coords[2]);
		}
*/
...
/*	    	if (coords[0] >=-4.0659 && coords[0] <= -4.0658 && coords[1] >= -3.9323 && coords[1] <= -3.9322 && coords[2] >= 2.1510 && coords[2] <= 2.1511)	{
			Locstate tmpst = Rect_state(ic, wave);
			verbose_print_state("XY_after_chem", tmpst);
			printf("Coords: (%f, %f, %f)\n", coords[0], coords[1], coords[2]);
		}
*/
            } 
...
//			F_mid[j][k] = F_mid[j][0] * u[k][j-1] / u[0][j-1];
		//TMP_XY_weno
		if (F_mid[j][0] >= 0)
		    for (k = 5; k < 5 + num_comps; ++k)
			F_mid[j][k] = F_mid[j][0] * u[k][j-1] / u[0][j-1];
		    ...
		    for (k = 5; k < 5 + num_comps; ++k)
			F_mid[j][k] = F_mid[j][0] * u[k][j] / u[0][j];
	}
