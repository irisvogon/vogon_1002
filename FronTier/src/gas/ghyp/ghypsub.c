	...
	Front		*fr)
{
	...
	for (i = 0; i < npts; ++i)
	{
	    ...
	    for (j = 0; j < dim; ++j)
	    	vel_field[j][i] = scalar_product(Q[j],Vel_Field(st),dim);
	...
	Vec_Gas_field_set(vst,vel_field) = YES;
        if(g_composition_type() == MULTI_COMP_NON_REACTIVE)
        {
	...
	int	 dim)
{
	...
	VECTOR(vst,wall_unit,vctr_size,FLOAT);
	MATRIX(vst,vel_field,MAXD,vctr_size,FLOAT);
	...
	VECTOR(vst,coords,vctr_size,sizeof(float*));
	return vst;
}		/*end g_alloc_vgas*/
	...
	Vec_Gas* vst)
{
	...
	Set_free(vst,m);
	Set_free(vst,wall_unit);
	Set_free(vst,vel_field);
	Set_free(vst,v);
	Set_free(vst,rho);
	Set_free(vst,en_den);
	Set_free(vst,e);
	...
	Vec_Muscl	*vmuscl)
{
	...
	{
	    float curvature = fabs(vmuscl->tsten->curvature);
	    float dh = mag_vector(vmuscl->front->rect_grid->h,
	                          vmuscl->front->rect_grid->dim);
	    float curvature_chi_mod =
...
EXPORT	void	compute_slope_limiting_coeffs_prev(
	int		start,
	int		end,
	Vec_Muscl	*vmuscl)
{
	...
	for (j = start; j < end; ++j)
	{
	    ...
	    if ((p[j+1]-p[j])*(p[j] - p[j-1]) < -eps*min(p[j+1],p[j-1]))
	    {
		chi[j] = 0.0;
	    }
	    else if (use_msf)
	    {
	        /* No corrections except for sufficiently strong waves */
	        dp = p[j+1] - p[j-1];
	        if (fabs(dp) > min(p[j+1],p[j-1])*eps)
		{
	            du = vn[j+1] - vn[j-1];
	            s = dp*du;
	            if (s > 0.0)
...
{
	int   j;
	float *wall_unit = vst->wall_unit + offset;
	...
	float *mn = vst->m[0] + offset, *vn = vst->v[0] + offset;
	float *vel_field = vst->vel_field[0] + offset;

#if !defined(UNRESTRICTED_THERMODYNAMICS)
	float 	*min_energy = vst->min_energy + offset;
#endif /* !defined(UNRESTRICTED_THERMODYNAMICS) */
	...
	case 2:
	    {
	        float *m2 = vst->m[1] + offset, *v2 = vst->v[1] + offset;
		float *vel_field2 = vst->vel_field[1] + offset;
	...
	case 3:
	    {
	        float *m2 = vst->m[1] + offset, *v2 = vst->v[1] + offset;
	        float *m3 = vst->m[2] + offset, *v3 = vst->v[2] + offset;
		float *vel_field2 = vst->vel_field[1] + offset;
		float *vel_field3 = vst->vel_field[2] + offset;
	...
	for (j = 0; j < vsize; ++j){
	    c[j] = sqrt(c2[j]);
	    if ( isnan(c[j]) ) printf("NAN sound speed c2 = %e\n",c2[j]); 
	}
	...
	Vec_Gas *vst)
{
	...
	Vec_Gas_field_set(vst,wall_unit) = NO;
	Vec_Gas_field_set(vst,vel_field) = NO;
	...
	int		pbuf)
{
	...
	for (i = imin; i < imax; ++i)
	{
		...
		Locstate state_old = Rect_state(icoords,wv);
		int j;	
		for(k =0; k< 4; k++)
			Boundary_flux(state)[k] = Boundary_flux(state_old)[k];
		
		flux_flag(state) = flux_flag(state_old);
		...
		{
		    for( j = 0; j < 3; j++)
			    Tau(state)[k][j] = Tau(state_old)[k][j];
		    for( j = 0; j < 10; j++)
