...
LOCAL   double		 wall_pressure_min;
	...
	Front *front)
{
	...
	Locstate st, out_st;
	int   	 st_type, icoords[3],out_icoords[3];
	float 	 *coords, *out_coords;
...
/* TK for nozzle + outflow */
	for(i=0; i<Mx+front->rect_grid->ubuf[0]; ++i)
	   ...
	   {	
		...
		get_constant_state(st, 2, coords, front->time);
	      if(icoords[0] == (Mx-1) && coords[0] > (front->rect_grid->GU[0]-2*dh) && !is_obstacle_state(st))
	      {
		...
		double rho_bc = wall_pressure_avg/pressure(st) * Dens(st);

	        set_state(st, GAS_STATE, st);
		for(int kk=0; kk<dim; ++kk)
		 	Mom(st)[kk] = rho_bc/Dens(st)*Mom(st)[kk];
		Energy(st) = rho_bc / Dens(st) * Energy(st);		
		
		if(g_composition_type() == MULTI_COMP_NON_REACTIVE)
                {
                    if(Params(st)->n_comps != 1)
	   		for(int kk = 0; kk < Params(st)->n_comps; kk++)
                            pdens(st)[kk] = rho_bc/Dens(st)*pdens(st)[kk];
		}
		Dens(st) = rho_bc;
		//verbose_print_state("#TK-out-ext",st);
	...
	//printf("#TK-wall_pressure_avg = %f\n",wall_pressure_avg);
		//verbose_print_state("#TK-out-newG",st);
	     ...
	     else if(icoords[0]>=Mx && coords[0] > front->rect_grid->GU[0] && !is_obstacle_state(st))
	     {	
	
		out_icoords[0] = Mx-1;
	        out_icoords[1] = icoords[1];
		out_icoords[2] = icoords[2];
	        out_coords = Rect_coords(out_icoords,wave);
	        out_st = Rect_state(out_icoords, wave);
		st_type = state_type(st);
		set_state(st, GAS_STATE, st);
		//set_state(out_st, GAS_STATE, out_st);
		//printf("#TK-out coords %f %f %f \n",out_coords[0], out_coords[1], out_coords[2]);
		//verbose_print_state("#TK-out_st-1",out_st);
		//verbose_print_state("#TK-st-1",st);

		for (ii = 0; ii < dim; ++ii)
            	     Mom(st)[ii] =  mom(ii,out_st);
		Energy(st) = energy(out_st);
		Dens(st) =  Dens(out_st);
		for( ii = 0; ii < Params(st)->n_comps ; ii++)
		    pdens(st)[ii] =  pdens(out_st)[ii];

		//verbose_print_state("#TK-st-2",st);
...
//	    set_type_of_state(st,TGAS_STATE);
...
//		printf("Mach too small: %f\n",m);
	    ...
	    p_min = min(pressure(st), p_min);
	    //p_min = min(Press(st), p_min);
    ...
    pp_global_min(&p_min, 1);

    double p_avg;
    if( num_cells > 0 )	
    {	
	p_avg = p_sum / num_cells;
    	if(p_avg > 2.0)
		p_avg = 2.0;
    }
    p_avg = max(p_avg, p_min);
	     	
    wall_pressure_avg = p_avg; // Set the global variable

	printf("wall_pressure_avg = %f min=%f \n",p_avg, p_min);
    return p_avg;
}
