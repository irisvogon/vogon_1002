...
LOCAL	void	fprint_viscosity_params(FILE*,AVISC*,bool);
LOCAL	void	g_FPrintUSWSSten2d(FILE*,USWSSten2d*);
LOCAL	void	print_max_min_val(FILE*,const char*,float,float*,float,float*,
                                  float,float*,float,float*,int);
LOCAL	void	print_probe_field(float,Locstate,Locstate,Locstate,float,
                                  FILE*,bool,int);
...
EXPORT  void    tecplot_sten_points(
	const char	*bname,
	WSSten		*sten)
{
	int	i;
	...
	sprintf(fname, "%s", bname);
	
	if ((file = fopen(fname,"w")) == NULL)
	{
	    screen("WARNING in tecplot_sten_points(), "
		   "can't open %s\n",fname);
	    return;
	}

	fprintf(file, "TITLE = \"tecplot sten\"\n"
		      "VARIABLES = \"x\", \"y\", \"z\"\n");
	
	fprintf(file, "ZONE T=\"CURVE\" I=%d\n", 2*sten->nsts);
	
	for(i = sten->nsts-1; i >= 0; i--)
	    fprintf(file,"%-9g %-9g %-9g\n", sten->lcrds[i][0],
		       sten->lcrds[i][1], sten->lcrds[i][2]);
	
	for(i = 0; i < sten->nsts; i++)
	    fprintf(file,"%-9g %-9g %-9g\n", sten->rcrds[i][0],
		       sten->rcrds[i][1], sten->rcrds[i][2]);
...
LOCAL	void	print_max_min_val(
	...
	const char *vname,
	float      front_val_max,
	float      *front_val_max_coords,
	float      wave_val_max,
	float      *wave_val_max_coords,
	float      front_val_min,
	float      *front_val_min_coords,
	float      wave_val_min,
	float      *wave_val_min_coords,
	int        dim)
{
	int i;
