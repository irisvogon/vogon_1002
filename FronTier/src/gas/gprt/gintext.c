...
LOCAL   void    print_effective_atwood_num(int *do_spikes, char *b_fn,
					   Grid*, Front*, Wave*, float percent,
					   float bub_height,float spike_height, 
					   float rfactor, const float *origin, 
					   float mix_zone_frac);
LOCAL	void    old_print_vol_frac(Wave *wave, Front *front,
				   float h_min, float h_max,
				   int dir, int index, float rfactor,
				   const float *origin, char *b_fn);


LOCAL   float   find_intersection_by_bisection(Front *front, float *coords_a,
					       float *coords_b, int dir);
LOCAL   float   find_particular_fluid_cell_volume(Front *front, 
						  COMPONENT vol_comp,
						  float *crds, float *dh);
LOCAL	void 	print_vol_frac(Wave *wave, Front *front, 
			       float h_min, float h_max,
			       COMPONENT comp_for_vol_frac, float rfactor,
			       const float *origin, char *b_fn);
LOCAL	void 	new_accumulate_fractions_in_layer(Front *front, float height,
						  float *frac, float rfactor,
						  const float *origin,
						  COMPONENT comp_for_vol_frac);
LOCAL	float 	new_height_at_fraction(Front *front, float h0, float dh,
				       int dir, float fraction, float *frac,
				       COMPONENT vol_comp, float rfactor,
				       const float *origin);

LOCAL  int cmp_for_qsort_dec( const void *vp, const void *vq);
LOCAL  int cmp_for_qsort_inc( const void *vp, const void *vq);
LOCAL  float compute_intfc_area(INTERFACE*);
LOCAL  float compute_tri_area(TRI*);
	...
	*/
	...
	{ 
	    ...
	    */
	    ...
	    FILE       *ALL_STATES;
	    float      frac[2]; /* assumes only 2 gas parameters */
	    float      allstates_height;
	    float      allstates_dh = (g_h_at_max - g_h_at_min)/100.0;
	    char       filename[256];
	    int        start_index;
	...
	bool			do_atwood)
{
	...
	*/
	float		        *heavy_dens, *light_dens;

	/* Remaining type declarations to help compute atwood5 
	   and atwood6 - egeorge */
	long                    *at5_count;
	long                    **vector_global_at5_count;
	float                   *sort_dens_l = NULL;
	float                   *sort_dens_h = NULL; 
	float                   *big_sort_dens_l = NULL; 
	float                   *big_sort_dens_h = NULL;
	long                    ii, jj;
	...
	*/
	if (do_atwood == YES)
	{
	    ...
	    */
	    qsort(big_sort_dens_l, at5_count[1], sizeof(float), 
		  cmp_for_qsort_inc);
	    qsort(big_sort_dens_h, at5_count[0], sizeof(float), 
		  cmp_for_qsort_dec);
					   ...
					   float       mix_zone_frac)
{
	...
	float            dens_minmax[2], atwood_sum;
	float            atwood;
	float            mix_zone_frac_outerbound;
	float            adjusted_bub_or_spike_height;
	float            mean_initial_interface_height;
	...
	float            atwood_sum2, atwood2;
	FILE             *ATWOOD2;
	char             filename2[256];
	Big_State        bst[2];

	float            atwood_sum3, atwood3;
	FILE             *ATWOOD3;
	char             filename3[256];

	float            atwood_sum5, atwood5;
	FILE             *ATWOOD5;
	char             filename5[256];

	float            atwood_sum6, atwood6;
	FILE             *ATWOOD6;
	char             filename6[256];

	const RECT_GRID *rgrid = wave->rect_grid;
	const float     *dh = rgrid->h;
	const int       dim = front->rect_grid->dim;
	int             vert_dir = dim -1;
...
*/
LOCAL   float    find_intersection_by_bisection(
	Front			*front,
	float                   *coords_a,
	float                   *coords_b,
	int                     dir)
{
        float                 ans;
	float                 crds_a[MAXD], crds_b[MAXD], cross_crds[MAXD];
	INTERFACE	      *intfc = front->interf;
						   ...
						   float *dh)
{
        INTERFACE		*intfc = front->interf;
        float                   ans = 0.0;
	float                   previous_coords[MAXD], coords[MAXD];
	...
	*/
	...
	uni_array(&intersection_coord, (int)pow(2,MAXD-1), sizeof(float));
	uni_array(&intersection_length, (int)pow(2,MAXD-1), sizeof(float));
	...
	COMPONENT               comp_for_vol_frac)
{
	...
	float                   dh[MAXD - 1];
	float                   coords[MAXD];
	const float             *L = rgrid->L;
	const float             *U = rgrid->U;
	const float             *GL = rgrid->GL;
	const float             *GU = rgrid->GU;
	float                   light_fluid_volume = 0.0;
	float                   ans;
...
LOCAL float 	new_height_at_fraction(
        Front			*front,
        float			h0,
        float			dh,
        int			dir,
...
{	
        bool                    stop;
	float                   height = h0;
	static float            old_frac, new_frac;
...
*/
EXPORT float get_mean_initial_interface_height( 
					       int version,
					       float *mean_height)
{
        static float ans;
 ...
 **/
LOCAL float compute_intfc_area(INTERFACE *intfc)
{
        SURFACE **surfs;
        ...
        float *L = gr->L;      /* Lower corner of global grid */
        float *U = gr->U;      /* Upper corner of global grid */
        int   i,j,count;
        float p_coord,surf_area = 0.0 ;
...
LOCAL  float compute_tri_area(
	TRI *tri)
{
        const float  *p[3];
        float        tri_area, s[3][3];
        static float n[3];
        int   i;
