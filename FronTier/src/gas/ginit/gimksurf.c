...
LOCAL 	float 	bilin_interpolate(float,float,float,float,float,
				  float,float,float,float,float,
				  float*,float*,int*); 
LOCAL 	float 	file_amplitudes_pert_func(POINTER,float*);
...
*/
LOCAL float file_amplitudes_pert_func(
	POINTER        func_params,
	float          *coords)
{
	RANDOM_PARAMS  *rand_params = (RANDOM_PARAMS*)func_params;
	...
	float          dist;
	float          crds[2];
	float          tolerance = 0.00000000001*min(gr->h[0],gr->h[1]);
	int            i,j,i0,j0;
	int            bdry_type[2] = {0,0};  
