	...
	INIT_DATA	   *init)
{
	...
	if (debugging("check_restart"))
	{
	    bool lbad = is_bad_state(lstate,NO,"g_restart_intfc_initializar");
	    bool rbad = is_bad_state(rstate,NO,"g_restart_intfc_initializar");
	    if (lbad || rbad)
	    {
	...
	reset_gamma(rstate);
        if(g_composition_type() == MULTI_COMP_NON_REACTIVE)
        {
               ...
               comp_type(positive_component(hs))->params->n_comps != 1)
            {
                ...
                /* New 051005 */
                if(fabs(pdens(rstate)[var-(2+dim)]) < 10.0*MACH_EPS && pdens(rstate)[var-(2+dim)] < 0.0)
                    pdens(rstate)[var-(2+dim)] = 0.0;
                /*
                else if(fabs(pdens(rstate)[var-(2+dim)]) > 10.0*MACH_EPS && pdens(rstate)[var-(2+dim)] < 0.0)
                {
                    printf("ERROR in g_restart_set_intfc_states()\n");
                    printf("partial density %20.18g < 0.0\n",pdens(rstate)[var-(2+dim)]);
                    clean_up(ERROR);
                }
                */
                /* End of New 051005 */
