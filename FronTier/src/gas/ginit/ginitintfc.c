	...
	INIT_PHYSICS	*ip)
{
	...
	//DEBUG_TMP null_sides_are_consistent();
	//DEBUG_TMP check_print_intfc("After g_init_interface", "init_rt_af", 'f', 
	//DEBUG_TMP 		front->interf, 1, -1, NO);
...
#define is_right_boundary(p)						\
	...
	for (pp = intfc->points; pp && (*pp); pp++)
	{
		...
		default:
	        ...
	        if (w_type == DIRICHLET_BOUNDARY || w_type == FLOW_BOUNDARY)
...
//  #bjet	    
	    ...
	    {
	        static Locstate  tmpsta = NULL;
		if (tmpsta == NULL)
	 	    alloc_state(intfc,&tmpsta,sizest);
		
	        set_ambient_boundary_state(tmpsta,coords,comp,intfc,init);
		//TMP_TK
		//verbose_print_state("tmpsta", tmpsta);
	    	
		Bstate._boundary_state_function = g_fixed_boundary_state;
		Bstate._boundary_state_function_name =
		    strdup("g_fixed_boundary_state");
		Bstate._boundary_state = tmpsta;
		Bstate._boundary_state_data = NULL;
		Bstate._fprint_boundary_state_data = 
				f_fprint_boundary_state_data;
		index = add_bstate_to_list(&Bstate,intfc,index);
...
leave:
	...
	if(debugging("riinit"))
	{
	    RI_initializer(lstate, Coords(p), ncomp);
	    RI_initializer(rstate, Coords(p), pcomp);
	}
	...
	INIT_DATA	*init)
{
	...
	state = Ambient(comp_type);
	    ...
	    */
		//TMP_TK
                //if(debugging("jetpdens"))
                //{
                    /*
                    if(Dens(tstate) > 0.00038)
                    {
                        pdens(tstate)[0] = Dens(tstate);
                        pdens(tstate)[1] = 0.0;
                    }
                    ...
                    {
                        pdens(tstate)[1] = Dens(tstate);
                        pdens(tstate)[0] = 0.0;
                    }
                    */

                    for(i = 0; i < Params(tstate)->n_comps; i++)
                    {
                        if(Dens(tstate) > 0.00038)
                        {
                             if(i == 4)
                                  pdens(tstate)[i] = Dens(tstate);
                             else
                                  pdens(tstate)[i] = 0.0;
                         }
                         else
                         {
                             if(i == 0)
                                  pdens(tstate)[i] = Dens(tstate);
                              /*else if(i == 8)
                                  pdens(tstate)[i] = 0.8*Dens(tstate);*/
                              else
                                  pdens(tstate)[i] = 0.0;
                         }
                    }
        ...
        Front     *front)
{
	...
	debug_print("init","Entered g_init_parabolic()\n");
        ...
        chart_of_front(front)->set_up_merge_cell = Set_up_merge_cell;
	chart_of_front(front)->update_merge_cell = Update_merge_cell;
	...
	{        
            ...
            if (strncmp(s,"n",1) == 0)
                temp_bool = NO;
            for (i = 0; i < nprms; i++)
                prms_list[i]->avisc.use_stokes_vis=temp_bool;
            temp = 0.0;
            ...
            if (strncmp(s,"y",1) == 0)
                temp_bool = YES;
            for (i = 0; i < nprms; i++)
                prms_list[i]->avisc.turbulence=temp_bool;
            if (strncmp(s,"y",1) == 0)
            {
            ...
            if (strncmp(s,"y",1) == 0)
                temp_bool = YES;
            for (i = 0; i < nprms; i++)
                prms_list[i]->avisc.aver_planar=temp_bool;
            temp_bool = NO;
            ...
            if (strncmp(s,"y",1) == 0)
                temp_bool = YES;
            for (i = 0; i < nprms; i++)
                prms_list[i]->avisc.subgrid_vis=temp_bool;
            temp_bool = NO;
            ...
            if (strncmp(s,"y",1) == 0)
                temp_bool = YES;
            for (i = 0; i < nprms; i++)
                prms_list[i]->avisc.subgrid_md=temp_bool;
            temp_bool = NO;
            ...
            if (strncmp(s,"y",1) == 0)
                temp_bool = YES;
            for (i = 0; i < nprms; i++)
                prms_list[i]->avisc.subgrid_con=temp_bool;
            }
            temp_bool = NO;
            screen("Turn on or turn off the chemistry model(y, n(dflt)): ");
            (void) Gets(s);
            if (strncmp(s, "y",1) == 0)
            {
                    temp_bool = YES;
