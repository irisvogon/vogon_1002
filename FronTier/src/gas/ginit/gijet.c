...
LOCAL  	float   jet_nozzle_func(POINTER,float*);
LOCAL  	float   jet_plane_func(POINTER,float*);
LOCAL  	float   jet_top_bdry_func(POINTER,float*);
LOCAL  	float   jet_bottom_bdry_func(POINTER,float*);
LOCAL  	bool    make_jet_surfaces(RECT_GRID*,COMPONENT,COMPONENT,COMPONENT,
                        float (*func1)(POINTER,float*),POINTER,
                        float (*func2)(POINTER,float*),POINTER,
			float (*func3)(POINTER,float*),POINTER,
		        float (*func4)(POINTER,float*),POINTER,
...
LOCAL   bool 	exist_func_root(float,float);
LOCAL   int     install_grid_crx2(float (*func)(POINTER,float*),POINTER,
                        EG_CRX*,RECT_GRID,COMPONENT,COMPONENT,int);

LOCAL   int     install_grid_topbdry_crx(float (*func)(POINTER,float*),POINTER,
		        EG_CRX*,RECT_GRID,COMPONENT,COMPONENT,int);
LOCAL   int     install_grid_bottombdry_crx(float (*func)(POINTER,float*),POINTER,
                        EG_CRX*,RECT_GRID,COMPONENT,COMPONENT,int);
LOCAL   int     install_curve_crx(float (*func_1)(POINTER,float*),POINTER,
                        float (*func_2)(POINTER,float*),POINTER,EG_CRX*,
			RECT_GRID,int*,POINT***,COMPONENT,COMPONENT,COMPONENT,
			int**,int*);
LOCAL   int 	face_crx_in_dir(float (*func1)(POINTER,float*),POINTER, 
			float (*func2)(POINTER,float*),POINTER,float***,float*,int);
LOCAL	float	return_sign(float f);
LOCAL	bool	find_func_root(float (*func)(POINTER,float*),
                        POINTER,float*,float*,float*);
LOCAL   float 	partial_func_derivative(float(*func)(POINTER,float*),
                        POINTER,float*,int);
LOCAL   float   partial_func_derivative2(float(*func)(POINTER,float*),
...
LOCAL   float jet_nozzle_func(
        POINTER func_params,
        float *coords)
{
        JET_PARAMS *params;
        float *cen,r1,r2,h1,h2,ratio,arg;
...
LOCAL   float jet_plane_func(
        POINTER func_params,
        float *coords)
{
        JET_PARAMS *params;
        float *cen,r1,r2,h1,ratio,arg;
...
LOCAL   float jet_bottom_bdry_func(
        POINTER func_params,
        float *coords)
{
        BDRY_BOX_PARAMS *params;
...
EXPORT bool make_jet_surfaces(
		...
		float       (*func3)(POINTER,float*),
		POINTER     func3_params,
		float       (*func4)(POINTER,float*),
...
LOCAL   int install_curve_crx(
	...
	int    curve_n[])
{
	float      ***face_coords, face_crds_crx[3];
	float      *L = grid.L;
	float      *h = grid.h;
	int        *gmax = grid.gmax;
	int        i,j,k;
	...
	COMPONENT  ***comp = eg_crx->comp;
	//TMP
        float      *cen,r2,h1,h2,h3,ratio,crds[2];
        JET_PARAMS *params;
        int        num_crx_curve1;
	...
	tri_array(&face_coords,3,2,2,sizeof(float));
...
{
	int i,j,k,num_iter;
	float c00[3],c01[3],c10[3],c11[3],a1[3],b1[3];
	float Jacobian[2][2],DetJ,InvJ[2][2];
	float f0,g0,func_f[2][2],func_g[2][2];

	float epsilon_1 = 1.0e-10;
	float coeff = 0.2;
	num_iter = 100;
	...
	//TMP
	{
	    float funcf,funcg,*cen,r1,r2,r3,h1,h2,h3,ratio;
...
LOCAL	float partial_func_derivative(
	float (*func)(POINTER,float*),
	POINTER func_params,
	float *coords,
	int dir)
{
	...
	else if (func == jet_plane_func)
	{
        ...
        {
            JET_PARAMS *params;
            float *cen,r0,r1,r2,h1,h2,h3,ratio;
            params = (JET_PARAMS *)func_params;
            cen = params->cen;
            r1 = params->r1;
            r2 = params->r2;
...
LOCAL   float partial_func_derivative2(
        float (*func)(POINTER,float*),
        POINTER func_params,
        float *coords,
        int dir,
        ...
        BOND *b)
{
        ...
        if (intfc == NULL)
            return;
        return (*i_user_interface(intfc)._reverse_bond)(b);
