...
struct _LAYER_FLAG {
	...
	bool _ALL_CONTACT;
	bool _HAS_ELLIPSOID;
	bool _ELLIPTICAL_REGION;
	bool _RT_LINEAR_THEORY;
	bool _INNERMOST_ELLIPSOID;
	bool _ALLOW_SECTORED_ELLIPSOIDS;
};
...
typedef	struct	{ float	nu,epsilon; }	SIN_SQR_PERT_PARAMS;
...
typedef struct {
	float	      center_of_mass[3];/* Center of mass for rigid body */
	float	      total_mass;	/* Total mass of rigid body */
	float	      mom_of_inertial;	/* Moment of inertial of rigid body */
} RIGID_BODY_PARAMS;
...
struct _ELLIPSOID {
	float 	      cen[3];		/* Center of Ellipsoid */
	float 	      rad[3];		/* Lengths of radii */
...
typedef	struct {
	int num_of_steps;
	float reaction_zone_width,x_intfc,x_bndry,wv_speed;
	float progress_data[300],step;
...
typedef struct {
	float		*wv_num, phase, amp_max;
	float		sigma_r, sigma_i;
	float		ksq, **a, **b;
} NORMAL_MODE;

struct	_RI_PERT {
	float	a, kx, amp, U, R, cen[3];
	float	beta, sigma;
	float	dens1, dens2, press;
};

typedef struct _RI_PERT	RI_PERT;
...
struct _G_INIT_DATA {
	...
	bool	_reflect_small_loop_shocks;
