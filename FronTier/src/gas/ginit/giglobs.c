...
LOCAL	void	read_gravity_data(FILE*,int,float**,int);
#if defined(ONED)
LOCAL	void	set_oned_state_from_interface(float*,Locstate,COMP_TYPE*,
					      ONED_OVERLAY*);
#endif /* defined(ONED) */
	...
	int	dim)
{
	...
	switch (composition_type)
	{
	    case PURE_NON_REACTIVE:
	    	...
	    	*nfloats = dim + 2;
            ...
            case MULTI_COMP_NON_REACTIVE:
                *sizest = sizeof(MGas) + ((*nfloats)-1)*sizeof(float);
	    ...
	    case TWO_CONSTITUENT_REACTIVE:
	    	*sizest = sizeof(MGas) + sizeof(float);
	    	*nfloats = dim + 4;
	    	break;
	    case THINFLAME:
                *sizest = sizeof(Gas);
...
#else /* defined(float) */
	static const char *fmts[] = {"","%f %f","%f %f %f","%f %f %f"};
#endif /* defined(float) */
...
#if defined(ONED)
EXPORT	bool	overlays_exist(void)
{
	int i;
	for (i = 0; i <= max_n_comps; ++i)
	{
...
double 	get_blasius_vel_coefficient_nozzle(
	double *coords,
	double wall)
{
	...
	double fixed_bl_thickness[2] = {3.0, 0.035}; // Set bl thickness to y at x
	double U = 10.0; // magnitude of flow velocity 
	//double nu = 17.4/0.000085; // kinematic viscosity = dynamic viscosity/rho
	double n = 5.0; // number of deltas to make the bdry layer
	double plate_start=5.0;

	// calculate nu based on the fixed thickness and plate start
	double nu = sqr(fixed_bl_thickness[1]/(double)n)*U/(plate_start-fixed_bl_thickness[0]);
//	printf("nu = %f\n",nu);

	// calculate delta based on nu, coords, etc.
	double delta = sqrt(nu*(plate_start-coords[2])/U);
	// eta = y/delta
	double eta = (wall-sqrt(sqr(coords[0])+sqr(coords[1])))/delta;
...
//	printf("for coords %f %f %f: eta=%f: delta=%f: thickness=%f\n", coords[0], coords[1], coords[2], eta, delta,n*delta);
	...
	return blasius_f(eta);
}

double 	get_blasius_vel_coefficient(
	double *coords,
	double wall)
{
	// Determine if the point is in the boundary layer. If so, return
