...
LOCAL	void	get_state_random_region(float*,Locstate,COMP_TYPE*,
	        			HYPER_SURF*,INTERFACE*,INIT_DATA*,int);
LOCAL	void	gram_schmidt(float**,int);
...
#else /* defined(float) */
	static const char *scan_3_floats = "%f %f %f";
#endif /* defined(float) */
	...
	RECT_GRID    *rgr)
{
	...
	float	**A = rstate->A;
	float	D[3];
	float	lambda_min = HUGE_VAL;
	float	*L = grid->L, *U = grid->U;
	float	dx;
