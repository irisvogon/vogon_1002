...
typedef struct {
        bool    bubble;
        bool    eos;
        float   R; /*radius of the bubble region */
        float   r; /*bubble radius*/
        float   frac;/*void fraction*/
        float   L; /*length of the bubble region*/
        float   l; /*length between centers of bubbles*/
        int     size;  /* bubble diameter in mesh size */
        int     N_layers;
        int     N_bubbles;
        float   rand_coef;
        Locstate st; /* initial state for bubbles */
        float   rho; /*density inside bubbles*/
        float   pr; /*pressure inside bubbles*/
        int     eos_n; /*number eos to use*/
        COMPONENT  bubblecomp;
        float   pr_crit; /*critical pressure for making bubbles*/
        float   GAMMA; /*bubbles (gamma - 1)*/
}  BUBBLES;

typedef struct {
        int    icoords[MAXD];  /* icrds of checked cells */
        float  center[MAXD];   /* bubble center */
        int    bmin[MAXD];     /* lower corner indx of blks the bubble ocupy */
        int    bmax[MAXD];     /* upper corner indx of blks the bubble ocupy */
        int    insert;
        float  rad_distr;      /* Mass ditribution radius */
        ...
        float  vapor_press;    /* vapor pressure from averaged states */
...
LOCAL   float   pi = 3.14159265358979323846;
LOCAL   float   bubble_spacing = 1.0;
...
LOCAL 	bool    check_crossings_in_region(int*,int,Wave*,Front*);
LOCAL   bool    check_crossings_in_region_ver2(int*,int,Wave*,Front*);
LOCAL	bool    check_near_by(int*,COMPONENT,int,int,Wave*,Front*,Locstate);
...
LOCAL   int     check_in_bubble_diameter_3d(INSERT_SITE*,Wave*,Front*,
                     COMPONENT,int*,int*,int*,int*);
LOCAL   void    blks_bubble_ocupy(INSERT_SITE,RECT_GRID*,int*,int*,float);
LOCAL   int     check_crx_in_bubble_space_2d(INSERT_SITE,Wave*,Front*,int*,int*,float);
LOCAL   int     check_crx_in_bubble_space_3d(INSERT_SITE,Wave*,Front*,int*,int*,float);
LOCAL   int     intfc_on_rect_domain_2d(INTERFACE*,float*,float*);
LOCAL   int     intfc_on_rect_domain_3d(INTERFACE*,float*,float*);
LOCAL   int     intfc_cross_rect(INTERFACE*,float*,float*);
LOCAL   int     intfc_cross_box(INTERFACE*,float*,float*);
LOCAL   int     tri_intersect_surf(TRI*,int,float,float,float,float,float);
...
LOCAL   int     intfc_intersect_segment(INTERFACE*,int,float,float,float);
LOCAL   int     intfc_intersect_surf(INTERFACE*,int,float,float,float,float,float);
LOCAL   void    set_state_on_phase_boundary(Locstate,int);
LOCAL   void    set_state_on_phase_boundary_ver2(Locstate,Locstate,float);
LOCAL   void    set_state_on_phase_boundary_ver3(Locstate,Locstate,float);
...
LOCAL   int     make_bubble_conserv_mass(INSERT_SITE*,int,Wave**,Front**,int,Wv_on_pc**,
                     int,float,COMPONENT,int***,int,Locstate,int,INSERT_SITE*);
        ...
        COMPONENT               bubblecomp)
{
        ...
        int                     ix,iy, i;
        float                   *GL = front->rect_grid->GL;
        float                   *GU = front->rect_grid->GU;
        float                   *L = front->rect_grid->L;
        float                   *U = front->rect_grid->U;
        float                   coords[MAXD];
...
{
        int   stype;
        float ratio = 2*spacing+1.;
        float alpha = ratio * ratio * ratio;
        float rho_i, e_i;
        float rho_v, e_v, e_l, beta;
        float v[2], v2;
        int   i;
        ...
        Locstate                rst)
{
        ...
        float    CG[MAXD], CL[MAXD], c[MAXD], coords[MAXD];
...
LOCAL   bool     check_crossings_in_region(
        int         *icoords,
        int         size,
        Wave        *wave,
        Front       *front)
...
/* If the crossings are found in the region, return YES */
LOCAL   bool     check_crossings_in_region_ver2(
        int         *icoords,
        int         size,
        Wave        *wave,
        Front       *front)
        ...
        COMPONENT               bubblecomp)
{
        ...
        float                   period = 5.e-6;
        Locstate                state;
        float                   T_sat, P_sat, press, ref_temp;
	...
	INSERT_SITE      *insert_site)
{
        ...
        float        D_add, ref_temp, all_mass_add, alpha = 1.0;
      ...
      int          **distr_site)
{
      ...
      Locstate     st;
      float        D, T, T_sat, P_sat, pres, d_sat, c_sqr, vapor_press, p_jump;
      float        *crds, area, alpha = 1.0, d_dif, s_alpha = 0.0;
      float        avail, d_incr, m0_incr, m1_incr, e_incr;
      float        in_vel0, in_vel1, n_vel0, n_vel1, vel0, vel1, D_add;
      int          num_cell = 0, count = 0, all_cell, more_count = 0;
      float        total_add_mass = 0.0;
        ...
        COMPONENT               bubblecomp)
{
        ...
        int                     n_site = 0, alloc_site = 0;
        int                     num_cells, **distr_site;
        float                   alpha = 1.0,D_add;
	stataic	float		templ,temph;
      ...
      int          **distr_site)
{
      ...
      float        fmin[MAXD], fmax[MAXD];
      float        *crds;
      Locstate     st;
      float        temp, pres, P_sat, vapor_press, p_jump, D_add, c_sqr;
      float        vel0, vel1, in_vel0, in_vel1, n_vel0, n_vel1;
      CURVE        **cc;
      BOND         *b;
      POINT        *pt;
      RECT_GRID    *gr = wave->rect_grid;
      ...
      float        *D_add)
{
      ...
      float        *crds, area, alpha = 1.0, d_dif, s_alpha = 0.0;
      Locstate     st;
      float        temp, pres, P_sat, vapor_press, p_jump, c_sqr;
      ...
      int          *inmax)
{
      ...
      float        center[MAXD], *crds;
      Locstate     state;
      float        T_sat, P_sat, press;
...
LOCAL  int    intfc_intersect_surf(
        ...
        int             dir,           /* direction of cut line normal */
        float           cut,           /* coordinate of cut line */
        float           L1,            /* lower coordinate of cut line in other direction */
        float           U1,
        float           L2,            /* upper coordinate of cut line in other direction */
        float           U2)            
{
        POINT             *p;
        SURFACE           **s;
