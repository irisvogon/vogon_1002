...
LOCAL	bool	prompt_for_artificial_parameter(float*,float,int,const char*,
...
LOCAL   float   timmes_woosley_flame_velocity(Locstate);
LOCAL   float   experimental_flame_velocity(Locstate);
LOCAL   float   dynamic_flame_velocity(Locstate);
...
LOCAL	bool prompt_for_artificial_parameter(
	float		*prm,
	float		nzdflt,
	int		restart,
	const char	*pname,
...
LOCAL	void prompt_for_muscl_artificial_parameters(
	...
	const char	*mesg2)
{
	static const float DEFAULT_MIN_SHOCK_JUMP   = 0.25;   /*TOLERANCE*/
	static const float DEFAULT_MIN_SP_VOL_JUMP  = 1.0e-6; /*TOLERANCE*/
	static const float DEFAULT_CHAR_SPD_CUTOFF  = 0.0;    /*TOLERANCE*/
	float	    min_shock_jump = DEFAULT_MIN_SHOCK_JUMP;
	float	    min_sp_vol_jump = DEFAULT_MIN_SP_VOL_JUMP;
	float	    char_speed_cutoff = DEFAULT_CHAR_SPD_CUTOFF;
	float       eta;
	char	    s[Gets_BUF_SIZE];
...
LOCAL  float   timmes_woosley_flame_velocity(
       Locstate state)
{
	float Xc = 0.5;			// Molar fraction of carbon
	float d = density(state);	// Density of the unburned fuel

	/* Ensure that it is cm/s */
	//float v = 92e5*pow(d/(2e9), 0.805)*pow(Xc/.5, 0.889);
	float v = 0.092*pow(d/2, 0.805)*pow(Xc*2, 0.889);
	/* Ensure that it is cm/s */

	return v;
}	/* end timmes_woosley_flame_velocity */

LOCAL float experimental_flame_velocity(
      Locstate state)
{
	float v;
	float p = pressure(state);
	float d = density(state);
...
LOCAL float dynamic_flame_velocity(
      Locstate state)
{
	float v;
	float gr  = adiabatic_gamma(state);
	float mu2 = (gr-1)/(gr+1);
	float p   = pressure(state);
	float d   = density(state);
	float T   = p/d;
	float c   = sound_speed(state);
	float q   = Heat_release(state);

	float a = 1 + 4*q*mu2/(T*(1-mu2*mu2));
