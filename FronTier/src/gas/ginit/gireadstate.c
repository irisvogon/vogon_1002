...
LOCAL	bool	new_read_print_avisc_field(const char*,const char*,
					   float*,const IO_TYPE*);
	...
	INTERFACE     *intfc)
{
	...
	{
            //RK_PRINT("getting bstate for hs %d: %p\n",i,hs);
...
LOCAL	bool new_read_print_avisc_field(
	const char    *s1,
	const char    *s2,
	float	      *coef,
	const IO_TYPE *io_type)
