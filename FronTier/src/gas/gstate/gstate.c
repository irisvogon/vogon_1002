...
LOCAL	float	compute_max(float,float*,float,float*,int);
LOCAL	float	compute_min(float,float*,float,float*,int);
LOCAL	void	initialize_extreme_values(EXTREME_VALUES*);
LOCAL	void	update_extreme_values(EXTREME_VALUES*,Locstate,float*,int);
#if defined(TWOD)
LOCAL	void 	g_check_front_state_consistency2d(Front*);
#endif /* defined(TWOD) */
#if defined(COMBUSTION_CODE)
LOCAL	float	prod(Locstate);
...
LOCAL	bool	open_fg = NO;

void	set_open_bdry_flag(bool);
void	set_open_bdry_flag(bool fg)
{
	open_fg = fg;
}
...
EXPORT void g_reflect_state(
	...
	float		*nor)	/* unit normal to reflection plane */
{
	float	v[MAXD], vdn;
	int	i;
	int	dim = intfc->dim;
	...
	if(debugging("gfmref"))
	{
	//in this case, Vel(state) is the normal of the point
	    vdn = 2.0*scalar_product(Vel(state),nor,dim);
	    for (i = 0; i < dim; ++i)
		Vel(state)[i] -= vdn*nor[i];
	    return;
	}
...
EXPORT void g_reflect_and_stratify_state(
	...
	INTERFACE	*intfc, /* interface being reflected */
	float		*pt,	/* position of state being reflected */
	float		*p,	/* arbitrary point on reflection plane */
	float		*nor)	/* unit normal to reflection plane */
{
	CHART	*chart = current_chart();
	int	i, dim = intfc->dim;
	float	gn;		/* normal component of gravity */
	float	ds;		/* distance from pt to reflection plane */
	float	dp[MAXD];
	float	pr[MAXD];	/* Coordinates of reflected point */
	float	v[MAXD];
	float   gpt[MAXD], gpr[MAXD], gbar[MAXD];
	float	time = chart->grid->time;
	...
	float			*vdir)
{
	...
	if (len > 0.0)
	{
	    float sp = scalar_product(vdir,nor,dim);
	    for (i = 0; i < dim; ++i)
...
EXPORT	float	RadialComponentOfVelocity(
	Locstate	state,
	float		*coords,
	int		dim)
{
...
*/
EXPORT	float	TangCmptOfXYVelocity(
	Locstate	state,
	float		*coords,
	int		dim)
{
...
EXPORT	float mach_number_squared(
	Locstate	state,
	float		*abs_v,
	float		*rel_v)
{
...
EXPORT	float mach_number(
	Locstate	 state,
	float		*abs_v)
{
	return sqrt(mach_number_squared(state,abs_v,(float *)NULL));
}		/*end mach_number*/
	...
	Locstate	st2)
{
	...
	if (fabs(T1-T2)>MACH_EPS ){
		printf("WARNING: States not equal!!\n");
		printf("  State type = %d\n", state_type(st1));
		printf("  T = %f\n",T1);
		printf("  IE = %E\n",internal_energy(st1));
		printf("  P = %E\n",pressure(st1) );
		printf("  gamma = %E\n", MGamma(st1) );
		printf("  density = %f\n",Dens(st1));
		printf("  momentum = ( %f, %f, %f)\n", Mom(st1)[0], Mom(st1)[1], Mom(st1)[2]);
		int i;
		for (i = 0; i<10; i++)
			printf("  pdens(st1)[%d] = %E\n",i,pdens(st1)[i]);
		printf("\n");
		printf("  State type = %d\n", state_type(st2));
		printf("  T = %f\n",T2);
		printf("  IE = %E\n",internal_energy(st2));
		...
		printf("  gamma = %E\n", MGamma(st2) );
		printf("  density = %f\n",Dens(st2));
		printf("  momentum = ( %f, %f, %f)\n", Mom(st2)[0], Mom(st2)[1], Mom(st2)[2]);
		for (i = 0; i<10; i++)
			printf("  pdens(st2)[%d] = %E\n",i,pdens(st2)[i]);
		printf("\n");
		printf("\n");
	}*/
	...
	INTERFACE	*intfc)
{
	...
	float		*L = gr->L, *h = gr->h, coords[3];

	//DEBUG_TMP printf("#g_check_params_comp\n");
...
LOCAL	float	compute_max(
	float val,
	float *coords,
	float current_max_val,
	float *current_max_val_coords,
	int   dim)
{
	int i;
...
LOCAL	float	compute_min(
	float val,
	float *coords,
	float current_min_val,
	float *current_min_val_coords,
	int   dim)
{
	int i;
