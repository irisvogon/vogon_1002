...
#endif /* defined(TWOD) */
LOCAL  void 	least_sqr_fit_old(float*,float*,float*,int,int);
	...
	Locstate	ans)
{
	...
	if(debugging("enp_int"))
	{
	    float	enp;

	    set_type_of_state(ans,TGAS_STATE);
	    for(i=0; i<dim; i++)
		Vel(ans)[i] = alpha*vel(i,s1) + beta*vel(i,s2);
	    enp = alpha*entropy(s1) + beta*entropy(s2);
	    Press(ans) = pressure_from_entropy(ans, enp);
	...
	Locstate	ans)
{
	...
	if(debugging("enp_int"))
	{
	    ...
	    set_type_of_state(ans,TGAS_STATE);
	    for(i=0; i<dim; i++)
		Vel(ans)[i] = f0*vel(i,s0) + f1*vel(i,s1) + f2*vel(i,s2);
	    enp = f0*entropy(s0) + f1*entropy(s1) + f2*entropy(s2);
	    Press(ans) = pressure_from_entropy(ans, enp);
	    set_state(ans, GAS_STATE, ans);
	}
	else
