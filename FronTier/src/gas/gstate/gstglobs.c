...
LOCAL const float *generalized_astrophysical_gravity(const float*,GRAVITY*);
LOCAL bool eval_partial_mass1D(float*,int,int);
LOCAL bool eval_partial_mass2D(float*,int,int);
LOCAL bool eval_partial_mass3D(float*,int,int);
#if defined(HYPRE)
LOCAL void solve_gravity();
LOCAL void interpolate_gravity(const float*,float*,float*,int);
#endif /* defined(HYPRE) */
...
EXPORT	float astrophys_grav_constant()
{
...
LOCAL const float *generalized_astrophysical_gravity(
	const float *coords,
	GRAVITY     *grav_data)
{
	float	*g = grav_data->g;
	...
	GEOMETRY_REMAP remap = gr->Remap.remap;
        float   sign[MAXD], tmpcoords[MAXD];
	...
	if(debugging("2d_gravity"))
	{
        ...
        interpolate_gravity((const float *)tmpcoords,g,h,dim);
...
float		ratio = 2.0;//use coarser grid to calculate gravity

LOCAL void solve_gravity()
{
	GRAVITY		*gd = Fparams.grav_data;
	float		***g_of_grid = gd->g_of_grid;
	static	float	**den;
	SN_ELLIP        ellip;
		...
		int	dim)
{
	...
	float		*L = gd->roots[0]->wave->rect_grid->L;
        float           ***g_of_grid = gd->g_of_grid;
	float		**fx = g_of_grid[0],**fy = g_of_grid[1];		
	float 		p01,p23;
	float 		pcoords[4][MAXD];
	float 		pg[4][MAXD];
	float 		x[4*3],b[3],y[4];
	int 		i,j,n=4,p=3;
...
EXPORT bool eval_partial_mass(float* Mp,
			      int N,
			      int mass_type)
{
	bool 		ret;
...
LOCAL bool eval_partial_mass1D(float* Mp,
			       int N,
			       int mass_type)
{
	int 		i, j;
	...
	static float	*pRecvBuff = NULL;
	Wave		*wave = Fparams.grav_data->roots[0]->wave;
	int		*gmax = Fparams.grav_data->gmax;
	int		*size = (nn>1)?wave->pp_grid->Zoom_grid.gmax:gmax;
	float		*cen = Fparams.grav_data->center;
	float		*GL = Fparams.grav_data->GL;
	float		*GU = Fparams.grav_data->GU;
...
/* flame */
LOCAL bool eval_partial_mass2D(float* Mp,
			       int N,
			       int mass_type)
{
	int 		i, j, k, n, nsec;
...
LOCAL bool eval_partial_mass3D(float* Mp,
                               int N,
                               int mass_type)
{
