...
LOCAL	void	contact_riemann(Locstate,Locstate,Locstate,Locstate,
	                     ...
	                     WSSten*,float*,NptWSpeedOpts*,float*,float,float);
LOCAL   void    contact_riemann_1(Locstate,Locstate,Locstate,Locstate,
                             Locstate,float,Locstate,float,Locstate,Locstate,
                             WSSten*,float*,NptWSpeedOpts*,float*,float*,float*);
LOCAL	void	g_Left_w_speed(const char*,Locstate,Locstate,float*,int);
LOCAL	void	midstate(Locstate,Locstate,float,float,float,int,
...
LOCAL	void	CJ_state_behind_curved_wave(Locstate,Locstate,float,float*,
	                                    int,int);
LOCAL   void flame_moc_plus_rh(float*,Locstate,Locstate,Locstate,
                                    Locstate,Locstate,Locstate,Locstate,
                                    float,float,float,float,float,float*,
                                    float*,float,float,float*,float,float,
                                    Front*);
LOCAL  bool flame_moc_plus_rh_signed(Locstate,Locstate,Locstate,
				    Locstate,Locstate,bool,
				    float,float,float,float*,
				    float*,float*,int);
LOCAL  float flame_moc_plus_rh_guessp(Locstate,Locstate,Locstate,
				    Locstate,Locstate,float,
				    float,float,float,float*,
				    float*,float*,int);
LOCAL  float flame_moc_plus_rh_deltap(Locstate,Locstate,Locstate,
				    Locstate,Locstate,float,
				    float,float,float,float*,
				    float*,float*,int);
LOCAL  float flame_moc_plus_rh_findstates(float,float,
	...
	float		*W)
{
	...
	float           dnl, dnr;
	float           difful,diffur,dterm; /*diffusion term*/
	static float	g[MAXD];
	...
	static Locstate	st_l1 = NULL, st_l2 = NULL, st_l3 = NULL,
        ...
        static float 	*difful_fr, *diffur_fr;
#if defined(COMBUSTION_CODE)
	static Locstate CJ = NULL;
#endif /* defined(COMBUSTION_CODE) */
	...
	if (w_type < FIRST_PHYSICS_WAVE_TYPE)
	{
	    ...
	    bdry_npt_w_speed(sten,ansl,ansr,&Opts);
            //RK_PRINT("@@@@@@@@@@@@@@@@@@@@ 1\n");
	    if (w_type == MOVABLE_NEUMANN_BOUNDARY)
	    {
                //RK_PRINT("@@@@@@@@@@@@@@@@@@@@ 1\n");
	    	...
	    	for (i = 0; i < dim; ++i)
		{
		    ...
		    rr[i] = Coords(sten->p)[i] - center_of_mass(hs)[i];
		}
	    	W[0] -= angular_velo(hs)*rr[1];
	    	W[1] += angular_velo(hs)*rr[0];
	    }
	    else
	    {
                //RK_PRINT("@@@@@@@@@@@@@@@@@@@@ 1\n");
	...
	{
            //RK_PRINT("@@@@@@@@@@@@@@@@@@@@ 1\n");
            int Nu = Params(sr)->n_comps;
	    uni_array(&difful_fr,Nu,FLOAT);
	    uni_array(&diffur_fr,Nu,FLOAT);
	}
        //RK_PRINT("@@@@@@@@@@@@@@@@@@@@ 1\n");
        if(g_composition_type() == MULTI_COMP_NON_REACTIVE)
        {
            //RK_PRINT("@@@@@@@@@@@@@@@@@@@@ 1\n");
            int       i;
            if(Params(sr)->n_comps != 1 && w_type == CONTACT)
            {
                for(i = 0; i < Params(sr)->n_comps; i++)
                {
                    int  N = Params(sr)->n_comps;
                    float  fll[N], fl[N], fr[N], frr[N];
                    float  l1, l2[N], l3[N], r1, r2[N],r3[N];

                    fll[i] = pdens(sll)[i]/Dens(sll);
                    fl[i] = pdens(sl)[i]/Dens(sl);
                    fr[i] = pdens(sr)[i]/Dens(sr);
                    frr[i] = pdens(srr)[i]/Dens(srr);

                    l1 = (Dens(srr) - Dens(sll))/(2*dn);
                    l2[i] = ((frr[i] - fll[i])/(2*dn));
                    l3[i] = ((frr[i] - 2*fl[i] + fll[i])/(dn*dn));

                    r1 = (Dens(srr) - Dens(sll))/(2*dn);
                    r2[i] = ((frr[i] - fll[i])/(2*dn));
                    r3[i] = ((frr[i] - 2*fr[i] + fll[i])/(dn*dn));

                   difful_fr[i] = dterm*(l1*l2[i] + Dens(sl)*l3[i]);
                   diffur_fr[i] = dterm*(r1*r2[i] + Dens(sr)*r3[i]);
               }
               //RK_PRINT("@@@@@@@@@@@@@@@@@@@@ 2\n");
            }
	    else
	    ...
	    else
	    {
	        ...
	        /* Use centered difference in time for wave speed */
	            ...
	            for (i = 0; i < dim; ++i)
                    ...
                    if(g_composition_type() == MULTI_COMP_NON_REACTIVE)
                    {
	                  include_source_1(pt,ansl,ul,dt,nor,W,g,gr,difful_fr,w_type);
	                  include_source_1(pt,ansr,uml,dt,nor,W,g,gr,diffur_fr,w_type);
                    }
                    else
                    {
...
//TM - Try W[i] = nor[i]*((cl/(cl+cr))*uml + (cr/(cl+cr))*umr);
	    ...
	    case RIEMANN:
                if(g_composition_type() == MULTI_COMP_NON_REACTIVE)
                     contact_riemann_1(left,Tsl,right,Tsr,st_l3,dnl,st_r1,dnr,ansl,
                                ansr,sten,W,&Opts,g,difful_fr,diffur_fr);
                else
	    ...
	    else
	    {
		    ...
		    reset_gamma(st_l3);
	            ...
	            for ( i= 0; i < dim; ++i)
                    ...
                    if(g_composition_type() == MULTI_COMP_NON_REACTIVE)
                    {
                        include_source_1(pt,ansl,umr,dt,nor,W,g,gr,difful_fr,w_type);
                        include_source_1(pt,ansr,ur,dt,nor,W,g,gr,diffur_fr,w_type);
                    }
                    else
                    {
	...
	NptWSpeedOpts   *opts)
{
	...
	if (debug_w_speed == YES)
        ...
        //RK_PRINT("wwwwwwwwwwwwwwwwwwwwwwwwwwwwwwww\n");
	if (Tsr == NULL) 
	{
            //RK_PRINT("wwwwwwwwwwwwwwwwwwwwwwwwwwwwwwww\n");
	...
	case FLOW_BOUNDARY:
	    ...
	    //  ((positive_component(hs) == 2 && negative_component(hs) == 0) ||
	    //   (positive_component(hs) == 0 && negative_component(hs) == 2)))
	   
	    //TMP_TK	 
	    //if(debugging("const_bdry") && 
	      if(((positive_component(hs) == 3 && negative_component(hs) == 0) ||
	       (positive_component(hs) == 0 && negative_component(hs) == 3)))
	    {
                //RK_PRINT("wwwwwwwwwwwwwwwwwwwwwwwwwwwwwwww\n");
		if (!is_obstacle_state(sl))
		{
                        //RK_PRINT("wwwwwwwwwwwwwwwwwwwwwwwwwwwwwwww\n");
			ft_assign(ansl, sl, sizest);
		        get_constant_state(ansl, 2, Coords(sten->p[0]), front->time);
	    	        //print_general_vector("p0l ", Coords(sten->p[0]), 3, "\n");
	        }
                //RK_PRINT("wwwwwwwwwwwwwwwwwwwwwwwwwwwwwwww\n");
	        if (!is_obstacle_state(sr))
	        {
                        //RK_PRINT("wwwwwwwwwwwwwwwwwwwwwwwwwwwwwwww\n");
			ft_assign(ansr, sr, sizest);
		        get_constant_state(ansr, 2, Coords(sten->p[0]), front->time);
	    	        //print_general_vector("p0r ", Coords(sten->p[0]), 3, "\n");
	        }
		...
		for (j = 0; j <= nrad; ++j)
                {
                    //RK_PRINT("wwwwwwwwwwwwwwwwwwwwwwwwwwwwwwww j=%d\n",j);
	...
	case MOVABLE_NEUMANN_BOUNDARY:
		...
		if(the_pt(pt))
		{
		    printf("#pt found1 ur=%15.8e cr=%15.8e dt=%15.8e\n", ur, cr, dt);
		    verbose_print_state("st_r1", st_r1);
		    verbose_print_state("Tsr", st_r1);
		    print_WSStenData(wssten);
		}
#if defined(DEBUG_W_SPEED)
	        if (debug_w_speed == YES)
	        {
		...
		{
		    printf("#pt found\n");
		    verbose_print_state("ansr_1", ansr_1);
		    verbose_print_state("ansr_2", ansr_2);
		}
        ...
        if ((is_grav) && (g != NULL))
        {
            if (!diffu_term)
            {
             ...
             else
             {
                //Dens(ans) += diffu*Dens(ans)*dt;
                g_factor = scalar_product(nor,g,dim)*dt;
                diffus = max(diffu[0],0.0);
                for (i = 0; i < dim; ++i)
                    Vel(ans)[i] = exp(-diffus*dt)*Vel(ans)[i] +
                                       nor[i]*g_factor;
             }
        }
        ...
        float           *diffur)
{
        ...
        include_source_1(pt,ansl,vnor0,dt,nor,W,grav,front->rect_grid,
                        difful,w_type);
        include_source_1(pt,ansr,vnor0,dt,nor,W,grav,front->rect_grid,
                        diffur,w_type);
}               /*end contact_riemann_1*/
...
{
        RECT_GRID *gr = front->rect_grid;
        float     u1, u2, u3, usl, usr, ull, urr;
	float     c1, c3;
	float 	  g1dt,g2dt,g3dt;
	...
	float     rl, rr;
	float     pl, pr;
	float     s, LHS, gam, V[MAXD];
	float     newpl,newpr;
	unsigned int iter=0;
	unsigned int maxiter=100;
			
	float rmin, radl, radr2, radr3;
	float ulfac = 0.0;
	float urfac = 0.0;
	float vflame;
	...
	/* Debugging variables */
	float mu2,heat,tmp1,tmp2,w1,w2,s1,M,M1,M2,M3;
	FILE *fpt;
	/* Debugging variables */
...
LOCAL bool flame_moc_plus_rh_signed(
	Locstate	st1,
	Locstate	st2,
	Locstate	st3,
	Locstate	ansl,
	...
	int	        dim)
{
        float pneg, ppos, delta;
        float gneg, gpos;
	float beta;
	float alpha = Press(st2);
	float pl,pr,rhol,rhor;
...
LOCAL float flame_moc_plus_rh_guessp(
	Locstate	st1,
	Locstate	st2,
	Locstate	st3,
	Locstate	ansl,
	...
	int		dim)
{
	float pl,pr,rl,rr,vl,vr,g;
	float u1, u2, u3;
	int i;	
	float vflame;
	float gam = adiabatic_gamma(st1);
	float mu2 = (gam - 1.)/(gam + 1.);
...
LOCAL float flame_moc_plus_rh_deltap(
	Locstate	st1,
	Locstate	st2,
	Locstate	st3,
	Locstate	ansl,
	...
	int	dim)
{
	float DP = max(0.0001*p, EPS);
	float dp = DP; 

        float gneg = flame_moc_plus_rh_guessp(st1,st2,st3,ansl,ansr,p,ulfac,
			urfac,heat,nor,pull,purr,dim);
        float gpos = flame_moc_plus_rh_guessp(st1,st2,st3,ansl,ansr,p+DP,
			ulfac,urfac,heat,nor,pull,purr,dim);
        float gdif = max((gpos-gneg)/DP, EPS);
...
LOCAL float flame_moc_plus_rh_findstates(
	float       pneg,
	float       ppos,
	Locstate	st1,
	Locstate	st2,
...
{
	float pmid, gmid;
 	float gneg = flame_moc_plus_rh_guessp(st1,st2,st3,ansl,ansr,pneg,
			ulfac,urfac,heat,nor,pull,purr,dim);
	do
	{
	     pmid = (pneg+ppos)/2;
