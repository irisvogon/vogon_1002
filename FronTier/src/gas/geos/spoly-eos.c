...
LOCAL	float	SPOLY_internal_energy(Locstate);
LOCAL	float	SPOLY_pressure(Locstate);
LOCAL	float	SPOLY_sound_speed_squared(Locstate);
LOCAL	float	SPOLY_acoustic_impedance_squared(Locstate);
LOCAL	float	SPOLY_specific_internal_energy(Locstate);

	/* SECONDARY AND SUPPORTING THERMODYNAMIC FUNCTIONS */
LOCAL	float	SPOLY_specific_enthalpy(Locstate);
LOCAL	float	SPOLY_temperature(Locstate);
LOCAL	float	SPOLY_entropy(Locstate);
LOCAL	float	SPOLY_adiabatic_gamma(Locstate);
LOCAL	float	SPOLY_gruneisen_gamma(Locstate);
LOCAL	float	SPOLY_fundamental_derivative(Locstate);
LOCAL	float	SPOLY_C_V(Locstate);
LOCAL	float	SPOLY_C_P(Locstate);
LOCAL	float	SPOLY_K_S(Locstate);
LOCAL	float	SPOLY_K_T(Locstate);
...
LOCAL	float	SPOLY_dens_Hugoniot(float,Locstate);
LOCAL	void	SPOLY_state_w_pr_on_Hugoniot(Locstate,float,Locstate,int);
LOCAL	bool	SPOLY_state_w_mf_sqr_on_Hugoniot(Locstate,float,Locstate,int);

	/* Velocity Related Hugoniot Functions */
LOCAL	float	SPOLY_pr_normal_vel_wave_curve(float,Locstate);

	/* Purely Thermodynamic Adiabatic Wave Curve Functions */
LOCAL	float	SPOLY_dens_rarefaction(float,Locstate);
LOCAL	float	SPOLY_pressure_rarefaction(float,Locstate);
LOCAL	void	SPOLY_state_on_adiabat_with_pr(Locstate,float,Locstate,int);
LOCAL	void	SPOLY_state_on_adiabat_with_dens(Locstate,float,Locstate,int);

	/* General Wave Curve Functions */
LOCAL	float	SPOLY_mass_flux(float,Locstate);
LOCAL	float	SPOLY_mass_flux_squared(float,Locstate);

	/* Functions for the Evaluation of Riemann Solutions */
LOCAL	float	SPOLY_oned_fan_state(float,Locstate,Locstate,Locstate,int,
				     bool*);

	/* Functions to Compute Riemann Solutions */
LOCAL	float	SPOLY_riemann_wave_curve(Locstate,float);
LOCAL	void	SPOLY_set_state_for_find_mid_state(Locstate,Locstate);
LOCAL	float	SPOLY_eps_for_Godunov(Locstate,float,float);
LOCAL	void	SPOLY_initialize_riemann_solver(Locstate,Locstate,float*,float*,
	                			float,float*,float*,
...
LOCAL	bool	SPOLY_steady_state_wave_curve(float,float,float*,Locstate);
LOCAL	float	SPOLY_pressure_at_sonic_point(float,Locstate);
LOCAL	bool	SPOLY_pr_at_max_turn_angle(float*,float,Locstate);
LOCAL	float	SPOLY_state_in_prandtl_meyer_wave(float,float,Locstate,
	                			  float,Locstate,Locstate,int);

#if defined(COMBUSTION_CODE)
	/* DETONATION SPECIFIC UTILITY FUNCTIONS */
LOCAL	float	SPOLY_CJ_state(Locstate,int,Locstate,int,int);
LOCAL	void	SPOLY_progress_state(float,Locstate,Locstate,float);
LOCAL	void	SPOLY_fprint_combustion_params(FILE*,Gas_param*);
#endif /* defined(COMBUSTION_CODE) */

	/* METHOD OF CHARACTERISTIC FUNCTIONS FOR W_SPEED */
LOCAL	void	SPOLY_neumann_riem_inv_moc(float*,Locstate,float,float,Locstate,
	                                   SIDE,Locstate,float,float*,Front*);
LOCAL	void	SPOLY_shock_ahead_state_riem_inv_moc(float*,Locstate,Locstate,
	                                             Locstate,Locstate,
	                                             Locstate,float,float,
	                                             float,float,float*,float*,
						     int,float,Front*);
LOCAL	bool	SPOLY_shock_moc_plus_rh(float*,Locstate,Locstate,Locstate,
...
LOCAL	float	SPOLY_RT_RS_f(float,Locstate,float,float,float);
LOCAL	void	SPOLY_RT_single_mode_perturbation_state(Locstate,float*,
	                				float,Locstate,float,
	                				float,MODE*,float);
LOCAL	void	SPOLY_KH_single_mode_state(Locstate,float*,float,Locstate,
	                                  float,float,float,MODE*);
LOCAL	void	SPOLY_compute_isothermal_stratified_state(Locstate,float,
	                				  float,Locstate);
LOCAL	void	SPOLY_compute_isentropic_stratified_state(Locstate,float,
	                				  float,Locstate);
	/* Equation of state domain functions */
LOCAL	float	SPOLY_Min_energy(Locstate);
LOCAL	float	SPOLY_Min_pressure(Locstate);

LOCAL	void	set_eos_function_hooks(EOS*);

	/* Polytropic specific utility functions */
LOCAL	bool	spoly_fS(float,float*,POINTER);

LOCAL	float	SPOLY_pressure_from_entropy(Locstate, float);
	...
	Gas_param *params)
{
	...
	float pinf = speos->pinf;
	float einf = speos->einf;
	float gam, GAM, coef6;
	...
	EOS *eos)
{
	...
	eos->_KH_single_mode_state = SPOLY_KH_single_mode_state;
	...
	eos->_pressure_from_entropy = SPOLY_pressure_from_entropy;
}

float	SPOLY_einf(Locstate);

float	SPOLY_einf(Locstate	st)
{
	return Einf(st);
}
...
LOCAL	float	SPOLY_internal_energy(
	Locstate state)
{
	float re = ERROR_FLOAT;
	float rho, p, T, gam, et;
...
LOCAL	float	SPOLY_sound_speed_squared(
	Locstate state)
{
	float c2 = Gamma(state)*stiff_pressure(state)/Dens(state);
	return c2;
...
LOCAL	float	SPOLY_acoustic_impedance_squared(
	Locstate state)
{
	float i2;
...
LOCAL	float	SPOLY_specific_internal_energy(
	Locstate state)
{
	float e = ERROR_FLOAT;
	float rho, p, T, GAM, et;
...
LOCAL	float	SPOLY_specific_enthalpy(
	Locstate state)
{
...
*/
LOCAL	float	SPOLY_entropy(
	Locstate state)
{
	float beta, cv, et, sp, rho;
	if (state_type(state) == VGAS_STATE)
...
//assume density is known
LOCAL	float	SPOLY_pressure_from_entropy(
	Locstate state,
	float	 beta)
{
	float cv, rho;
	...
	return pow(rho,Gamma(state))*exp(beta/cv) - Pinf(state);
}
...
LOCAL	float	SPOLY_adiabatic_gamma(
	Locstate state)
{
...
LOCAL	float	SPOLY_gruneisen_gamma(
	Locstate state)
{
	return GAMMA(state);
...
LOCAL	float	SPOLY_fundamental_derivative(
	Locstate state)
{
...
LOCAL	float	SPOLY_C_P(
	Locstate state)
{
	float cp = Gamma(state)*Cv(state);
	float et = Et(state);
	if (et != 0.0)
	{
	    float rho = Dens(state);
	    float p = stiff_pressure(state);
	    float GAM = GAMMA(state);
	    float x = et*GAM*GAM*pow(rho,GAM);
	...
	int vsize)
{
	...
	int   k;
#if !defined(UNRESTRICTED_THERMODYNAMICS)
	float *min_pressure = vst->min_pressure + offset;
#endif /* !defined(UNRESTRICTED_THERMODYNAMICS) */
...
LOCAL	float	SPOLY_dens_Hugoniot(
	float p1,
	Locstate st0)
{
	float p0, c4;
...
LOCAL	bool	SPOLY_state_w_mf_sqr_on_Hugoniot(
	Locstate st0,
	float m2,
	Locstate st1,
	int stype1)
...
LOCAL	float	SPOLY_pr_normal_vel_wave_curve(
...
LOCAL	float	SPOLY_dens_rarefaction(
	float p1,
	Locstate st0)
{
	float spr = (p1+Pinf(st0))/stiff_pressure(st0);
	float gam = Gamma(st0);
...
LOCAL	float	SPOLY_pressure_rarefaction(
	float rho1,
	Locstate st0)
{
...
LOCAL	void	SPOLY_state_on_adiabat_with_dens(
	...
	int stype1)
{
	float p1s; 	/* pressure of the answer state */
...
LOCAL	float	SPOLY_mass_flux_squared(
	float p,
	Locstate st0)
{
	float p0, rho0;
...
/*ARGSUSED*/
LOCAL	float	SPOLY_oned_fan_state(
	float    w,
	Locstate sta,
	Locstate stb,
	Locstate stm,
...
LOCAL	float	SPOLY_riemann_wave_curve(
	Locstate st0,
	float pstar)
{
	float rho0 = Dens(st0), p0 = stiff_pressure(st0);
	float c1, c2, c3;
...
/*ARGSUSED*/
LOCAL	float	SPOLY_eps_for_Godunov(
	Locstate state,
	float pstar,
	float r_eps)
{
...
LOCAL	bool	SPOLY_steady_state_wave_curve(
	float p1,
	float M0sq,
	float *theta,
	Locstate st0)
{
	float	p0 = stiff_pressure(st0);
	float	dp, tmp;
	float	c4, cf8;
	float	Mnsq, cotb;
	float	A0, A1;
...
LOCAL	float	SPOLY_pressure_at_sonic_point(
	float M0sq,
	Locstate st0)
{
	float x;
...
LOCAL	bool	SPOLY_pr_at_max_turn_angle(
	float *prm,
...
/*ARGSUSED*/
LOCAL	float	SPOLY_state_in_prandtl_meyer_wave(
	float w,
	float A_a,
	Locstate sta,
	float A_b,
	...
	Front     *front)
{
	...
	if (et != 0.0)
	{
	    float a1 = c1sqr/pow(r1,GAM);

	    a += et*gam*GAM*(x-1.0);
	    dS1 *= 1.0 - et*gam*GAM/a1;
	    dS  *= 1.0 - et*gam*GAM/a;
	}
	entropy_coef  = 0.5 * cf3 * dS/R;
	entropy_coef1 = 0.5 * cf3 * dS1/R;
	...
	Front     *front)
{
	...
	float     pr, p2, r1, r2, r3;
	float     alpha1, alpha3, chi1, chi2, chi3;
	float     gam, GAM, cv, cf6, et;
...
typedef struct {
	...
	float sgn;
	float gam, GAM, c4, c8;
	float rada, rfacb, alpha;
} SPOLY_S_MPRH;

LOCAL	bool	SPOLY_shock_moc_plus_rh(
	float     *pt,
	Locstate  sta,
	Locstate  stm,
	Locstate  stb,
	...
	Front	  *front)
{
	...
	float alpha = 0.5*nor[0]*dt*rotational_symmetry();
	...
	if (is_rotational_symmetry() && alpha != 0.0)
	{
	    float rada = pos_radius(pta[0],gr);
	    float radb = pos_radius(ptb[0],gr);
	    float rmin = fabs(pos_radius(0.0,gr));
	...
	Gas_param *params)
{
	POLY_EOS  *peos =   (POLY_EOS *)params->eos;
	SPOLY_EOS *speos = (SPOLY_EOS *)params->eos;
	float gam = peos->gamma;
	float Cv = peos->cv;
	float pinf = speos->pinf;
	float einf = speos->einf;
	float et = speos->et;
	float rhoinf = speos->rhoinf;
...
#else /* defined(float) */
	static const char *fmt = "%f %f %f %f %f";
#endif /* defined(float) */
	...
	Locstate	ref_st)
{
	...
	if (et != 0.0)
	{
	    float     gam, GAM, A, Y;
	    float     rg, rho_last, f;
	    const float epsilon = 10.0*MACH_EPS;/*TOLERANCE*/
	    int       i;
...
//not finished see POLY
LOCAL	void	SPOLY_KH_single_mode_state(
	Locstate	ans,
	float		*coords,
	float		t,
	Locstate	amb_st,
	...
	MODE		*mode)
{
	...
	Set_params(ans,amb_st);
	set_type_of_state(ans,TGAS_STATE);
	csq = sound_speed_squared(amb_st);
	
	amp = mode->amplitude;
	if(debugging("scalamp"))
	    amp *= 0.3;
	...
	//3d only
	FORTRAN_NAME(khstate)(coords,coords+2,&t,&Dens(amb_st),&z_intfc,
	                      &z_bdry,&amp,mode->wave_number,
	                      &mode->growth_rate,&mode->propagation_speed,
	                      &mode->phase,Vel(ans),Vel(ans)+2,
...
LOCAL	float	SPOLY_Min_energy(
	Locstate	state)
{
	float emin = -HUGE_VAL;
#if !defined(UNRESTRICTED_THERMODYNAMICS)
	float pmin = Min_pressure(state), rho = Dens(state);
	float gam = Gamma(state), pinf = Pinf(state), einf = Einf(state);
	float c6 = Coef6(state), min_energy = Params(state)->min_energy;
...
LOCAL	float	SPOLY_Min_pressure(
	Locstate	state)
{
#if defined(UNRESTRICTED_THERMODYNAMICS)
	return -HUGE_VAL;
	...
	POINTER prm)
{
	...
	float pb, ib, ub, pa, ca, ua, gdt;
	float sgn, c4, c8;
	float rho, rr, c, u, ibar;
	float gam;
	float f;
