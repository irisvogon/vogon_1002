...
LOCAL	void	get_thermo_data(Locstate,float*,float*,float*,float*,
				float*,float*);
	...
	INIT_PHYSICS	*ip)
{
	...
	Gas_param	*params;
	REMAP           Remap;
	static	float	L[3] = {0.0, 0.0, 0.0};
	static	float	U[3] = {1.0, 1.0, 1.0};
...
LOCAL void hugoniots_adiabats(
	...
	Gas_param	*params)
{
	bool		keep_prompting = YES;
	char		s[Gets_BUF_SIZE];
	int		number_hug, number_adbt;
	int		i, logs, var, MORE = 0;
	float		rho0, p0, e0, T0, rho1, p1, e1, T1, dpr, drho, pmax;
	float		rhomin, pmin, csq, csq0, S;
	float		log_pmax, log_p1, log_rho0, log_rho1;
	int             stype;
	...
	Gas_param	*params)
{
	...
	float		rho, p, e, T, drho, rhomin, rhomax, csq, S;
	float		log_rho, log_p, log_e, dlog_rho, log_rhomin, log_rhomax;
	char		s[Gets_BUF_SIZE];
...
{
	bool		keep_prompting = YES;
	float		rho, p, e, T, dvar;
	float		indep_var, indep_min, indep_max, csq, S;
	int		i, num_points, var, first = YES, logs = NO;
	float		log_T, log_p, log_e, dlog_var;
	float		log_indmin, log_indmax, log_var;
	char		s[Gets_BUF_SIZE];
	static Locstate state = NULL;
	...
	Gas_param	*params)
{
	...
	int		i, num_points, first = YES, logs = NO;
	float		rho, p, e, T, drho, rhomin, rhomax, csq, S;
	float		log_rho, log_T, log_e, dlog_rho, log_rhomin, log_rhomax;
	char		s[Gets_BUF_SIZE];
	...
	Gas_param	*params)
{
	...
	int		i, num_points, first = YES, logs = NO;
	float		rho, p, e, T, drho, rhomin, rhomax, csq, S;
	float		log_rho, log_p, log_T, dlog_rho, log_rhomin, log_rhomax;
	char		s[Gets_BUF_SIZE];
