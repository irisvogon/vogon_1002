...
LOCAL	float	STELLAR_internal_energy(Locstate);
LOCAL	float	STELLAR_pressure(Locstate);
LOCAL	float	STELLAR_sound_speed_squared(Locstate);
LOCAL	float	STELLAR_acoustic_impedance_squared(Locstate);
LOCAL	float	STELLAR_specific_internal_energy(Locstate);

	/* SECONDARY AND SUPPORTING THERMODYNAMIC FUNCTIONS */
LOCAL	float	STELLAR_specific_enthalpy(Locstate);
LOCAL	float	STELLAR_temperature(Locstate);
LOCAL	float	STELLAR_entropy(Locstate);
LOCAL	float	STELLAR_adiabatic_gamma(Locstate);
LOCAL	float	STELLAR_gruneisen_gamma(Locstate);
LOCAL	float	STELLAR_fundamental_derivative(Locstate);
LOCAL	float	STELLAR_C_V(Locstate);
LOCAL	float	STELLAR_C_P(Locstate);
LOCAL	float	STELLAR_K_S(Locstate);
LOCAL	float	STELLAR_K_T(Locstate);

	/* MATERIAL PROPERTY FUNCTIONS */
LOCAL	float	STELLAR_bulk_viscosity(Locstate);
LOCAL	float	STELLAR_shear_viscosity(Locstate);
LOCAL   float   STELLAR_density(Locstate);
LOCAL 	float   STELLAR_heat_coeff(Locstate);
...
LOCAL	float	STELLAR_dens_Hugoniot(float,Locstate);
LOCAL	void	STELLAR_state_w_pr_on_Hugoniot(Locstate,float,Locstate,int);
LOCAL	bool	STELLAR_state_w_mf_sqr_on_Hugoniot(Locstate,float,Locstate,int);

	/* Velocity Related Hugoniot Functions */
LOCAL	float	STELLAR_pr_normal_vel_wave_curve(float,Locstate);

	/* Purely Thermodynamic Adiabatic Wave Curve Functions */
LOCAL	float	STELLAR_dens_rarefaction(float,Locstate);
LOCAL	float	STELLAR_pressure_rarefaction(float,Locstate);
LOCAL	void	STELLAR_state_on_adiabat_with_pr(Locstate,float,Locstate,int);
LOCAL	void	STELLAR_state_on_adiabat_with_dens(Locstate,float,Locstate,int);

	/* General Wave Curve Functions */
LOCAL	float	STELLAR_mass_flux(float,Locstate);
LOCAL	float	STELLAR_mass_flux_squared(float,Locstate);

	/* Functions for the Evaluation of Riemann Solutions */
LOCAL	float	STELLAR_oned_fan_state(float,Locstate,Locstate,Locstate,int,
				    bool*);

	/* Functions to Compute Riemann Solutions */
LOCAL	float	STELLAR_riemann_wave_curve(Locstate,float);
LOCAL	void	STELLAR_set_state_for_find_mid_state(Locstate,Locstate);
LOCAL	float	STELLAR_eps_for_Godunov(Locstate,float,float);
LOCAL	void	STELLAR_initialize_riemann_solver(Locstate,Locstate,float*,float*,
	                                       float,float*,float*,
...
LOCAL	bool	STELLAR_steady_state_wave_curve(float,float,float*,Locstate);
LOCAL	float	STELLAR_pressure_at_sonic_point(float,Locstate);
LOCAL	bool	STELLAR_pr_at_max_turn_angle(float*,float,Locstate);
LOCAL	float	STELLAR_state_in_prandtl_meyer_wave(float,float,Locstate,float,
	                                         Locstate,Locstate,int);

#if defined(COMBUSTION_CODE)
	/* DETONATION SPECIFIC UTILITY FUNCTIONS */
LOCAL	float	STELLAR_CJ_state(Locstate,int,Locstate,int,int);
LOCAL	void	STELLAR_progress_state(float,Locstate,Locstate,float);
LOCAL	void	STELLAR_fprint_combustion_params(FILE*,Gas_param*);
#endif /* defined(COMBUSTION_CODE) */

	/* METHOD OF CHARACTERISTIC FUNCTIONS FOR W_SPEED */
LOCAL	void	STELLAR_neumann_riem_inv_moc(float*,Locstate,float,float,Locstate,
	                                  SIDE,Locstate,float,float*,Front*);
LOCAL	void	STELLAR_shock_ahead_state_riem_inv_moc(float*,Locstate,Locstate,
	                                            Locstate,Locstate,
	                                            Locstate,float,float,
	                                            float,float,float*,float*,
						    int,float,Front*);
LOCAL	bool	STELLAR_shock_moc_plus_rh(float*,Locstate,Locstate,Locstate,
...
LOCAL	float	STELLAR_RT_RS_f(float,Locstate,float,float,float);
LOCAL	void	STELLAR_RT_single_mode_perturbation_state(Locstate,float*,float,
	                                               Locstate,float,float,
	                                               MODE*,float);
LOCAL	void	STELLAR_KH_single_mode_state(Locstate,float*,float,Locstate,
	                                  float,float,float,MODE*);
LOCAL	void	STELLAR_compute_isothermal_stratified_state(Locstate,float,float,
	                                                 Locstate);
LOCAL	void	STELLAR_compute_isentropic_stratified_state(Locstate,float,float,
	                                                 Locstate);
...
LOCAL	bool	stellar_fS(float,float*,POINTER);
...
LOCAL	float	STELLAR_internal_energy(
	Locstate state)
{
...
LOCAL	float	STELLAR_sound_speed_squared(
	Locstate state)
{
	if (Local_gamma_set(state) == NO)
	    set_local_gamma(state);
...
LOCAL	float	STELLAR_acoustic_impedance_squared(
	Locstate state)
{
	float i2;
	if (Local_gamma_set(state) == NO)
...
LOCAL	float	STELLAR_specific_internal_energy(
	Locstate state)
{
	if (Local_gamma_set(state) == NO)
	    set_local_gamma(state);
...
LOCAL	float	STELLAR_specific_enthalpy(
	Locstate state)
{
	if (Local_gamma_set(state) == NO)
	    set_local_gamma(state);
...
LOCAL	float	STELLAR_temperature(
	Locstate state)
{
	if (Local_gamma_set(state) == NO)
	    set_local_gamma(state);
...
LOCAL	float	STELLAR_adiabatic_gamma(
	Locstate state)
{
	if (Local_gamma_set(state) == NO)
	    set_local_gamma(state);
...
LOCAL	float	STELLAR_gruneisen_gamma(
	Locstate state)
{
	if (Local_gamma_set(state) == NO)
	    set_local_gamma(state);
...
LOCAL	float	STELLAR_fundamental_derivative(
	Locstate state)
{
	if (Local_gamma_set(state) == NO)
	    set_local_gamma(state);
...
LOCAL	float	STELLAR_bulk_viscosity(
	Locstate state)
{
	return 0.0;
}	/*end STELLAR_bulk_viscosity */

LOCAL	float	STELLAR_shear_viscosity(
	Locstate state)
{
	return 0.0;
...
*               STELLAR_heat_coeff():
*/
LOCAL   float   STELLAR_heat_coeff(
        Locstate state)
{
	return 0.0;
...
LOCAL	float	STELLAR_dens_Hugoniot(
	float p1,
	Locstate st0)
{
	float	p0, c4;
...
LOCAL	bool	STELLAR_state_w_mf_sqr_on_Hugoniot(
	Locstate st0,
	float m2,
	Locstate st1,
	int stype1)
...
LOCAL	float	STELLAR_pr_normal_vel_wave_curve(
...
LOCAL	float	STELLAR_dens_rarefaction(
	float p1,
	Locstate st0)
{
...
LOCAL	float	STELLAR_pressure_rarefaction(
	float rho1,
	Locstate st0)
{
	if (Local_gamma_set(st0) == NO)
...
LOCAL	float	STELLAR_mass_flux_squared(
	float p,
	Locstate st0)
{
	float p0, rho0;
...
/*ARGSUSED*/
LOCAL	float	STELLAR_oned_fan_state(
	float    w,
	Locstate sta,
	Locstate stb,
	Locstate stm,
...
LOCAL	float	STELLAR_riemann_wave_curve(
	Locstate st0,
	float pstar)
{
...
/*ARGSUSED*/
LOCAL	float	STELLAR_eps_for_Godunov(
	Locstate state,
	float pstar,
	float r_eps)
{
...
LOCAL	bool	STELLAR_steady_state_wave_curve(
	float p1,
	float M0sq,
	float *theta,
	Locstate st0)
...
LOCAL	float	STELLAR_pressure_at_sonic_point(
	float M0sq,
	Locstate st0)
{
	float x;
...
LOCAL	bool	STELLAR_pr_at_max_turn_angle(
	float *prm,
...
/*ARGSUSED*/
LOCAL	float	STELLAR_state_in_prandtl_meyer_wave(
	float w,
	float A_a,
	Locstate sta,
	float A_b,
...
LOCAL	bool	STELLAR_shock_moc_plus_rh(
	float     *pt,
	Locstate  sta,
	Locstate  stm,
	Locstate  stb,
	...
	Gas_param *params)
{
	...
	float	*xmass = ((STELLAR_EOS *) params->eos)->_xmass;
	float	*aion = ((STELLAR_EOS *) params->eos)->_aion;
	float	*zion = ((STELLAR_EOS *) params->eos)->_zion;
	...
	Gas_param	*params)
{
	...
	int	c, i, nc;
	float	*xmass = eos->_xmass;
	float	*aion = eos->_aion;
	float	*zion = eos->_zion;
	...
	const char	*message2)
{
	...
	int             n,nc,len;
	float		*xmass = speos->_xmass;
	float		*aion = speos->_aion;
	float		*zion = speos->_zion;
	char		s[100];
...
LOCAL	bool	stellar_fS(float x, float *fans, POINTER prm)
{
	float c, rr, v;
...
//TMP
LOCAL	float max_gamma = 0;
LOCAL	float min_gamma = 2;
LOCAL	void set_local_gamma(
	Locstate state)
{
        int     ionmax, input;
        float   *xmass, *aion, *zion;
        float   den, temp, ener, pres,entropy, csond; 
	float   dpd, dpt, c_v, c_p, gammac, gam2, gam3;
