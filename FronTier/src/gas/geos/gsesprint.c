...
LOCAL	float	re_entropy(float*,Front*,POINTER,COMPONENT,Locstate);
LOCAL	float	re_log_pressure(float*,Front*,POINTER,COMPONENT,Locstate);
LOCAL	float	re_riv(float*,Front*,POINTER,COMPONENT,Locstate);
LOCAL	float	re_log_temperature(float*,Front*,POINTER,COMPONENT,Locstate);

LOCAL	float	rt_entropy(float*,Front*,POINTER,COMPONENT,Locstate);
LOCAL	float	rt_log_energy(float*,Front*,POINTER,COMPONENT,Locstate);
LOCAL	float	rt_log_pressure(float*,Front*,POINTER,COMPONENT,Locstate);
LOCAL	float	rt_riv(float*,Front*,POINTER,COMPONENT,Locstate);

LOCAL	float	rs_log_pressure(float*,Front*,POINTER,COMPONENT,Locstate);
LOCAL	float	rs_log_energy(float*,Front*,POINTER,COMPONENT,Locstate);
LOCAL	float	rs_log_temperature(float*,Front*,POINTER,COMPONENT,Locstate);
LOCAL	float	rs_riv(float*,Front*,POINTER,COMPONENT,Locstate);

LOCAL	float	ps_log_density(float*,Front*,POINTER,COMPONENT,Locstate);
LOCAL	float	ps_log_energy(float*,Front*,POINTER,COMPONENT,Locstate);
LOCAL	float	ps_log_temperature(float*,Front*,POINTER,COMPONENT,Locstate);
LOCAL	float	ps_riv(float*,Front*,POINTER,COMPONENT,Locstate);

LOCAL	void	verbose_ses_print_curve_states(CURVE*,SESAME_TABLE_TYPE,
					       SESAME_EOS*);
LOCAL	void	verbose_ses_print_state(const char*,float*,Locstate,
					SESAME_TABLE_TYPE,SESAME_EOS*);

LOCAL	float	vp_log_energy(float*,Front*,POINTER,COMPONENT,Locstate);
LOCAL	float	vp_entropy(float*,Front*,POINTER,COMPONENT,Locstate);
LOCAL	float	vp_log_temperature(float*,Front*,POINTER,COMPONENT,Locstate);
LOCAL	float	vp_riv(float*,Front*,POINTER,COMPONENT,Locstate);
...
LOCAL	void verbose_ses_print_state(
	...
	SESAME_EOS	  *seos)
{
	float 		log_rho, log_T, log_p, log_e;
	float		red_p, red_e, cold_p, cold_e;
	float 		rho, T, p, e, adb_gam, gru_gam;
	float		S, c, stf_gam, ply_gam, R, p0, S0;
	...
	SESAME_EOS	*seos)
{
	...
	float	   (*plot_fns[4])(float*,Front*,POINTER,COMPONENT,Locstate);
...
LOCAL float rt_log_energy(
	...
	Locstate	state)
{
	float	T = ses_rt_temp_from_grid(coords[1],Ses_front_seos(front));
	return plog(ses_rt_rede(state)*T+ses_rt_colde(state));
}		/*end rt_log_energy*/

/*ARGSUSED*/
LOCAL float rt_entropy(
	float		*coords,
	Front		*front,
	POINTER		wave,
	COMPONENT	comp,
	...
	Locstate	state)
{
	...
	float	rho = ses_rt_rho_from_grid(coords[0],seos);
	float	T = ses_rt_temp_from_grid(coords[1],seos);
	float	p = ses_rt_coldp(state) + ses_rt_redp(state)*rho*T;
	float	g = ses_rt_adb_gam(state);
	return 0.5*(sqrt(g*p/rho)+cref)*ses_rt_riv(state);
}		/*end rt_riv*/

/*ARGSUSED*/
LOCAL float re_log_pressure(
	float		*coords,
	Front		*front,
	POINTER		wave,
	COMPONENT	comp,
...
{
	SESAME_EOS	*seos = Ses_front_seos(front);
	float	T = ses_re_temp_from_var(ses_re_Tvar(state),seos);
	float	rho = ses_re_rho_from_grid(coords[0],seos);
	return plog(ses_re_redp(state)*T*rho + ses_re_coldp(state));
}		/*end re_log_pressure*/

/*ARGSUSED*/
LOCAL float re_log_temperature(
	float		*coords,
	Front		*front,
	POINTER		wave,
	COMPONENT	comp,
	...
	Locstate	state)
{
	...
	float	rho = ses_re_rho_from_grid(coords[0],seos);
	float	T = ses_re_temp_from_var(ses_re_Tvar(state),seos);
	float	p = ses_re_coldp(state) + ses_re_redp(state)*rho*T;
	float	g = ses_re_adb_gam(state);
	return 0.5*(sqrt(g*p/rho)+cref)*ses_re_riv(state);
}		/*end re_riv*/

/*ARGSUSED*/
LOCAL float rs_log_pressure(
	float		*coords,
	Front		*front,
	POINTER		wave,
	COMPONENT	comp,
...
{
	SESAME_EOS	*seos = Ses_front_seos(front);
	float	T = ses_rs_temp_from_var(ses_rs_Tvar(state),seos);
	float	rho = ses_rs_rho_from_grid(coords[0],seos);
	return plog(ses_rs_redp(state)*T*rho + ses_rs_coldp(state));
}		/*end rs_log_pressure*/

/*ARGSUSED*/
LOCAL float rs_log_temperature(
	float		*coords,
	Front		*front,
	POINTER		wave,
	COMPONENT	comp,
	...
	Locstate	state)
{
	...
	float	rho = ses_rs_rho_from_grid(coords[0],seos);
	float	T = ses_rs_temp_from_var(ses_rs_Tvar(state),seos);
	float	p = ses_rs_coldp(state) + ses_rs_redp(state)*rho*T;
	float	g = ses_rs_adb_gam(state);
	return 0.5*(sqrt(g*p/rho)+cref)*ses_rs_riv(state);
}		/*end rs_riv*/

/*ARGSUSED*/
LOCAL float ps_log_energy(
	float		*coords,
	Front		*front,
	POINTER		wave,
	COMPONENT	comp,
...
{
	SESAME_EOS	*seos = Ses_front_seos(front);
	float	T = ses_ps_temp_from_var(ses_ps_Tvar(state),seos);

	return plog(ses_ps_rede(state)*T+ses_ps_colde(state));
}		/*end ps_log_energy*/

/*ARGSUSED*/
LOCAL float ps_log_temperature(
	float		*coords,
	Front		*front,
	POINTER		wave,
	COMPONENT	comp,
	...
	Locstate	state)
{
	SESAME_EOS	*seos = Ses_front_seos(front);
	float		cref = Reference_sound_speed(seos);
	float	rho = ses_ps_rho_from_var(ses_ps_rho_var(state),seos);
	float	p = ses_ps_press_from_grid(coords[0],seos);
	float	g = ses_ps_adb_gam(state);
	return 0.5*(sqrt(g*p/rho)+cref)*ses_ps_riv(state);
}		/*end ps_riv*/

/*ARGSUSED*/
LOCAL float vp_entropy(
	float		*coords,
	Front		*front,
	POINTER		wave,
	COMPONENT	comp,
...
/*ARGSUSED*/
LOCAL float vp_log_temperature(
	float		*coords,
	Front		*front,
	POINTER		wave,
	COMPONENT	comp,
...
{
	SESAME_EOS	*seos = Ses_front_seos(front);
	float	T = ses_vp_temp_from_var(ses_vp_Tvar(state),seos);
	...
	Locstate	state)
{
	SESAME_EOS	*seos = Ses_front_seos(front);
	float		cref = Reference_sound_speed(seos);
	float	rho = 1.0/ses_vp_vol_from_grid(coords[0],seos);
	float	p = ses_vp_press_from_grid(coords[1],seos);
	float	g = ses_vp_adb_gam(state);
