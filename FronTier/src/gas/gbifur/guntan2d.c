...
LOCAL	int	untangle_satisfies_CFL(NODE**,float**,Front*,int);
LOCAL	int	vector_vector_unravel(Front*,CROSS*);
LOCAL	int	wave_end_vector_vector_unravel(Front*,CROSS*);
LOCAL	void	adjust_node_velocities_for_symmetry(float**,int);
...
EXPORT	bool	g_replace_unphys_loop(
	NNLIST	*nl,
	...
	int	flag)
{
	...
	static	float	    ***t = NULL;
	static	float	    **nod_v = NULL;
	static	float	    **tgnt = NULL;
...
LOCAL	void adjust_node_velocities_for_symmetry(
	...
	int		dim)
{
	float		mag[2], alpha[2], tmpx[2], tmpy[2];
	float		nx, ny, tx, ty, len;
	static const float 	SYMM_TOL = 0.5;	/* TOLERANCE */
	int		i;
	...
	int		flag)
{
	...
	float		dt;
	float		frac;
	float		max_sep = Max_new_node_separation(front);
