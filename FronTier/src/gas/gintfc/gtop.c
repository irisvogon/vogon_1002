...
LOCAL   bool    intfc_delete_needles_on_node(Front*); 
LOCAL   bool    bond_is_consecutive(BOND*,BOND*); 
 ...
 **/
EXPORT	bool g_redist_on_cc_node(
	Front		*fr)
{
        NODE      **nn; 
...
LOCAL bool bond_is_consecutive(
        BOND  *b1,
        BOND  *b2)
{
 ...
 */
LOCAL bool intfc_delete_needles_on_node(
        Front        *fr)
{
        INTERFACE    *intfc = fr->interf;
        NODE            **n;
 ...
 ** Consider to move to gbifur/guntan2d.c if there is the need.
 **/
EXPORT	bool g_parallel_redist_on_cc_node(
	Front		*fr)
{
        INTERFACE  *sav_intfc;
