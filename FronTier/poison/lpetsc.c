        ...
        Front           *fr)
{
	...
	MPI_Comm_size(PETSC_COMM_WORLD,&comm_size);
        bi_array(&p_cell,Nx,Ny,sizeof(P_CELL2D));
	...
	bi_array(&I_to_ijk,Nx*Ny,2,sizeof( int));
	bi_array(&IJK_to_I,(Nx+4),(Ny+4),sizeof( int));
	tri_array(&grad_phi,Nx,Ny,2,sizeof(double));
	...
	printf("Extreme test SV is [%7.6e ; %7.6e]\n",min,max);
        ...
        if (its == 0 || norm > 0.001)
        {
            ...
            free_these(3,n_dist,I_to_ijk,IJK_to_I);
            free_these(1,p_cell);
            return;
        }
        ...
        int             **IJK_to_I)
{
	...
	uni_array(&crxings,10,sizeof(CRXING*));
	...
	{
            for(i = 0; i < 4; ++i)
              scalar(&state[i],size_of_state(fr->interf));
	}
        ...
        int             **IJK_to_I)
{
	...
	int lbuf[2] = {1,2};
	int ubuf[2] = {1,2};
	scatter_cell_index(fr,lbuf,ubuf,(POINTER)IJK_to_I);
         ...
         RECT_GRID     *gr)
{
       ...
       if(!state)
           scalar(&state,size_of_state(fr->interf));
	...
	int **IJK_to_I)
{
	...
	bi_array(&bfs,2,max_size,sizeof(int));
        bi_array(&bfr,2,max_size,sizeof(int));
